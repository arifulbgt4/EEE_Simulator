# CMP-DIGITAL-LOGIC-DIGITAL-COMPARATOR-MAGNITUDE-F0-CAT - Specify Magnitude catalog preset
...
| Variant | `var-digital-logic-digital-comparator-magnitude` - Magnitude |
| Fidelity | F0 |
| Concern | CAT |
| Release | R5 |
...
Create the complete, immutable catalog definition for **Magnitude** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-digital-logic-digital-comparator` (Digital comparator); task scope: variant `var-digital-logic-digital-comparator-magnitude` (Magnitude); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Digital comparator family-specific implementation reference](../../catalog/families/fam-digital-logic-digital-comparator.md#family-specific-implementation-reference), registry row `fam-digital-logic-digital-comparator`, and variant `var-digital-logic-digital-comparator-magnitude` (Magnitude).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: The selected signed/unsigned equality or magnitude relation maps fixed-width inputs to exact comparison outputs with X propagation.
...
- `docs/tasks/models/cmp-digital-logic-digital-comparator-magnitude-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-digital-logic-digital-comparator-magnitude` (Magnitude), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-DIGITAL-LOGIC-DIGITAL-COMPARATOR-MAGNITUDE-F0-CAT`: variant `var-digital-logic-digital-comparator-magnitude` (Magnitude), fidelity `F0`, concern `CAT`, and requirements REQ-018, REQ-021, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-DIGITAL-LOGIC-DIGITAL-COMPARATOR-MAGNITUDE-F0-CAT-NOMINAL`
- `TEST-CMP-DIGITAL-LOGIC-DIGITAL-COMPARATOR-MAGNITUDE-F0-CAT-BOUNDARY`
- `TEST-CMP-DIGITAL-LOGIC-DIGITAL-COMPARATOR-MAGNITUDE-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-DIGITAL-LOGIC-DIGITAL-COMPARATOR-MAGNITUDE-F0-CAT` exists and is limited to variant `var-digital-logic-digital-comparator-magnitude` (Magnitude), `F0`, and `CAT`.
