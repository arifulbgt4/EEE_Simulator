# CMP-MEMORY-STORAGE-SRAM-DUAL-PORT-F0-CAT - Specify Dual Port catalog preset
...
| Variant | `var-memory-storage-sram-dual-port` - Dual Port |
| Fidelity | F0 |
| Concern | CAT |
| Release | R8 |
...
Create the complete, immutable catalog definition for **Dual Port** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-memory-storage-sram` (Static RAM); task scope: variant `var-memory-storage-sram-dual-port` (Dual Port); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Static RAM family-specific implementation reference](../../catalog/families/fam-memory-storage-sram.md#family-specific-implementation-reference), registry row `fam-memory-storage-sram`, and variant `var-memory-storage-sram-dual-port` (Dual Port).
...
- `docs/tasks/models/cmp-memory-storage-sram-dual-port-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-memory-storage-sram-dual-port` (Dual Port), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-MEMORY-STORAGE-SRAM-DUAL-PORT-F0-CAT`: variant `var-memory-storage-sram-dual-port` (Dual Port), fidelity `F0`, concern `CAT`, and requirements REQ-018, REQ-021, REQ-025, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MEMORY-STORAGE-SRAM-DUAL-PORT-F0-CAT-NOMINAL`
- `TEST-CMP-MEMORY-STORAGE-SRAM-DUAL-PORT-F0-CAT-BOUNDARY`
- `TEST-CMP-MEMORY-STORAGE-SRAM-DUAL-PORT-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-MEMORY-STORAGE-SRAM-DUAL-PORT-F0-CAT` exists and is limited to variant `var-memory-storage-sram-dual-port` (Dual Port), `F0`, and `CAT`.
