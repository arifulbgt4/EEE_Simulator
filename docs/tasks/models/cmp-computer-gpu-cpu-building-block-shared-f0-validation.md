# CMP-COMPUTER-GPU-CPU-BUILDING-BLOCK-SHARED-F0-VALIDATION - Validate CPU building block F0
...
Produce independent validation evidence for **CPU building block** at **F0** without modifying the model under test.
...
- Candidate and independent reference must evaluate the same F0 rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The selected datapath/control/pipeline/cache/MMU block advances a versioned cycle-state transition with fixed-width data, control, timing, and exception rules.
...
- `docs/tasks/models/cmp-computer-gpu-cpu-building-block-shared-f0-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-CPU-CPU_BUILDING_BLOCK-NOMINAL`, `GOLD-CPU-CPU_BUILDING_BLOCK-BOUNDARY`, `GOLD-CPU-CPU_BUILDING_BLOCK-FAILURE` at `F0`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-COMPUTER-GPU-CPU-BUILDING-BLOCK-SHARED-F0-VALIDATION`: shared family scope across `var-computer-gpu-cpu-building-block-alu`, `var-computer-gpu-cpu-building-block-register-file`, `var-computer-gpu-cpu-building-block-control-unit`, `var-computer-gpu-cpu-building-block-datapath`, `var-computer-gpu-cpu-building-block-pipeline`, `var-computer-gpu-cpu-building-block-cache`, `var-computer-gpu-cpu-building-block-mmu`, fidelity `F0`, concern `VALIDATION`, and requirements REQ-025, REQ-027, REQ-028, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-COMPUTER-GPU-CPU-BUILDING-BLOCK-SHARED-F0-VALIDATION-NOMINAL`
- `TEST-CMP-COMPUTER-GPU-CPU-BUILDING-BLOCK-SHARED-F0-VALIDATION-BOUNDARY`
- `TEST-CMP-COMPUTER-GPU-CPU-BUILDING-BLOCK-SHARED-F0-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-COMPUTER-GPU-CPU-BUILDING-BLOCK-SHARED-F0-VALIDATION` exists and is limited to shared family scope across `var-computer-gpu-cpu-building-block-alu`, `var-computer-gpu-cpu-building-block-register-file`, `var-computer-gpu-cpu-building-block-control-unit`, `var-computer-gpu-cpu-building-block-datapath`, `var-computer-gpu-cpu-building-block-pipeline`, `var-computer-gpu-cpu-building-block-cache`, `var-computer-gpu-cpu-building-block-mmu`, `F0`, and `VALIDATION`.
