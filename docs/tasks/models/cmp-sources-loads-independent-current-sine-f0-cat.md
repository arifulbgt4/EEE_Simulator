# CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SINE-F0-CAT - Specify Sine catalog preset
...
| Variant | `var-sources-loads-independent-current-sine` - Sine |
| Fidelity | F0 |
| Concern | CAT |
| Release | R2 |
...
Create the complete, immutable catalog definition for **Sine** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-sources-loads-independent-current` (Independent current source); task scope: variant `var-sources-loads-independent-current-sine` (Sine); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Independent current source family-specific implementation reference](../../catalog/families/fam-sources-loads-independent-current.md#family-specific-implementation-reference), registry row `fam-sources-loads-independent-current`, and variant `var-sources-loads-independent-current-sine` (Sine).
...
- `docs/tasks/models/cmp-sources-loads-independent-current-sine-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-sources-loads-independent-current-sine` (Sine), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SINE-F0-CAT`: variant `var-sources-loads-independent-current-sine` (Sine), fidelity `F0`, concern `CAT`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SINE-F0-CAT-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SINE-F0-CAT-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SINE-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SINE-F0-CAT` exists and is limited to variant `var-sources-loads-independent-current-sine` (Sine), `F0`, and `CAT`.
