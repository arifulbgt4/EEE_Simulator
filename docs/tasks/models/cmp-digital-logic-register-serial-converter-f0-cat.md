# CMP-DIGITAL-LOGIC-REGISTER-SERIAL-CONVERTER-F0-CAT - Specify Serial Converter catalog preset
...
| Variant | `var-digital-logic-register-serial-converter` - Serial Converter |
| Fidelity | F0 |
| Concern | CAT |
| Release | R5 |
...
Create the complete, immutable catalog definition for **Serial Converter** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-digital-logic-register` (Register); task scope: variant `var-digital-logic-register-serial-converter` (Serial Converter); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Register family-specific implementation reference](../../catalog/families/fam-digital-logic-register.md#family-specific-implementation-reference), registry row `fam-digital-logic-register`, and variant `var-digital-logic-register-serial-converter` (Serial Converter).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: A fixed-width state vector updates on the declared edge/enable/load operation and exposes deterministic reset, shift, timing, and X behavior.
...
- `docs/tasks/models/cmp-digital-logic-register-serial-converter-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-digital-logic-register-serial-converter` (Serial Converter), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-DIGITAL-LOGIC-REGISTER-SERIAL-CONVERTER-F0-CAT`: variant `var-digital-logic-register-serial-converter` (Serial Converter), fidelity `F0`, concern `CAT`, and requirements REQ-018, REQ-021, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-DIGITAL-LOGIC-REGISTER-SERIAL-CONVERTER-F0-CAT-NOMINAL`
- `TEST-CMP-DIGITAL-LOGIC-REGISTER-SERIAL-CONVERTER-F0-CAT-BOUNDARY`
- `TEST-CMP-DIGITAL-LOGIC-REGISTER-SERIAL-CONVERTER-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-DIGITAL-LOGIC-REGISTER-SERIAL-CONVERTER-F0-CAT` exists and is limited to variant `var-digital-logic-register-serial-converter` (Serial Converter), `F0`, and `CAT`.
