# CMP-PASSIVES-CHOKE-SHARED-F3-VALIDATION - Validate Choke F3
...
Produce independent validation evidence for **Choke** at **F3** without modifying the model under test.
...
- Stable family: `fam-passives-choke` (Choke); task scope: shared family scope across `var-passives-choke-common-mode`, `var-passives-choke-differential-mode`; fidelity `F3`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F3 rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Each winding follows v = d(lambda)/dt with the declared differential/common-mode inductance matrix, winding loss, and saturation envelope.
...
- `docs/tasks/models/cmp-passives-choke-shared-f3-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-PAS-CHOKE-NOMINAL`, `GOLD-PAS-CHOKE-BOUNDARY`, `GOLD-PAS-CHOKE-FAILURE` at `F3`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-PASSIVES-CHOKE-SHARED-F3-VALIDATION`: shared family scope across `var-passives-choke-common-mode`, `var-passives-choke-differential-mode`, fidelity `F3`, concern `VALIDATION`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-CHOKE-SHARED-F3-VALIDATION-NOMINAL`
- `TEST-CMP-PASSIVES-CHOKE-SHARED-F3-VALIDATION-BOUNDARY`
- `TEST-CMP-PASSIVES-CHOKE-SHARED-F3-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-CHOKE-SHARED-F3-VALIDATION` exists and is limited to shared family scope across `var-passives-choke-common-mode`, `var-passives-choke-differential-mode`, `F3`, and `VALIDATION`.
