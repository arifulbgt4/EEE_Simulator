# CMP-POWER-ELECTRONICS-GATE-DRIVER-SHARED-F0-VALIDATION - Validate Gate driver F0
...
Produce independent validation evidence for **Gate driver** at **F0** without modifying the model under test.
...
- Stable family: `fam-power-electronics-gate-driver` (Gate driver); task scope: shared family scope across `var-power-electronics-gate-driver-low-side`, `var-power-electronics-gate-driver-high-side`, `var-power-electronics-gate-driver-isolated`; fidelity `F0`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F0 rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Input logic commands bounded source/sink gate current with declared delay, dead time, undervoltage lockout, isolation, and supply constraints.
...
- `docs/tasks/models/cmp-power-electronics-gate-driver-shared-f0-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-PWR-GATE_DRIVER-NOMINAL`, `GOLD-PWR-GATE_DRIVER-BOUNDARY`, `GOLD-PWR-GATE_DRIVER-FAILURE` at `F0`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-POWER-ELECTRONICS-GATE-DRIVER-SHARED-F0-VALIDATION`: shared family scope across `var-power-electronics-gate-driver-low-side`, `var-power-electronics-gate-driver-high-side`, `var-power-electronics-gate-driver-isolated`, fidelity `F0`, concern `VALIDATION`, and requirements REQ-009, REQ-010, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-POWER-ELECTRONICS-GATE-DRIVER-SHARED-F0-VALIDATION-NOMINAL`
- `TEST-CMP-POWER-ELECTRONICS-GATE-DRIVER-SHARED-F0-VALIDATION-BOUNDARY`
- `TEST-CMP-POWER-ELECTRONICS-GATE-DRIVER-SHARED-F0-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-POWER-ELECTRONICS-GATE-DRIVER-SHARED-F0-VALIDATION` exists and is limited to shared family scope across `var-power-electronics-gate-driver-low-side`, `var-power-electronics-gate-driver-high-side`, `var-power-electronics-gate-driver-isolated`, `F0`, and `VALIDATION`.
