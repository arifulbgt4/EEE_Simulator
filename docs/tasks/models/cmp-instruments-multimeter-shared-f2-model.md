# CMP-INSTRUMENTS-MULTIMETER-SHARED-F2-MODEL - Implement Multimeter F2 model
...
Implement one model tier, **F2**, for **Multimeter** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-instruments-multimeter` (Multimeter); task scope: shared family scope across `var-instruments-multimeter-handheld`, `var-instruments-multimeter-bench`, `var-instruments-multimeter-clamp`; fidelity `F2`; concern `MODEL`.
...
- Exact F2 execution rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: The selected V/I/R/continuity function computes the documented measurement with range, burden/input impedance, resolution, and overload behavior.
...
- `docs/tasks/models/cmp-instruments-multimeter-shared-f2-model.md`
...
- One `ModelBinding` for `fam-instruments-multimeter` at `F2` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-INSTRUMENTS-MULTIMETER-SHARED-F2-MODEL`: shared family scope across `var-instruments-multimeter-handheld`, `var-instruments-multimeter-bench`, `var-instruments-multimeter-clamp`, fidelity `F2`, concern `MODEL`, and requirements REQ-007, REQ-021, REQ-022, REQ-023, REQ-034, REQ-037.
...
- `TEST-CMP-INSTRUMENTS-MULTIMETER-SHARED-F2-MODEL-NOMINAL`
- `TEST-CMP-INSTRUMENTS-MULTIMETER-SHARED-F2-MODEL-BOUNDARY`
- `TEST-CMP-INSTRUMENTS-MULTIMETER-SHARED-F2-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-INSTRUMENTS-MULTIMETER-SHARED-F2-MODEL` exists and is limited to shared family scope across `var-instruments-multimeter-handheld`, `var-instruments-multimeter-bench`, `var-instruments-multimeter-clamp`, `F2`, and `MODEL`.
