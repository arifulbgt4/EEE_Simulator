# CMP-PASSIVES-PHOTORESISTOR-SHARED-F3-VALIDATION - Validate Photoresistor F3
...
Produce independent validation evidence for **Photoresistor** at **F3** without modifying the model under test.
...
- Stable family: `fam-passives-photoresistor` (Photoresistor); task scope: shared family scope across `var-passives-photoresistor-ldr`; fidelity `F3`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F3 rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Resistance is the declared bounded transfer R = f(illuminance,T) with response-time state; zero or out-of-range stimulus uses explicit saturation behavior.
...
- `docs/tasks/models/cmp-passives-photoresistor-shared-f3-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-PAS-PHOTORESISTOR-NOMINAL`, `GOLD-PAS-PHOTORESISTOR-BOUNDARY`, `GOLD-PAS-PHOTORESISTOR-FAILURE` at `F3`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-PASSIVES-PHOTORESISTOR-SHARED-F3-VALIDATION`: shared family scope across `var-passives-photoresistor-ldr`, fidelity `F3`, concern `VALIDATION`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-PHOTORESISTOR-SHARED-F3-VALIDATION-NOMINAL`
- `TEST-CMP-PASSIVES-PHOTORESISTOR-SHARED-F3-VALIDATION-BOUNDARY`
- `TEST-CMP-PASSIVES-PHOTORESISTOR-SHARED-F3-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-PHOTORESISTOR-SHARED-F3-VALIDATION` exists and is limited to shared family scope across `var-passives-photoresistor-ldr`, `F3`, and `VALIDATION`.
