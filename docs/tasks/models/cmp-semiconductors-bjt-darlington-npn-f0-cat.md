# CMP-SEMICONDUCTORS-BJT-DARLINGTON-NPN-F0-CAT - Specify Darlington Npn catalog preset
...
| Variant | `var-semiconductors-bjt-darlington-npn` - Darlington Npn |
| Fidelity | F0 |
| Concern | CAT |
| Release | R3 |
...
Create the complete, immutable catalog definition for **Darlington Npn** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-semiconductors-bjt` (Bipolar junction transistor); task scope: variant `var-semiconductors-bjt-darlington-npn` (Darlington Npn); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Bipolar junction transistor family-specific implementation reference](../../catalog/families/fam-semiconductors-bjt.md#family-specific-implementation-reference), registry row `fam-semiconductors-bjt`, and variant `var-semiconductors-bjt-darlington-npn` (Darlington Npn).
...
- `docs/tasks/models/cmp-semiconductors-bjt-darlington-npn-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-semiconductors-bjt-darlington-npn` (Darlington Npn), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-SEMICONDUCTORS-BJT-DARLINGTON-NPN-F0-CAT`: variant `var-semiconductors-bjt-darlington-npn` (Darlington Npn), fidelity `F0`, concern `CAT`, and requirements REQ-009, REQ-010, REQ-011, REQ-012, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SEMICONDUCTORS-BJT-DARLINGTON-NPN-F0-CAT-NOMINAL`
- `TEST-CMP-SEMICONDUCTORS-BJT-DARLINGTON-NPN-F0-CAT-BOUNDARY`
- `TEST-CMP-SEMICONDUCTORS-BJT-DARLINGTON-NPN-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-SEMICONDUCTORS-BJT-DARLINGTON-NPN-F0-CAT` exists and is limited to variant `var-semiconductors-bjt-darlington-npn` (Darlington Npn), `F0`, and `CAT`.
