# CMP-DEFERRED-RESEARCH-TCAD-PHYSICS-SEMICONDUCTOR-DEVICE-F5-CAT - Specify Semiconductor Device catalog preset
...
| Variant | `var-deferred-research-tcad-physics-semiconductor-device` - Semiconductor Device |
| Fidelity | F5 |
| Concern | CAT |
| Release | R13 |
...
Create the complete, immutable catalog definition for **Semiconductor Device** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-deferred-research-tcad-physics` (TCAD physical device model); task scope: variant `var-deferred-research-tcad-physics-semiconductor-device` (Semiconductor Device); fidelity `F5`; concern `CAT`.
...
- Canonical source: [TCAD physical device model family-specific implementation reference](../../catalog/families/fam-deferred-research-tcad-physics.md#family-specific-implementation-reference), registry row `fam-deferred-research-tcad-physics`, and variant `var-deferred-research-tcad-physics-semiconductor-device` (Semiconductor Device).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: F5 behavior exists only as an isolated adapter to a pinned TCAD solver, mesh, material, boundary-condition, and dataset configuration.
...
- `docs/tasks/models/cmp-deferred-research-tcad-physics-semiconductor-device-f5-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-deferred-research-tcad-physics-semiconductor-device` (Semiconductor Device), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-DEFERRED-RESEARCH-TCAD-PHYSICS-SEMICONDUCTOR-DEVICE-F5-CAT`: variant `var-deferred-research-tcad-physics-semiconductor-device` (Semiconductor Device), fidelity `F5`, concern `CAT`, and requirements REQ-001, REQ-002, REQ-022, REQ-023, REQ-028, REQ-034, REQ-037, REQ-038.
...
- `TEST-CMP-DEFERRED-RESEARCH-TCAD-PHYSICS-SEMICONDUCTOR-DEVICE-F5-CAT-NOMINAL`
- `TEST-CMP-DEFERRED-RESEARCH-TCAD-PHYSICS-SEMICONDUCTOR-DEVICE-F5-CAT-BOUNDARY`
- `TEST-CMP-DEFERRED-RESEARCH-TCAD-PHYSICS-SEMICONDUCTOR-DEVICE-F5-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-DEFERRED-RESEARCH-TCAD-PHYSICS-SEMICONDUCTOR-DEVICE-F5-CAT` exists and is limited to variant `var-deferred-research-tcad-physics-semiconductor-device` (Semiconductor Device), `F5`, and `CAT`.
