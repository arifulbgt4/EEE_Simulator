# CMP-SENSORS-ENVIRONMENT-MAGNETIC-MOTION-SENSOR-ACCELEROMETER-F0-CAT - Specify Accelerometer catalog preset
...
| Variant | `var-sensors-environment-magnetic-motion-sensor-accelerometer` - Accelerometer |
| Fidelity | F0 |
| Concern | CAT |
| Release | R7 |
...
Create the complete, immutable catalog definition for **Accelerometer** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-sensors-environment-magnetic-motion-sensor` (Magnetic and motion sensor); task scope: variant `var-sensors-environment-magnetic-motion-sensor-accelerometer` (Accelerometer); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Magnetic and motion sensor family-specific implementation reference](../../catalog/families/fam-sensors-environment-magnetic-motion-sensor.md#family-specific-implementation-reference), registry row `fam-sensors-environment-magnetic-motion-sensor`, and variant `var-sensors-environment-magnetic-motion-sensor-accelerometer` (Accelerometer).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Output follows the selected magnetic/acceleration/angular-rate/position transfer and axis transform with bandwidth, bias, range, and noise.
...
- `docs/tasks/models/cmp-sensors-environment-magnetic-motion-sensor-accelerometer-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-sensors-environment-magnetic-motion-sensor-accelerometer` (Accelerometer), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-SENSORS-ENVIRONMENT-MAGNETIC-MOTION-SENSOR-ACCELEROMETER-F0-CAT`: variant `var-sensors-environment-magnetic-motion-sensor-accelerometer` (Accelerometer), fidelity `F0`, concern `CAT`, and requirements REQ-014, REQ-015, REQ-018, REQ-020, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SENSORS-ENVIRONMENT-MAGNETIC-MOTION-SENSOR-ACCELEROMETER-F0-CAT-NOMINAL`
- `TEST-CMP-SENSORS-ENVIRONMENT-MAGNETIC-MOTION-SENSOR-ACCELEROMETER-F0-CAT-BOUNDARY`
- `TEST-CMP-SENSORS-ENVIRONMENT-MAGNETIC-MOTION-SENSOR-ACCELEROMETER-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-SENSORS-ENVIRONMENT-MAGNETIC-MOTION-SENSOR-ACCELEROMETER-F0-CAT` exists and is limited to variant `var-sensors-environment-magnetic-motion-sensor-accelerometer` (Accelerometer), `F0`, and `CAT`.
