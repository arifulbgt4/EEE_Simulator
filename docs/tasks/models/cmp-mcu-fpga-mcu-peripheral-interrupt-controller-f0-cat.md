# CMP-MCU-FPGA-MCU-PERIPHERAL-INTERRUPT-CONTROLLER-F0-CAT - Specify Interrupt Controller catalog preset
...
| Variant | `var-mcu-fpga-mcu-peripheral-interrupt-controller` - Interrupt Controller |
| Fidelity | F0 |
| Concern | CAT |
| Release | R7 |
...
Create the complete, immutable catalog definition for **Interrupt Controller** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-mcu-fpga-mcu-peripheral` (MCU peripheral); task scope: variant `var-mcu-fpga-mcu-peripheral-interrupt-controller` (Interrupt Controller); fidelity `F0`; concern `CAT`.
...
- Canonical source: [MCU peripheral family-specific implementation reference](../../catalog/families/fam-mcu-fpga-mcu-peripheral.md#family-specific-implementation-reference), registry row `fam-mcu-fpga-mcu-peripheral`, and variant `var-mcu-fpga-mcu-peripheral-interrupt-controller` (Interrupt Controller).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: The selected timer/PWM/watchdog/interrupt/DMA peripheral uses a versioned register and event state machine with declared clock/reset behavior.
...
- `docs/tasks/models/cmp-mcu-fpga-mcu-peripheral-interrupt-controller-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-mcu-fpga-mcu-peripheral-interrupt-controller` (Interrupt Controller), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-MCU-FPGA-MCU-PERIPHERAL-INTERRUPT-CONTROLLER-F0-CAT`: variant `var-mcu-fpga-mcu-peripheral-interrupt-controller` (Interrupt Controller), fidelity `F0`, concern `CAT`, and requirements REQ-020, REQ-021, REQ-024, REQ-025, REQ-026, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MCU-FPGA-MCU-PERIPHERAL-INTERRUPT-CONTROLLER-F0-CAT-NOMINAL`
- `TEST-CMP-MCU-FPGA-MCU-PERIPHERAL-INTERRUPT-CONTROLLER-F0-CAT-BOUNDARY`
- `TEST-CMP-MCU-FPGA-MCU-PERIPHERAL-INTERRUPT-CONTROLLER-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-MCU-FPGA-MCU-PERIPHERAL-INTERRUPT-CONTROLLER-F0-CAT` exists and is limited to variant `var-mcu-fpga-mcu-peripheral-interrupt-controller` (Interrupt Controller), `F0`, and `CAT`.
