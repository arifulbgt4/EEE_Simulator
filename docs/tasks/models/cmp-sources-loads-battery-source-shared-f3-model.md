# CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F3-MODEL - Implement Battery source F3 model
...
Implement one model tier, **F3**, for **Battery source** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-sources-loads-battery-source` (Battery source); task scope: shared family scope across `var-sources-loads-battery-source-battery-pack`; fidelity `F3`; concern `MODEL`.
...
- Exact F3 execution rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Terminal voltage is OCV(SOC,T) - I*R_internal(SOC,T), with SOC advanced by charge balance dSOC/dt = -I/Q_capacity under the declared sign convention.
...
- `docs/tasks/models/cmp-sources-loads-battery-source-shared-f3-model.md`
...
- One `ModelBinding` for `fam-sources-loads-battery-source` at `F3` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F3-MODEL`: shared family scope across `var-sources-loads-battery-source-battery-pack`, fidelity `F3`, concern `MODEL`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F3-MODEL-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F3-MODEL-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F3-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F3-MODEL` exists and is limited to shared family scope across `var-sources-loads-battery-source-battery-pack`, `F3`, and `MODEL`.
