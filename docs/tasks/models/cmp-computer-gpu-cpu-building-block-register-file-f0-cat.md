# CMP-COMPUTER-GPU-CPU-BUILDING-BLOCK-REGISTER-FILE-F0-CAT - Specify Register File catalog preset
...
| Variant | `var-computer-gpu-cpu-building-block-register-file` - Register File |
| Fidelity | F0 |
| Concern | CAT |
| Release | R8 |
...
Create the complete, immutable catalog definition for **Register File** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-computer-gpu-cpu-building-block` (CPU building block); task scope: variant `var-computer-gpu-cpu-building-block-register-file` (Register File); fidelity `F0`; concern `CAT`.
...
- Canonical source: [CPU building block family-specific implementation reference](../../catalog/families/fam-computer-gpu-cpu-building-block.md#family-specific-implementation-reference), registry row `fam-computer-gpu-cpu-building-block`, and variant `var-computer-gpu-cpu-building-block-register-file` (Register File).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: The selected datapath/control/pipeline/cache/MMU block advances a versioned cycle-state transition with fixed-width data, control, timing, and exception rules.
...
- `docs/tasks/models/cmp-computer-gpu-cpu-building-block-register-file-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-computer-gpu-cpu-building-block-register-file` (Register File), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-COMPUTER-GPU-CPU-BUILDING-BLOCK-REGISTER-FILE-F0-CAT`: variant `var-computer-gpu-cpu-building-block-register-file` (Register File), fidelity `F0`, concern `CAT`, and requirements REQ-025, REQ-027, REQ-028, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-COMPUTER-GPU-CPU-BUILDING-BLOCK-REGISTER-FILE-F0-CAT-NOMINAL`
- `TEST-CMP-COMPUTER-GPU-CPU-BUILDING-BLOCK-REGISTER-FILE-F0-CAT-BOUNDARY`
- `TEST-CMP-COMPUTER-GPU-CPU-BUILDING-BLOCK-REGISTER-FILE-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-COMPUTER-GPU-CPU-BUILDING-BLOCK-REGISTER-FILE-F0-CAT` exists and is limited to variant `var-computer-gpu-cpu-building-block-register-file` (Register File), `F0`, and `CAT`.
