# CMP-MEMORY-STORAGE-FUNCTIONAL-STORAGE-BLOCK-DEVICE-F0-CAT - Specify Block Device catalog preset
...
| Variant | `var-memory-storage-functional-storage-block-device` - Block Device |
| Fidelity | F0 |
| Concern | CAT |
| Release | R8 |
...
Create the complete, immutable catalog definition for **Block Device** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-memory-storage-functional-storage` (Functional storage); task scope: variant `var-memory-storage-functional-storage-block-device` (Block Device); fidelity `F0`; concern `CAT`.
...
- `docs/tasks/models/cmp-memory-storage-functional-storage-block-device-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-memory-storage-functional-storage-block-device` (Block Device), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-MEMORY-STORAGE-FUNCTIONAL-STORAGE-BLOCK-DEVICE-F0-CAT`: variant `var-memory-storage-functional-storage-block-device` (Block Device), fidelity `F0`, concern `CAT`, and requirements REQ-018, REQ-021, REQ-025, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MEMORY-STORAGE-FUNCTIONAL-STORAGE-BLOCK-DEVICE-F0-CAT-NOMINAL`
- `TEST-CMP-MEMORY-STORAGE-FUNCTIONAL-STORAGE-BLOCK-DEVICE-F0-CAT-BOUNDARY`
- `TEST-CMP-MEMORY-STORAGE-FUNCTIONAL-STORAGE-BLOCK-DEVICE-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-MEMORY-STORAGE-FUNCTIONAL-STORAGE-BLOCK-DEVICE-F0-CAT` exists and is limited to variant `var-memory-storage-functional-storage-block-device` (Block Device), `F0`, and `CAT`.
