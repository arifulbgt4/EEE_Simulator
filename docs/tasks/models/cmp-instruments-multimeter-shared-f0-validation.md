# CMP-INSTRUMENTS-MULTIMETER-SHARED-F0-VALIDATION - Validate Multimeter F0
...
Produce independent validation evidence for **Multimeter** at **F0** without modifying the model under test.
...
- Stable family: `fam-instruments-multimeter` (Multimeter); task scope: shared family scope across `var-instruments-multimeter-handheld`, `var-instruments-multimeter-bench`, `var-instruments-multimeter-clamp`; fidelity `F0`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F0 rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The selected V/I/R/continuity function computes the documented measurement with range, burden/input impedance, resolution, and overload behavior.
...
- `docs/tasks/models/cmp-instruments-multimeter-shared-f0-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-INS-MULTIMETER-NOMINAL`, `GOLD-INS-MULTIMETER-BOUNDARY`, `GOLD-INS-MULTIMETER-FAILURE` at `F0`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-INSTRUMENTS-MULTIMETER-SHARED-F0-VALIDATION`: shared family scope across `var-instruments-multimeter-handheld`, `var-instruments-multimeter-bench`, `var-instruments-multimeter-clamp`, fidelity `F0`, concern `VALIDATION`, and requirements REQ-007, REQ-021, REQ-022, REQ-023, REQ-034, REQ-037.
...
- `TEST-CMP-INSTRUMENTS-MULTIMETER-SHARED-F0-VALIDATION-NOMINAL`
- `TEST-CMP-INSTRUMENTS-MULTIMETER-SHARED-F0-VALIDATION-BOUNDARY`
- `TEST-CMP-INSTRUMENTS-MULTIMETER-SHARED-F0-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-INSTRUMENTS-MULTIMETER-SHARED-F0-VALIDATION` exists and is limited to shared family scope across `var-instruments-multimeter-handheld`, `var-instruments-multimeter-bench`, `var-instruments-multimeter-clamp`, `F0`, and `VALIDATION`.
