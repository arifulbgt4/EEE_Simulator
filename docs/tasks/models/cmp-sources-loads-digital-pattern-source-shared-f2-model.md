# CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F2-MODEL - Implement Digital pattern source F2 model
...
Implement one model tier, **F2**, for **Digital pattern source** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-sources-loads-digital-pattern-source` (Digital pattern source); task scope: shared family scope across `var-sources-loads-digital-pattern-source-pattern`; fidelity `F2`; concern `MODEL`.
...
- `docs/tasks/models/cmp-sources-loads-digital-pattern-source-shared-f2-model.md`
...
- One `ModelBinding` for `fam-sources-loads-digital-pattern-source` at `F2` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F2-MODEL`: shared family scope across `var-sources-loads-digital-pattern-source-pattern`, fidelity `F2`, concern `MODEL`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F2-MODEL-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F2-MODEL-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F2-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F2-MODEL` exists and is limited to shared family scope across `var-sources-loads-digital-pattern-source-pattern`, `F2`, and `MODEL`.
