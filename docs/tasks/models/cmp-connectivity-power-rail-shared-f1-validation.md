# CMP-CONNECTIVITY-POWER-RAIL-SHARED-F1-VALIDATION - Validate Power rail F1
...
Produce independent validation evidence for **Power rail** at **F1** without modifying the model under test.
...
- Stable family: `fam-connectivity-power-rail` (Power rail); task scope: shared family scope across `var-connectivity-power-rail-parameterized-rail`; fidelity `F1`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F1 rule: Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: A rail creates a named power net; it imposes a voltage equation only when a source/rail profile explicitly declares one, otherwise it is connectivity metadata.
...
- `docs/tasks/models/cmp-connectivity-power-rail-shared-f1-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-CON-POWER_RAIL-NOMINAL`, `GOLD-CON-POWER_RAIL-BOUNDARY`, `GOLD-CON-POWER_RAIL-FAILURE` at `F1`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-CONNECTIVITY-POWER-RAIL-SHARED-F1-VALIDATION`: shared family scope across `var-connectivity-power-rail-parameterized-rail`, fidelity `F1`, concern `VALIDATION`, and requirements REQ-004, REQ-005, REQ-008, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-CONNECTIVITY-POWER-RAIL-SHARED-F1-VALIDATION-NOMINAL`
- `TEST-CMP-CONNECTIVITY-POWER-RAIL-SHARED-F1-VALIDATION-BOUNDARY`
- `TEST-CMP-CONNECTIVITY-POWER-RAIL-SHARED-F1-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-CONNECTIVITY-POWER-RAIL-SHARED-F1-VALIDATION` exists and is limited to shared family scope across `var-connectivity-power-rail-parameterized-rail`, `F1`, and `VALIDATION`.
