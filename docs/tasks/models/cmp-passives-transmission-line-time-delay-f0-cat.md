# CMP-PASSIVES-TRANSMISSION-LINE-TIME-DELAY-F0-CAT - Specify Time Delay catalog preset
...
| Variant | `var-passives-transmission-line-time-delay` - Time Delay |
| Fidelity | F0 |
| Concern | CAT |
| Release | R2 |
...
Create the complete, immutable catalog definition for **Time Delay** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-passives-transmission-line` (Transmission line); task scope: variant `var-passives-transmission-line-time-delay` (Time Delay); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Transmission line family-specific implementation reference](../../catalog/families/fam-passives-transmission-line.md#family-specific-implementation-reference), registry row `fam-passives-transmission-line`, and variant `var-passives-transmission-line-time-delay` (Time Delay).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Voltage/current propagation follows the declared characteristic impedance and delay or RLGC telegrapher model; terminations and reference direction are explicit.
...
- `docs/tasks/models/cmp-passives-transmission-line-time-delay-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-passives-transmission-line-time-delay` (Time Delay), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-PASSIVES-TRANSMISSION-LINE-TIME-DELAY-F0-CAT`: variant `var-passives-transmission-line-time-delay` (Time Delay), fidelity `F0`, concern `CAT`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-TRANSMISSION-LINE-TIME-DELAY-F0-CAT-NOMINAL`
- `TEST-CMP-PASSIVES-TRANSMISSION-LINE-TIME-DELAY-F0-CAT-BOUNDARY`
- `TEST-CMP-PASSIVES-TRANSMISSION-LINE-TIME-DELAY-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-TRANSMISSION-LINE-TIME-DELAY-F0-CAT` exists and is limited to variant `var-passives-transmission-line-time-delay` (Time Delay), `F0`, and `CAT`.
