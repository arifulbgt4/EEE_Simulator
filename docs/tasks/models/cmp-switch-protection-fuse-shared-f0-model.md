# CMP-SWITCH-PROTECTION-FUSE-SHARED-F0-MODEL - Implement Fuse and resettable protector F0 model
...
Implement one model tier, **F0**, for **Fuse and resettable protector** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-switch-protection-fuse` (Fuse and resettable protector); task scope: shared family scope across `var-switch-protection-fuse-fast-blow`, `var-switch-protection-fuse-slow-blow`, `var-switch-protection-fuse-ptc-resettable`; fidelity `F0`; concern `MODEL`.
...
- Exact F0 execution rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The element integrates the declared I^2*t or thermal damage measure and transitions irreversibly or resetably to its open/high-resistance state at threshold.
...
- `docs/tasks/models/cmp-switch-protection-fuse-shared-f0-model.md`
...
- One `ModelBinding` for `fam-switch-protection-fuse` at `F0` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-SWITCH-PROTECTION-FUSE-SHARED-F0-MODEL`: shared family scope across `var-switch-protection-fuse-fast-blow`, `var-switch-protection-fuse-slow-blow`, `var-switch-protection-fuse-ptc-resettable`, fidelity `F0`, concern `MODEL`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SWITCH-PROTECTION-FUSE-SHARED-F0-MODEL-NOMINAL`
- `TEST-CMP-SWITCH-PROTECTION-FUSE-SHARED-F0-MODEL-BOUNDARY`
- `TEST-CMP-SWITCH-PROTECTION-FUSE-SHARED-F0-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-SWITCH-PROTECTION-FUSE-SHARED-F0-MODEL` exists and is limited to shared family scope across `var-switch-protection-fuse-fast-blow`, `var-switch-protection-fuse-slow-blow`, `var-switch-protection-fuse-ptc-resettable`, `F0`, and `MODEL`.
