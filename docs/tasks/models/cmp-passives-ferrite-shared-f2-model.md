# CMP-PASSIVES-FERRITE-SHARED-F2-MODEL - Implement Ferrite suppressor F2 model
...
Implement one model tier, **F2**, for **Ferrite suppressor** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-passives-ferrite` (Ferrite suppressor); task scope: shared family scope across `var-passives-ferrite-bead`; fidelity `F2`; concern `MODEL`.
...
- Exact F2 execution rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: The branch uses the declared frequency-dependent complex impedance Z(f,T,bias); time-domain use requires an approved causal fitted model.
...
- `docs/tasks/models/cmp-passives-ferrite-shared-f2-model.md`
...
- One `ModelBinding` for `fam-passives-ferrite` at `F2` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-PASSIVES-FERRITE-SHARED-F2-MODEL`: shared family scope across `var-passives-ferrite-bead`, fidelity `F2`, concern `MODEL`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-FERRITE-SHARED-F2-MODEL-NOMINAL`
- `TEST-CMP-PASSIVES-FERRITE-SHARED-F2-MODEL-BOUNDARY`
- `TEST-CMP-PASSIVES-FERRITE-SHARED-F2-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-FERRITE-SHARED-F2-MODEL` exists and is limited to shared family scope across `var-passives-ferrite-bead`, `F2`, and `MODEL`.
