# CMP-MEMORY-STORAGE-EEPROM-FLASH-NAND-FLASH-F0-CAT - Specify Nand Flash catalog preset
...
| Variant | `var-memory-storage-eeprom-flash-nand-flash` - Nand Flash |
| Fidelity | F0 |
| Concern | CAT |
| Release | R8 |
...
Create the complete, immutable catalog definition for **Nand Flash** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-memory-storage-eeprom-flash` (EEPROM and Flash); task scope: variant `var-memory-storage-eeprom-flash-nand-flash` (Nand Flash); fidelity `F0`; concern `CAT`.
...
- Canonical source: [EEPROM and Flash family-specific implementation reference](../../catalog/families/fam-memory-storage-eeprom-flash.md#family-specific-implementation-reference), registry row `fam-memory-storage-eeprom-flash`, and variant `var-memory-storage-eeprom-flash-nand-flash` (Nand Flash).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: A persistent word array follows declared read/program/erase commands, timing, granularity, initialization, endurance metadata, and invalid-command behavior.
...
- `docs/tasks/models/cmp-memory-storage-eeprom-flash-nand-flash-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-memory-storage-eeprom-flash-nand-flash` (Nand Flash), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-MEMORY-STORAGE-EEPROM-FLASH-NAND-FLASH-F0-CAT`: variant `var-memory-storage-eeprom-flash-nand-flash` (Nand Flash), fidelity `F0`, concern `CAT`, and requirements REQ-018, REQ-021, REQ-025, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MEMORY-STORAGE-EEPROM-FLASH-NAND-FLASH-F0-CAT-NOMINAL`
- `TEST-CMP-MEMORY-STORAGE-EEPROM-FLASH-NAND-FLASH-F0-CAT-BOUNDARY`
- `TEST-CMP-MEMORY-STORAGE-EEPROM-FLASH-NAND-FLASH-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-MEMORY-STORAGE-EEPROM-FLASH-NAND-FLASH-F0-CAT` exists and is limited to variant `var-memory-storage-eeprom-flash-nand-flash` (Nand Flash), `F0`, and `CAT`.
