# CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F2-MODEL - Implement Independent current source F2 model
...
Implement one model tier, **F2**, for **Independent current source** using the family equations/behavior and no higher-fidelity claims.
...
- Exact F2 execution rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: The oriented branch current is i(P->N) = I(t), with I(t) supplied by the selected DC/AC/transient profile.
...
- `docs/tasks/models/cmp-sources-loads-independent-current-shared-f2-model.md`
...
- One `ModelBinding` for `fam-sources-loads-independent-current` at `F2` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F2-MODEL`: shared family scope across `var-sources-loads-independent-current-dc`, `var-sources-loads-independent-current-ac-small-signal`, `var-sources-loads-independent-current-sine`, `var-sources-loads-independent-current-pulse`, fidelity `F2`, concern `MODEL`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F2-MODEL-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F2-MODEL-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F2-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F2-MODEL` exists and is limited to shared family scope across `var-sources-loads-independent-current-dc`, `var-sources-loads-independent-current-ac-small-signal`, `var-sources-loads-independent-current-sine`, `var-sources-loads-independent-current-pulse`, `F2`, and `MODEL`.
