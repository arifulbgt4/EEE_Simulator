# CMP-SEMICONDUCTORS-OPTO-ISOLATOR-PHOTOTRANSISTOR-OUTPUT-F0-CAT - Specify Phototransistor Output catalog preset
...
| Variant | `var-semiconductors-opto-isolator-phototransistor-output` - Phototransistor Output |
| Fidelity | F0 |
| Concern | CAT |
| Release | R3 |
...
Create the complete, immutable catalog definition for **Phototransistor Output** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-semiconductors-opto-isolator` (Optically isolated semiconductor); task scope: variant `var-semiconductors-opto-isolator-phototransistor-output` (Phototransistor Output); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Optically isolated semiconductor family-specific implementation reference](../../catalog/families/fam-semiconductors-opto-isolator.md#family-specific-implementation-reference), registry row `fam-semiconductors-opto-isolator`, and variant `var-semiconductors-opto-isolator-phototransistor-output` (Phototransistor Output).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Input diode current produces output photocurrent or logic transfer through declared CTR, delay, saturation, isolation leakage, and temperature behavior.
...
- `docs/tasks/models/cmp-semiconductors-opto-isolator-phototransistor-output-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-semiconductors-opto-isolator-phototransistor-output` (Phototransistor Output), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-SEMICONDUCTORS-OPTO-ISOLATOR-PHOTOTRANSISTOR-OUTPUT-F0-CAT`: variant `var-semiconductors-opto-isolator-phototransistor-output` (Phototransistor Output), fidelity `F0`, concern `CAT`, and requirements REQ-009, REQ-010, REQ-011, REQ-012, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SEMICONDUCTORS-OPTO-ISOLATOR-PHOTOTRANSISTOR-OUTPUT-F0-CAT-NOMINAL`
- `TEST-CMP-SEMICONDUCTORS-OPTO-ISOLATOR-PHOTOTRANSISTOR-OUTPUT-F0-CAT-BOUNDARY`
- `TEST-CMP-SEMICONDUCTORS-OPTO-ISOLATOR-PHOTOTRANSISTOR-OUTPUT-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-SEMICONDUCTORS-OPTO-ISOLATOR-PHOTOTRANSISTOR-OUTPUT-F0-CAT` exists and is limited to variant `var-semiconductors-opto-isolator-phototransistor-output` (Phototransistor Output), `F0`, and `CAT`.
