# CMP-DIGITAL-LOGIC-FLIP-FLOP-JK-F0-CAT - Specify Jk catalog preset
...
| Variant | `var-digital-logic-flip-flop-jk` - Jk |
| Fidelity | F0 |
| Concern | CAT |
| Release | R5 |
...
Create the complete, immutable catalog definition for **Jk** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-digital-logic-flip-flop` (Flip-flop); task scope: variant `var-digital-logic-flip-flop-jk` (Jk); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Flip-flop family-specific implementation reference](../../catalog/families/fam-digital-logic-flip-flop.md#family-specific-implementation-reference), registry row `fam-digital-logic-flip-flop`, and variant `var-digital-logic-flip-flop-jk` (Jk).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Stored state updates on the declared clock edge from D/JK/T/SR inputs, subject to async controls, setup/hold, clock, and X rules.
...
- `docs/tasks/models/cmp-digital-logic-flip-flop-jk-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-digital-logic-flip-flop-jk` (Jk), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-DIGITAL-LOGIC-FLIP-FLOP-JK-F0-CAT`: variant `var-digital-logic-flip-flop-jk` (Jk), fidelity `F0`, concern `CAT`, and requirements REQ-018, REQ-021, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-DIGITAL-LOGIC-FLIP-FLOP-JK-F0-CAT-NOMINAL`
- `TEST-CMP-DIGITAL-LOGIC-FLIP-FLOP-JK-F0-CAT-BOUNDARY`
- `TEST-CMP-DIGITAL-LOGIC-FLIP-FLOP-JK-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-DIGITAL-LOGIC-FLIP-FLOP-JK-F0-CAT` exists and is limited to variant `var-digital-logic-flip-flop-jk` (Jk), `F0`, and `CAT`.
