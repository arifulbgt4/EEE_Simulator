# CMP-DIGITAL-LOGIC-COUNTER-SYNCHRONOUS-F0-CAT - Specify Synchronous catalog preset
...
| Variant | `var-digital-logic-counter-synchronous` - Synchronous |
| Fidelity | F0 |
| Concern | CAT |
| Release | R5 |
...
Create the complete, immutable catalog definition for **Synchronous** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-digital-logic-counter` (Counter); task scope: variant `var-digital-logic-counter-synchronous` (Synchronous); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Counter family-specific implementation reference](../../catalog/families/fam-digital-logic-counter.md#family-specific-implementation-reference), registry row `fam-digital-logic-counter`, and variant `var-digital-logic-counter-synchronous` (Synchronous).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: A bounded state value advances, decrements, loads, or resets on declared events with modulus, carry/borrow, enable, and timing rules.
...
- `docs/tasks/models/cmp-digital-logic-counter-synchronous-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-digital-logic-counter-synchronous` (Synchronous), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-DIGITAL-LOGIC-COUNTER-SYNCHRONOUS-F0-CAT`: variant `var-digital-logic-counter-synchronous` (Synchronous), fidelity `F0`, concern `CAT`, and requirements REQ-018, REQ-021, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-DIGITAL-LOGIC-COUNTER-SYNCHRONOUS-F0-CAT-NOMINAL`
- `TEST-CMP-DIGITAL-LOGIC-COUNTER-SYNCHRONOUS-F0-CAT-BOUNDARY`
- `TEST-CMP-DIGITAL-LOGIC-COUNTER-SYNCHRONOUS-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-DIGITAL-LOGIC-COUNTER-SYNCHRONOUS-F0-CAT` exists and is limited to variant `var-digital-logic-counter-synchronous` (Synchronous), `F0`, and `CAT`.
