# CMP-SEMICONDUCTORS-OPTO-ISOLATOR-SHARED-F1-VALIDATION - Validate Optically isolated semiconductor F1
...
Produce independent validation evidence for **Optically isolated semiconductor** at **F1** without modifying the model under test.
...
- Stable family: `fam-semiconductors-opto-isolator` (Optically isolated semiconductor); task scope: shared family scope across `var-semiconductors-opto-isolator-phototransistor-output`, `var-semiconductors-opto-isolator-triac-output`; fidelity `F1`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F1 rule: Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: Input diode current produces output photocurrent or logic transfer through declared CTR, delay, saturation, isolation leakage, and temperature behavior.
...
- `docs/tasks/models/cmp-semiconductors-opto-isolator-shared-f1-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SEM-OPTO_ISOLATOR-NOMINAL`, `GOLD-SEM-OPTO_ISOLATOR-BOUNDARY`, `GOLD-SEM-OPTO_ISOLATOR-FAILURE` at `F1`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-SEMICONDUCTORS-OPTO-ISOLATOR-SHARED-F1-VALIDATION`: shared family scope across `var-semiconductors-opto-isolator-phototransistor-output`, `var-semiconductors-opto-isolator-triac-output`, fidelity `F1`, concern `VALIDATION`, and requirements REQ-009, REQ-010, REQ-011, REQ-012, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SEMICONDUCTORS-OPTO-ISOLATOR-SHARED-F1-VALIDATION-NOMINAL`
- `TEST-CMP-SEMICONDUCTORS-OPTO-ISOLATOR-SHARED-F1-VALIDATION-BOUNDARY`
- `TEST-CMP-SEMICONDUCTORS-OPTO-ISOLATOR-SHARED-F1-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SEMICONDUCTORS-OPTO-ISOLATOR-SHARED-F1-VALIDATION` exists and is limited to shared family scope across `var-semiconductors-opto-isolator-phototransistor-output`, `var-semiconductors-opto-isolator-triac-output`, `F1`, and `VALIDATION`.
