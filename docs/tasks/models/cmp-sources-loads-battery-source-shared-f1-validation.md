# CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F1-VALIDATION - Validate Battery source F1
...
Produce independent validation evidence for **Battery source** at **F1** without modifying the model under test.
...
- Stable family: `fam-sources-loads-battery-source` (Battery source); task scope: shared family scope across `var-sources-loads-battery-source-battery-pack`; fidelity `F1`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F1 rule: Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: Terminal voltage is OCV(SOC,T) - I*R_internal(SOC,T), with SOC advanced by charge balance dSOC/dt = -I/Q_capacity under the declared sign convention.
...
- `docs/tasks/models/cmp-sources-loads-battery-source-shared-f1-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SRC-BATTERY_SOURCE-NOMINAL`, `GOLD-SRC-BATTERY_SOURCE-BOUNDARY`, `GOLD-SRC-BATTERY_SOURCE-FAILURE` at `F1`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F1-VALIDATION`: shared family scope across `var-sources-loads-battery-source-battery-pack`, fidelity `F1`, concern `VALIDATION`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F1-VALIDATION-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F1-VALIDATION-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F1-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F1-VALIDATION` exists and is limited to shared family scope across `var-sources-loads-battery-source-battery-pack`, `F1`, and `VALIDATION`.
