# CMP-ANALOG-MIXED-SIGNAL-SCHMITT-TRIGGER-SHARED-F2-VALIDATION - Validate Schmitt trigger F2
...
Produce independent validation evidence for **Schmitt trigger** at **F2** without modifying the model under test.
...
- Stable family: `fam-analog-mixed-signal-schmitt-trigger` (Schmitt trigger); task scope: shared family scope across `var-analog-mixed-signal-schmitt-trigger-inverting`, `var-analog-mixed-signal-schmitt-trigger-non-inverting`; fidelity `F2`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F2 rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: A two-threshold hysteretic state machine maps the input to output logic; threshold ordering and unknown-band behavior are explicit.
...
- `docs/tasks/models/cmp-analog-mixed-signal-schmitt-trigger-shared-f2-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-AMS-SCHMITT_TRIGGER-NOMINAL`, `GOLD-AMS-SCHMITT_TRIGGER-BOUNDARY`, `GOLD-AMS-SCHMITT_TRIGGER-FAILURE` at `F2`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-ANALOG-MIXED-SIGNAL-SCHMITT-TRIGGER-SHARED-F2-VALIDATION`: shared family scope across `var-analog-mixed-signal-schmitt-trigger-inverting`, `var-analog-mixed-signal-schmitt-trigger-non-inverting`, fidelity `F2`, concern `VALIDATION`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-018, REQ-020, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ANALOG-MIXED-SIGNAL-SCHMITT-TRIGGER-SHARED-F2-VALIDATION-NOMINAL`
- `TEST-CMP-ANALOG-MIXED-SIGNAL-SCHMITT-TRIGGER-SHARED-F2-VALIDATION-BOUNDARY`
- `TEST-CMP-ANALOG-MIXED-SIGNAL-SCHMITT-TRIGGER-SHARED-F2-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-ANALOG-MIXED-SIGNAL-SCHMITT-TRIGGER-SHARED-F2-VALIDATION` exists and is limited to shared family scope across `var-analog-mixed-signal-schmitt-trigger-inverting`, `var-analog-mixed-signal-schmitt-trigger-non-inverting`, `F2`, and `VALIDATION`.
