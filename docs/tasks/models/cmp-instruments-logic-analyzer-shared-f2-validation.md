# CMP-INSTRUMENTS-LOGIC-ANALYZER-SHARED-F2-VALIDATION - Validate Logic analyzer F2
...
Produce independent validation evidence for **Logic analyzer** at **F2** without modifying the model under test.
...
- Stable family: `fam-instruments-logic-analyzer` (Logic analyzer); task scope: shared family scope across `var-instruments-logic-analyzer-eight-channel`, `var-instruments-logic-analyzer-sixteen-channel`; fidelity `F2`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F2 rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: Channels resolve sampled thresholds to 0/1/X/Z and capture deterministic timestamped events under declared trigger and memory rules.
...
- `docs/tasks/models/cmp-instruments-logic-analyzer-shared-f2-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-INS-LOGIC_ANALYZER-NOMINAL`, `GOLD-INS-LOGIC_ANALYZER-BOUNDARY`, `GOLD-INS-LOGIC_ANALYZER-FAILURE` at `F2`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-INSTRUMENTS-LOGIC-ANALYZER-SHARED-F2-VALIDATION`: shared family scope across `var-instruments-logic-analyzer-eight-channel`, `var-instruments-logic-analyzer-sixteen-channel`, fidelity `F2`, concern `VALIDATION`, and requirements REQ-007, REQ-021, REQ-022, REQ-023, REQ-034, REQ-037.
...
- `TEST-CMP-INSTRUMENTS-LOGIC-ANALYZER-SHARED-F2-VALIDATION-NOMINAL`
- `TEST-CMP-INSTRUMENTS-LOGIC-ANALYZER-SHARED-F2-VALIDATION-BOUNDARY`
- `TEST-CMP-INSTRUMENTS-LOGIC-ANALYZER-SHARED-F2-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-INSTRUMENTS-LOGIC-ANALYZER-SHARED-F2-VALIDATION` exists and is limited to shared family scope across `var-instruments-logic-analyzer-eight-channel`, `var-instruments-logic-analyzer-sixteen-channel`, `F2`, and `VALIDATION`.
