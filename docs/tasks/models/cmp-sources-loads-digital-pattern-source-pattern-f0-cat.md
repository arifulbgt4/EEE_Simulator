# CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-PATTERN-F0-CAT - Specify Pattern catalog preset
...
| Variant | `var-sources-loads-digital-pattern-source-pattern` - Pattern |
| Fidelity | F0 |
| Concern | CAT |
| Release | R2 |
...
Create the complete, immutable catalog definition for **Pattern** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-sources-loads-digital-pattern-source` (Digital pattern source); task scope: variant `var-sources-loads-digital-pattern-source-pattern` (Pattern); fidelity `F0`; concern `CAT`.
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: The source emits the declared 0/1/X/Z vector at integer ticks; simultaneous changes use the scheduler tie order and never depend on host scheduling.
...
- `docs/tasks/models/cmp-sources-loads-digital-pattern-source-pattern-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-sources-loads-digital-pattern-source-pattern` (Pattern), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-PATTERN-F0-CAT`: variant `var-sources-loads-digital-pattern-source-pattern` (Pattern), fidelity `F0`, concern `CAT`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-PATTERN-F0-CAT-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-PATTERN-F0-CAT-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-PATTERN-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-PATTERN-F0-CAT` exists and is limited to variant `var-sources-loads-digital-pattern-source-pattern` (Pattern), `F0`, and `CAT`.
