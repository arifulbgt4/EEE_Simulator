...
- Stable family: `fam-digital-logic-state-machine` (State machine); task scope: shared family scope across `var-digital-logic-state-machine-moore`, `var-digital-logic-state-machine-mealy`; fidelity `F2`; concern `MODEL`.
...
- Exact F2 execution rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: At each declared event, next_state = F(state,input) and output = G(state,input) using the versioned transition/output tables.
...
- `docs/tasks/models/cmp-digital-logic-state-machine-shared-f2-model.md`
...
- One `ModelBinding` for `fam-digital-logic-state-machine` at `F2` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-DIGITAL-LOGIC-STATE-MACHINE-SHARED-F2-MODEL`: shared family scope across `var-digital-logic-state-machine-moore`, `var-digital-logic-state-machine-mealy`, fidelity `F2`, concern `MODEL`, and requirements REQ-018, REQ-021, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-DIGITAL-LOGIC-STATE-MACHINE-SHARED-F2-MODEL-NOMINAL`
- `TEST-CMP-DIGITAL-LOGIC-STATE-MACHINE-SHARED-F2-MODEL-BOUNDARY`
- `TEST-CMP-DIGITAL-LOGIC-STATE-MACHINE-SHARED-F2-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-DIGITAL-LOGIC-STATE-MACHINE-SHARED-F2-MODEL` exists and is limited to shared family scope across `var-digital-logic-state-machine-moore`, `var-digital-logic-state-machine-mealy`, `F2`, and `MODEL`.
