# CMP-ACTUATORS-HMI-STEPPER-MOTOR-SHARED-F1-MODEL - Implement Stepper motor F1 model
...
Implement one model tier, **F1**, for **Stepper motor** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-actuators-hmi-stepper-motor` (Stepper motor); task scope: shared family scope across `var-actuators-hmi-stepper-motor-unipolar`, `var-actuators-hmi-stepper-motor-bipolar`, `var-actuators-hmi-stepper-motor-hybrid`; fidelity `F1`; concern `MODEL`.
...
- Exact F1 execution rule: Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: Phase-current and rotor angle/speed states follow the declared step/torque/detent/inertia model with missed-step and limit behavior.
...
- `docs/tasks/models/cmp-actuators-hmi-stepper-motor-shared-f1-model.md`
...
- One `ModelBinding` for `fam-actuators-hmi-stepper-motor` at `F1` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-ACTUATORS-HMI-STEPPER-MOTOR-SHARED-F1-MODEL`: shared family scope across `var-actuators-hmi-stepper-motor-unipolar`, `var-actuators-hmi-stepper-motor-bipolar`, `var-actuators-hmi-stepper-motor-hybrid`, fidelity `F1`, concern `MODEL`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ACTUATORS-HMI-STEPPER-MOTOR-SHARED-F1-MODEL-NOMINAL`
- `TEST-CMP-ACTUATORS-HMI-STEPPER-MOTOR-SHARED-F1-MODEL-BOUNDARY`
- `TEST-CMP-ACTUATORS-HMI-STEPPER-MOTOR-SHARED-F1-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-ACTUATORS-HMI-STEPPER-MOTOR-SHARED-F1-MODEL` exists and is limited to shared family scope across `var-actuators-hmi-stepper-motor-unipolar`, `var-actuators-hmi-stepper-motor-bipolar`, `var-actuators-hmi-stepper-motor-hybrid`, `F1`, and `MODEL`.
