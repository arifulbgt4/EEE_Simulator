# CMP-MEMORY-STORAGE-SRAM-SYNCHRONOUS-F0-CAT - Specify Synchronous catalog preset
...
| Variant | `var-memory-storage-sram-synchronous` - Synchronous |
| Fidelity | F0 |
| Concern | CAT |
| Release | R8 |
...
- Stable family: `fam-memory-storage-sram` (Static RAM); task scope: variant `var-memory-storage-sram-synchronous` (Synchronous); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Static RAM family-specific implementation reference](../../catalog/families/fam-memory-storage-sram.md#family-specific-implementation-reference), registry row `fam-memory-storage-sram`, and variant `var-memory-storage-sram-synchronous` (Synchronous).
...
- `docs/tasks/models/cmp-memory-storage-sram-synchronous-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-memory-storage-sram-synchronous` (Synchronous), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-MEMORY-STORAGE-SRAM-SYNCHRONOUS-F0-CAT`: variant `var-memory-storage-sram-synchronous` (Synchronous), fidelity `F0`, concern `CAT`, and requirements REQ-018, REQ-021, REQ-025, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MEMORY-STORAGE-SRAM-SYNCHRONOUS-F0-CAT-NOMINAL`
- `TEST-CMP-MEMORY-STORAGE-SRAM-SYNCHRONOUS-F0-CAT-BOUNDARY`
- `TEST-CMP-MEMORY-STORAGE-SRAM-SYNCHRONOUS-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-MEMORY-STORAGE-SRAM-SYNCHRONOUS-F0-CAT` exists and is limited to variant `var-memory-storage-sram-synchronous` (Synchronous), `F0`, and `CAT`.
