# CMP-POWER-ELECTRONICS-CHARGER-BMS-BMS-F0-CAT - Specify Bms catalog preset
...
| Variant | `var-power-electronics-charger-bms-bms` - Bms |
| Fidelity | F0 |
| Concern | CAT |
| Release | R7 |
...
Create the complete, immutable catalog definition for **Bms** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-power-electronics-charger-bms` (Charger and battery management); task scope: variant `var-power-electronics-charger-bms-bms` (Bms); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Charger and battery management family-specific implementation reference](../../catalog/families/fam-power-electronics-charger-bms.md#family-specific-implementation-reference), registry row `fam-power-electronics-charger-bms`, and variant `var-power-electronics-charger-bms-bms` (Bms).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Charging and protection follow the declared CC/CV or chemistry profile plus cell-voltage/current/temperature state machine and balancing rules.
...
- `docs/tasks/models/cmp-power-electronics-charger-bms-bms-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-power-electronics-charger-bms-bms` (Bms), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-POWER-ELECTRONICS-CHARGER-BMS-BMS-F0-CAT`: variant `var-power-electronics-charger-bms-bms` (Bms), fidelity `F0`, concern `CAT`, and requirements REQ-009, REQ-010, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-POWER-ELECTRONICS-CHARGER-BMS-BMS-F0-CAT-NOMINAL`
- `TEST-CMP-POWER-ELECTRONICS-CHARGER-BMS-BMS-F0-CAT-BOUNDARY`
- `TEST-CMP-POWER-ELECTRONICS-CHARGER-BMS-BMS-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-POWER-ELECTRONICS-CHARGER-BMS-BMS-F0-CAT` exists and is limited to variant `var-power-electronics-charger-bms-bms` (Bms), `F0`, and `CAT`.
