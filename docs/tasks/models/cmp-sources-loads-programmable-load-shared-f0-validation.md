# CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F0-VALIDATION - Validate Programmable load F0
...
Produce independent validation evidence for **Programmable load** at **F0** without modifying the model under test.
...
- Stable family: `fam-sources-loads-programmable-load` (Programmable load); task scope: shared family scope across `var-sources-loads-programmable-load-profiled-load`; fidelity `F0`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F0 rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The selected R/I/P setpoint is a deterministic function of time or control input with declared slew, compliance, and low-voltage behavior.
...
- `docs/tasks/models/cmp-sources-loads-programmable-load-shared-f0-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SRC-PROGRAMMABLE_LOAD-NOMINAL`, `GOLD-SRC-PROGRAMMABLE_LOAD-BOUNDARY`, `GOLD-SRC-PROGRAMMABLE_LOAD-FAILURE` at `F0`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F0-VALIDATION`: shared family scope across `var-sources-loads-programmable-load-profiled-load`, fidelity `F0`, concern `VALIDATION`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F0-VALIDATION-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F0-VALIDATION-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F0-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F0-VALIDATION` exists and is limited to shared family scope across `var-sources-loads-programmable-load-profiled-load`, `F0`, and `VALIDATION`.
