# CMP-ACTUATORS-HMI-SERVO-CONTINUOUS-ROTATION-F0-CAT - Specify Continuous Rotation catalog preset
...
| Variant | `var-actuators-hmi-servo-continuous-rotation` - Continuous Rotation |
| Fidelity | F0 |
| Concern | CAT |
| Release | R7 |
...
Create the complete, immutable catalog definition for **Continuous Rotation** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-actuators-hmi-servo` (Servo); task scope: variant `var-actuators-hmi-servo-continuous-rotation` (Continuous Rotation); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Servo family-specific implementation reference](../../catalog/families/fam-actuators-hmi-servo.md#family-specific-implementation-reference), registry row `fam-actuators-hmi-servo`, and variant `var-actuators-hmi-servo-continuous-rotation` (Continuous Rotation).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Command pulse/position drives a bounded closed-loop angle state with declared range, speed, dead band, torque/load, current, and stall behavior.
...
- `docs/tasks/models/cmp-actuators-hmi-servo-continuous-rotation-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-actuators-hmi-servo-continuous-rotation` (Continuous Rotation), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-ACTUATORS-HMI-SERVO-CONTINUOUS-ROTATION-F0-CAT`: variant `var-actuators-hmi-servo-continuous-rotation` (Continuous Rotation), fidelity `F0`, concern `CAT`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ACTUATORS-HMI-SERVO-CONTINUOUS-ROTATION-F0-CAT-NOMINAL`
- `TEST-CMP-ACTUATORS-HMI-SERVO-CONTINUOUS-ROTATION-F0-CAT-BOUNDARY`
- `TEST-CMP-ACTUATORS-HMI-SERVO-CONTINUOUS-ROTATION-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-ACTUATORS-HMI-SERVO-CONTINUOUS-ROTATION-F0-CAT` exists and is limited to variant `var-actuators-hmi-servo-continuous-rotation` (Continuous Rotation), `F0`, and `CAT`.
