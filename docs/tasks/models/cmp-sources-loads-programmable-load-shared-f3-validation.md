# CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F3-VALIDATION - Validate Programmable load F3
...
Produce independent validation evidence for **Programmable load** at **F3** without modifying the model under test.
...
- Stable family: `fam-sources-loads-programmable-load` (Programmable load); task scope: shared family scope across `var-sources-loads-programmable-load-profiled-load`; fidelity `F3`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F3 rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: The selected R/I/P setpoint is a deterministic function of time or control input with declared slew, compliance, and low-voltage behavior.
...
- `docs/tasks/models/cmp-sources-loads-programmable-load-shared-f3-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SRC-PROGRAMMABLE_LOAD-NOMINAL`, `GOLD-SRC-PROGRAMMABLE_LOAD-BOUNDARY`, `GOLD-SRC-PROGRAMMABLE_LOAD-FAILURE` at `F3`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F3-VALIDATION`: shared family scope across `var-sources-loads-programmable-load-profiled-load`, fidelity `F3`, concern `VALIDATION`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F3-VALIDATION-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F3-VALIDATION-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F3-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F3-VALIDATION` exists and is limited to shared family scope across `var-sources-loads-programmable-load-profiled-load`, `F3`, and `VALIDATION`.
