# CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F3-MODEL - Implement Programmable load F3 model
...
Implement one model tier, **F3**, for **Programmable load** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-sources-loads-programmable-load` (Programmable load); task scope: shared family scope across `var-sources-loads-programmable-load-profiled-load`; fidelity `F3`; concern `MODEL`.
...
- Exact F3 execution rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: The selected R/I/P setpoint is a deterministic function of time or control input with declared slew, compliance, and low-voltage behavior.
...
- `docs/tasks/models/cmp-sources-loads-programmable-load-shared-f3-model.md`
...
- One `ModelBinding` for `fam-sources-loads-programmable-load` at `F3` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F3-MODEL`: shared family scope across `var-sources-loads-programmable-load-profiled-load`, fidelity `F3`, concern `MODEL`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F3-MODEL-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F3-MODEL-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F3-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F3-MODEL` exists and is limited to shared family scope across `var-sources-loads-programmable-load-profiled-load`, `F3`, and `MODEL`.
