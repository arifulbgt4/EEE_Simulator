# CMP-COMPUTER-GPU-EDUCATIONAL-CPU-SMALL-RISC-V-F0-CAT - Specify Small Risc V catalog preset
...
| Variant | `var-computer-gpu-educational-cpu-small-risc-v` - Small Risc V |
| Fidelity | F0 |
| Concern | CAT |
| Release | R8 |
...
Create the complete, immutable catalog definition for **Small Risc V** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-computer-gpu-educational-cpu` (Educational CPU); task scope: variant `var-computer-gpu-educational-cpu-small-risc-v` (Small Risc V); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Educational CPU family-specific implementation reference](../../catalog/families/fam-computer-gpu-educational-cpu.md#family-specific-implementation-reference), registry row `fam-computer-gpu-educational-cpu`, and variant `var-computer-gpu-educational-cpu-small-risc-v` (Small Risc V).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: The CPU advances the declared ISA/microarchitecture state per instruction or cycle, including registers, flags, PC, memory/bus transactions, interrupts, reset, and traps.
...
- `docs/tasks/models/cmp-computer-gpu-educational-cpu-small-risc-v-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-computer-gpu-educational-cpu-small-risc-v` (Small Risc V), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-COMPUTER-GPU-EDUCATIONAL-CPU-SMALL-RISC-V-F0-CAT`: variant `var-computer-gpu-educational-cpu-small-risc-v` (Small Risc V), fidelity `F0`, concern `CAT`, and requirements REQ-025, REQ-027, REQ-028, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-COMPUTER-GPU-EDUCATIONAL-CPU-SMALL-RISC-V-F0-CAT-NOMINAL`
- `TEST-CMP-COMPUTER-GPU-EDUCATIONAL-CPU-SMALL-RISC-V-F0-CAT-BOUNDARY`
- `TEST-CMP-COMPUTER-GPU-EDUCATIONAL-CPU-SMALL-RISC-V-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-COMPUTER-GPU-EDUCATIONAL-CPU-SMALL-RISC-V-F0-CAT` exists and is limited to variant `var-computer-gpu-educational-cpu-small-risc-v` (Small Risc V), `F0`, and `CAT`.
