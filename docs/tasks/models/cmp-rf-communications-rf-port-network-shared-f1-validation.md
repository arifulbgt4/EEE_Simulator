# CMP-RF-COMMUNICATIONS-RF-PORT-NETWORK-SHARED-F1-VALIDATION - Validate RF port and network model F1
...
Produce independent validation evidence for **RF port and network model** at **F1** without modifying the model under test.
...
- Candidate and independent reference must evaluate the same F1 rule: Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: At each declared frequency, incident/reflected waves satisfy b = S(f)*a at the stated reference impedance; passivity, causality, and port order are validated.
...
- `docs/tasks/models/cmp-rf-communications-rf-port-network-shared-f1-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-RFC-RF_PORT_NETWORK-NOMINAL`, `GOLD-RFC-RF_PORT_NETWORK-BOUNDARY`, `GOLD-RFC-RF_PORT_NETWORK-FAILURE` at `F1`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-RF-COMMUNICATIONS-RF-PORT-NETWORK-SHARED-F1-VALIDATION`: shared family scope across `var-rf-communications-rf-port-network-rf-port`, `var-rf-communications-rf-port-network-termination`, `var-rf-communications-rf-port-network-s-parameter-black-box`, fidelity `F1`, concern `VALIDATION`, and requirements REQ-011, REQ-012, REQ-024, REQ-028, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-RF-COMMUNICATIONS-RF-PORT-NETWORK-SHARED-F1-VALIDATION-NOMINAL`
- `TEST-CMP-RF-COMMUNICATIONS-RF-PORT-NETWORK-SHARED-F1-VALIDATION-BOUNDARY`
- `TEST-CMP-RF-COMMUNICATIONS-RF-PORT-NETWORK-SHARED-F1-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-RF-COMMUNICATIONS-RF-PORT-NETWORK-SHARED-F1-VALIDATION` exists and is limited to shared family scope across `var-rf-communications-rf-port-network-rf-port`, `var-rf-communications-rf-port-network-termination`, `var-rf-communications-rf-port-network-s-parameter-black-box`, `F1`, and `VALIDATION`.
