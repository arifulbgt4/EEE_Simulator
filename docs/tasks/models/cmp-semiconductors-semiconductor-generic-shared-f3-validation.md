# CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-SHARED-F3-VALIDATION - Validate Generic semiconductor black box F3
...
Produce independent validation evidence for **Generic semiconductor black box** at **F3** without modifying the model under test.
...
- Stable family: `fam-semiconductors-semiconductor-generic` (Generic semiconductor black box); task scope: shared family scope across `var-semiconductors-semiconductor-generic-equation-model`; fidelity `F3`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F3 rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Behavior comes only from the selected validated black-box/compact model; its ordered pins, analyses, envelope, provenance, and unsupported behavior are mandatory inputs.
...
- `docs/tasks/models/cmp-semiconductors-semiconductor-generic-shared-f3-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SEM-SEMICONDUCTOR_GENERIC-NOMINAL`, `GOLD-SEM-SEMICONDUCTOR_GENERIC-BOUNDARY`, `GOLD-SEM-SEMICONDUCTOR_GENERIC-FAILURE` at `F3`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-SHARED-F3-VALIDATION`: shared family scope across `var-semiconductors-semiconductor-generic-equation-model`, fidelity `F3`, concern `VALIDATION`, and requirements REQ-009, REQ-010, REQ-011, REQ-012, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-SHARED-F3-VALIDATION-NOMINAL`
- `TEST-CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-SHARED-F3-VALIDATION-BOUNDARY`
- `TEST-CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-SHARED-F3-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-SHARED-F3-VALIDATION` exists and is limited to shared family scope across `var-semiconductors-semiconductor-generic-equation-model`, `F3`, and `VALIDATION`.
