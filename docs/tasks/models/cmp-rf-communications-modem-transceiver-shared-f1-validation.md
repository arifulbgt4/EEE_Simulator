# CMP-RF-COMMUNICATIONS-MODEM-TRANSCEIVER-SHARED-F1-VALIDATION - Validate Modem and transceiver F1
...
Produce independent validation evidence for **Modem and transceiver** at **F1** without modifying the model under test.
...
- Stable family: `fam-rf-communications-modem-transceiver` (Modem and transceiver); task scope: shared family scope across `var-rf-communications-modem-transceiver-analog-modem`, `var-rf-communications-modem-transceiver-ook-fsk`, `var-rf-communications-modem-transceiver-qam-ofdm`; fidelity `F1`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F1 rule: Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: Bits/symbols map through the selected modulation, coding, framing, timing, RF/baseband, synchronization, and error state machine.
...
- `docs/tasks/models/cmp-rf-communications-modem-transceiver-shared-f1-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-RFC-MODEM_TRANSCEIVER-NOMINAL`, `GOLD-RFC-MODEM_TRANSCEIVER-BOUNDARY`, `GOLD-RFC-MODEM_TRANSCEIVER-FAILURE` at `F1`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-RF-COMMUNICATIONS-MODEM-TRANSCEIVER-SHARED-F1-VALIDATION`: shared family scope across `var-rf-communications-modem-transceiver-analog-modem`, `var-rf-communications-modem-transceiver-ook-fsk`, `var-rf-communications-modem-transceiver-qam-ofdm`, fidelity `F1`, concern `VALIDATION`, and requirements REQ-011, REQ-012, REQ-024, REQ-028, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-RF-COMMUNICATIONS-MODEM-TRANSCEIVER-SHARED-F1-VALIDATION-NOMINAL`
- `TEST-CMP-RF-COMMUNICATIONS-MODEM-TRANSCEIVER-SHARED-F1-VALIDATION-BOUNDARY`
- `TEST-CMP-RF-COMMUNICATIONS-MODEM-TRANSCEIVER-SHARED-F1-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-RF-COMMUNICATIONS-MODEM-TRANSCEIVER-SHARED-F1-VALIDATION` exists and is limited to shared family scope across `var-rf-communications-modem-transceiver-analog-modem`, `var-rf-communications-modem-transceiver-ook-fsk`, `var-rf-communications-modem-transceiver-qam-ofdm`, `F1`, and `VALIDATION`.
