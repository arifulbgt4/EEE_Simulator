# CMP-ACTUATORS-HMI-STEPPER-MOTOR-SHARED-F3-VALIDATION - Validate Stepper motor F3
...
Produce independent validation evidence for **Stepper motor** at **F3** without modifying the model under test.
...
- Stable family: `fam-actuators-hmi-stepper-motor` (Stepper motor); task scope: shared family scope across `var-actuators-hmi-stepper-motor-unipolar`, `var-actuators-hmi-stepper-motor-bipolar`, `var-actuators-hmi-stepper-motor-hybrid`; fidelity `F3`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F3 rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Phase-current and rotor angle/speed states follow the declared step/torque/detent/inertia model with missed-step and limit behavior.
...
- `docs/tasks/models/cmp-actuators-hmi-stepper-motor-shared-f3-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-ACT-STEPPER_MOTOR-NOMINAL`, `GOLD-ACT-STEPPER_MOTOR-BOUNDARY`, `GOLD-ACT-STEPPER_MOTOR-FAILURE` at `F3`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-ACTUATORS-HMI-STEPPER-MOTOR-SHARED-F3-VALIDATION`: shared family scope across `var-actuators-hmi-stepper-motor-unipolar`, `var-actuators-hmi-stepper-motor-bipolar`, `var-actuators-hmi-stepper-motor-hybrid`, fidelity `F3`, concern `VALIDATION`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ACTUATORS-HMI-STEPPER-MOTOR-SHARED-F3-VALIDATION-NOMINAL`
- `TEST-CMP-ACTUATORS-HMI-STEPPER-MOTOR-SHARED-F3-VALIDATION-BOUNDARY`
- `TEST-CMP-ACTUATORS-HMI-STEPPER-MOTOR-SHARED-F3-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-ACTUATORS-HMI-STEPPER-MOTOR-SHARED-F3-VALIDATION` exists and is limited to shared family scope across `var-actuators-hmi-stepper-motor-unipolar`, `var-actuators-hmi-stepper-motor-bipolar`, `var-actuators-hmi-stepper-motor-hybrid`, `F3`, and `VALIDATION`.
