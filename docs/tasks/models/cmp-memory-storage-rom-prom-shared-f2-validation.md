# CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F2-VALIDATION - Validate ROM and PROM F2
...
Produce independent validation evidence for **ROM and PROM** at **F2** without modifying the model under test.
...
- Stable family: `fam-memory-storage-rom-prom` (ROM and PROM); task scope: shared family scope across `var-memory-storage-rom-prom-mask-rom`, `var-memory-storage-rom-prom-prom`, `var-memory-storage-rom-prom-eprom`; fidelity `F2`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F2 rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: Address selects an immutable/programmed word; range, enable, output delay, initialization, and unknown-address behavior are explicit.
...
- `docs/tasks/models/cmp-memory-storage-rom-prom-shared-f2-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-MEM-ROM_PROM-NOMINAL`, `GOLD-MEM-ROM_PROM-BOUNDARY`, `GOLD-MEM-ROM_PROM-FAILURE` at `F2`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F2-VALIDATION`: shared family scope across `var-memory-storage-rom-prom-mask-rom`, `var-memory-storage-rom-prom-prom`, `var-memory-storage-rom-prom-eprom`, fidelity `F2`, concern `VALIDATION`, and requirements REQ-018, REQ-021, REQ-025, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F2-VALIDATION-NOMINAL`
- `TEST-CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F2-VALIDATION-BOUNDARY`
- `TEST-CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F2-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F2-VALIDATION` exists and is limited to shared family scope across `var-memory-storage-rom-prom-mask-rom`, `var-memory-storage-rom-prom-prom`, `var-memory-storage-rom-prom-eprom`, `F2`, and `VALIDATION`.
