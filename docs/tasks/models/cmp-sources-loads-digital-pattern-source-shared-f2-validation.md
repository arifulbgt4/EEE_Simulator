# CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F2-VALIDATION - Validate Digital pattern source F2
...
Produce independent validation evidence for **Digital pattern source** at **F2** without modifying the model under test.
...
- Stable family: `fam-sources-loads-digital-pattern-source` (Digital pattern source); task scope: shared family scope across `var-sources-loads-digital-pattern-source-pattern`; fidelity `F2`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F2 rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: The source emits the declared 0/1/X/Z vector at integer ticks; simultaneous changes use the scheduler tie order and never depend on host scheduling.
...
- `docs/tasks/models/cmp-sources-loads-digital-pattern-source-shared-f2-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SRC-DIGITAL_PATTERN_SOURCE-NOMINAL`, `GOLD-SRC-DIGITAL_PATTERN_SOURCE-BOUNDARY`, `GOLD-SRC-DIGITAL_PATTERN_SOURCE-FAILURE` at `F2`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F2-VALIDATION`: shared family scope across `var-sources-loads-digital-pattern-source-pattern`, fidelity `F2`, concern `VALIDATION`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F2-VALIDATION-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F2-VALIDATION-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F2-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F2-VALIDATION` exists and is limited to shared family scope across `var-sources-loads-digital-pattern-source-pattern`, `F2`, and `VALIDATION`.
