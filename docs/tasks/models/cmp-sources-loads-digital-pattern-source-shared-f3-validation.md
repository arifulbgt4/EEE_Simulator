# CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F3-VALIDATION - Validate Digital pattern source F3
...
Produce independent validation evidence for **Digital pattern source** at **F3** without modifying the model under test.
...
- Stable family: `fam-sources-loads-digital-pattern-source` (Digital pattern source); task scope: shared family scope across `var-sources-loads-digital-pattern-source-pattern`; fidelity `F3`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F3 rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: The source emits the declared 0/1/X/Z vector at integer ticks; simultaneous changes use the scheduler tie order and never depend on host scheduling.
...
- `docs/tasks/models/cmp-sources-loads-digital-pattern-source-shared-f3-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SRC-DIGITAL_PATTERN_SOURCE-NOMINAL`, `GOLD-SRC-DIGITAL_PATTERN_SOURCE-BOUNDARY`, `GOLD-SRC-DIGITAL_PATTERN_SOURCE-FAILURE` at `F3`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F3-VALIDATION`: shared family scope across `var-sources-loads-digital-pattern-source-pattern`, fidelity `F3`, concern `VALIDATION`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F3-VALIDATION-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F3-VALIDATION-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F3-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F3-VALIDATION` exists and is limited to shared family scope across `var-sources-loads-digital-pattern-source-pattern`, `F3`, and `VALIDATION`.
