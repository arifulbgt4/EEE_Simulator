# CMP-PASSIVES-FERRITE-BEAD-F0-CAT - Specify Bead catalog preset
...
| Variant | `var-passives-ferrite-bead` - Bead |
| Fidelity | F0 |
| Concern | CAT |
| Release | R2 |
...
Create the complete, immutable catalog definition for **Bead** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-passives-ferrite` (Ferrite suppressor); task scope: variant `var-passives-ferrite-bead` (Bead); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Ferrite suppressor family-specific implementation reference](../../catalog/families/fam-passives-ferrite.md#family-specific-implementation-reference), registry row `fam-passives-ferrite`, and variant `var-passives-ferrite-bead` (Bead).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: The branch uses the declared frequency-dependent complex impedance Z(f,T,bias); time-domain use requires an approved causal fitted model.
...
- `docs/tasks/models/cmp-passives-ferrite-bead-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-passives-ferrite-bead` (Bead), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-PASSIVES-FERRITE-BEAD-F0-CAT`: variant `var-passives-ferrite-bead` (Bead), fidelity `F0`, concern `CAT`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-FERRITE-BEAD-F0-CAT-NOMINAL`
- `TEST-CMP-PASSIVES-FERRITE-BEAD-F0-CAT-BOUNDARY`
- `TEST-CMP-PASSIVES-FERRITE-BEAD-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-FERRITE-BEAD-F0-CAT` exists and is limited to variant `var-passives-ferrite-bead` (Bead), `F0`, and `CAT`.
