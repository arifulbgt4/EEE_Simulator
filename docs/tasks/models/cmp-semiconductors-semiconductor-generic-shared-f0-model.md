# CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-SHARED-F0-MODEL - Implement Generic semiconductor black box F0 model
...
Implement one model tier, **F0**, for **Generic semiconductor black box** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-semiconductors-semiconductor-generic` (Generic semiconductor black box); task scope: shared family scope across `var-semiconductors-semiconductor-generic-equation-model`; fidelity `F0`; concern `MODEL`.
...
- Exact F0 execution rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Behavior comes only from the selected validated black-box/compact model; its ordered pins, analyses, envelope, provenance, and unsupported behavior are mandatory inputs.
...
- `docs/tasks/models/cmp-semiconductors-semiconductor-generic-shared-f0-model.md`
...
- One `ModelBinding` for `fam-semiconductors-semiconductor-generic` at `F0` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-SHARED-F0-MODEL`: shared family scope across `var-semiconductors-semiconductor-generic-equation-model`, fidelity `F0`, concern `MODEL`, and requirements REQ-009, REQ-010, REQ-011, REQ-012, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-SHARED-F0-MODEL-NOMINAL`
- `TEST-CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-SHARED-F0-MODEL-BOUNDARY`
- `TEST-CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-SHARED-F0-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-SHARED-F0-MODEL` exists and is limited to shared family scope across `var-semiconductors-semiconductor-generic-equation-model`, `F0`, and `MODEL`.
