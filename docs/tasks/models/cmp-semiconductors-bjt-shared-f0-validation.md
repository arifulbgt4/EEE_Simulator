# CMP-SEMICONDUCTORS-BJT-SHARED-F0-VALIDATION - Validate Bipolar junction transistor F0
...
Produce independent validation evidence for **Bipolar junction transistor** at **F0** without modifying the model under test.
...
- Stable family: `fam-semiconductors-bjt` (Bipolar junction transistor); task scope: shared family scope across `var-semiconductors-bjt-npn`, `var-semiconductors-bjt-pnp`, `var-semiconductors-bjt-darlington-npn`, `var-semiconductors-bjt-darlington-pnp`; fidelity `F0`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F0 rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: F1 uses a documented Ebers-Moll-style junction/current-gain relation with NPN/PNP polarity; F3 binds a pinned compact model and parameter envelope.
...
- `docs/tasks/models/cmp-semiconductors-bjt-shared-f0-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SEM-BJT-NOMINAL`, `GOLD-SEM-BJT-BOUNDARY`, `GOLD-SEM-BJT-FAILURE` at `F0`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-SEMICONDUCTORS-BJT-SHARED-F0-VALIDATION`: shared family scope across `var-semiconductors-bjt-npn`, `var-semiconductors-bjt-pnp`, `var-semiconductors-bjt-darlington-npn`, `var-semiconductors-bjt-darlington-pnp`, fidelity `F0`, concern `VALIDATION`, and requirements REQ-009, REQ-010, REQ-011, REQ-012, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SEMICONDUCTORS-BJT-SHARED-F0-VALIDATION-NOMINAL`
- `TEST-CMP-SEMICONDUCTORS-BJT-SHARED-F0-VALIDATION-BOUNDARY`
- `TEST-CMP-SEMICONDUCTORS-BJT-SHARED-F0-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SEMICONDUCTORS-BJT-SHARED-F0-VALIDATION` exists and is limited to shared family scope across `var-semiconductors-bjt-npn`, `var-semiconductors-bjt-pnp`, `var-semiconductors-bjt-darlington-npn`, `var-semiconductors-bjt-darlington-pnp`, `F0`, and `VALIDATION`.
