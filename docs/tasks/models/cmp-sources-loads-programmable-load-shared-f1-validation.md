# CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F1-VALIDATION - Validate Programmable load F1
...
Produce independent validation evidence for **Programmable load** at **F1** without modifying the model under test.
...
- Stable family: `fam-sources-loads-programmable-load` (Programmable load); task scope: shared family scope across `var-sources-loads-programmable-load-profiled-load`; fidelity `F1`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F1 rule: Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: The selected R/I/P setpoint is a deterministic function of time or control input with declared slew, compliance, and low-voltage behavior.
...
- `docs/tasks/models/cmp-sources-loads-programmable-load-shared-f1-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SRC-PROGRAMMABLE_LOAD-NOMINAL`, `GOLD-SRC-PROGRAMMABLE_LOAD-BOUNDARY`, `GOLD-SRC-PROGRAMMABLE_LOAD-FAILURE` at `F1`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F1-VALIDATION`: shared family scope across `var-sources-loads-programmable-load-profiled-load`, fidelity `F1`, concern `VALIDATION`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F1-VALIDATION-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F1-VALIDATION-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F1-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F1-VALIDATION` exists and is limited to shared family scope across `var-sources-loads-programmable-load-profiled-load`, `F1`, and `VALIDATION`.
