# CMP-CONNECTIVITY-TERMINAL-BRIDGE-JUMPER-F0-CAT - Specify Jumper catalog preset
...
| Variant | `var-connectivity-terminal-bridge-jumper` - Jumper |
| Fidelity | F0 |
| Concern | CAT |
| Release | R1 |
...
Create the complete, immutable catalog definition for **Jumper** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-connectivity-terminal-bridge` (Terminal, net tie, jumper, and domain adapter); task scope: variant `var-connectivity-terminal-bridge-jumper` (Jumper); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Terminal, net tie, jumper, and domain adapter family-specific implementation reference](../../catalog/families/fam-connectivity-terminal-bridge.md#family-specific-implementation-reference), registry row `fam-connectivity-terminal-bridge`, and variant `var-connectivity-terminal-bridge-jumper` (Jumper).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Terminal, net-tie, jumper, and domain-adapter variants use an explicit connection matrix; only matrix entries declared closed may share current or potential.
...
- `docs/tasks/models/cmp-connectivity-terminal-bridge-jumper-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-connectivity-terminal-bridge-jumper` (Jumper), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-CONNECTIVITY-TERMINAL-BRIDGE-JUMPER-F0-CAT`: variant `var-connectivity-terminal-bridge-jumper` (Jumper), fidelity `F0`, concern `CAT`, and requirements REQ-004, REQ-005, REQ-008, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-CONNECTIVITY-TERMINAL-BRIDGE-JUMPER-F0-CAT-NOMINAL`
- `TEST-CMP-CONNECTIVITY-TERMINAL-BRIDGE-JUMPER-F0-CAT-BOUNDARY`
- `TEST-CMP-CONNECTIVITY-TERMINAL-BRIDGE-JUMPER-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-CONNECTIVITY-TERMINAL-BRIDGE-JUMPER-F0-CAT` exists and is limited to variant `var-connectivity-terminal-bridge-jumper` (Jumper), `F0`, and `CAT`.
