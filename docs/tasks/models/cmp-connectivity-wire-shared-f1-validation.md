# CMP-CONNECTIVITY-WIRE-SHARED-F1-VALIDATION - Validate Wire and conductor F1
...
Produce independent validation evidence for **Wire and conductor** at **F1** without modifying the model under test.
...
- Stable family: `fam-connectivity-wire` (Wire and conductor); task scope: shared family scope across `var-connectivity-wire-conductor`; fidelity `F1`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F1 rule: Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: Ideal connectivity enforces v(P) - v(N) = 0; any F1 non-ideal profile replaces the ideal branch with an explicitly parameterized series impedance.
...
- `docs/tasks/models/cmp-connectivity-wire-shared-f1-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-CON-WIRE-NOMINAL`, `GOLD-CON-WIRE-BOUNDARY`, `GOLD-CON-WIRE-FAILURE` at `F1`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-CONNECTIVITY-WIRE-SHARED-F1-VALIDATION`: shared family scope across `var-connectivity-wire-conductor`, fidelity `F1`, concern `VALIDATION`, and requirements REQ-004, REQ-005, REQ-008, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-CONNECTIVITY-WIRE-SHARED-F1-VALIDATION-NOMINAL`
- `TEST-CMP-CONNECTIVITY-WIRE-SHARED-F1-VALIDATION-BOUNDARY`
- `TEST-CMP-CONNECTIVITY-WIRE-SHARED-F1-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-CONNECTIVITY-WIRE-SHARED-F1-VALIDATION` exists and is limited to shared family scope across `var-connectivity-wire-conductor`, `F1`, and `VALIDATION`.
