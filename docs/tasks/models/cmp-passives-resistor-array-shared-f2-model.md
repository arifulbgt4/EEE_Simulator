# CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F2-MODEL - Implement Resistor array F2 model
...
Implement one model tier, **F2**, for **Resistor array** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-passives-resistor-array` (Resistor array); task scope: shared family scope across `var-passives-resistor-array-isolated`, `var-passives-resistor-array-bussed`; fidelity `F2`; concern `MODEL`.
...
- Exact F2 execution rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: Each declared resistor branch obeys v_k = R_k*i_k; shared-terminal and matching/correlation rules come only from the selected array topology.
...
- `docs/tasks/models/cmp-passives-resistor-array-shared-f2-model.md`
...
- One `ModelBinding` for `fam-passives-resistor-array` at `F2` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F2-MODEL`: shared family scope across `var-passives-resistor-array-isolated`, `var-passives-resistor-array-bussed`, fidelity `F2`, concern `MODEL`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F2-MODEL-NOMINAL`
- `TEST-CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F2-MODEL-BOUNDARY`
- `TEST-CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F2-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F2-MODEL` exists and is limited to shared family scope across `var-passives-resistor-array-isolated`, `var-passives-resistor-array-bussed`, `F2`, and `MODEL`.
