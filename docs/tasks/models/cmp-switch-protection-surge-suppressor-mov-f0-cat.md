# CMP-SWITCH-PROTECTION-SURGE-SUPPRESSOR-MOV-F0-CAT - Specify Mov catalog preset
...
| Variant | `var-switch-protection-surge-suppressor-mov` - Mov |
| Fidelity | F0 |
| Concern | CAT |
| Release | R7 |
...
Create the complete, immutable catalog definition for **Mov** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-switch-protection-surge-suppressor` (Surge and ESD suppressor); task scope: variant `var-switch-protection-surge-suppressor-mov` (Mov); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Surge and ESD suppressor family-specific implementation reference](../../catalog/families/fam-switch-protection-surge-suppressor.md#family-specific-implementation-reference), registry row `fam-switch-protection-surge-suppressor`, and variant `var-switch-protection-surge-suppressor-mov` (Mov).
...
- `docs/tasks/models/cmp-switch-protection-surge-suppressor-mov-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-switch-protection-surge-suppressor-mov` (Mov), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-SWITCH-PROTECTION-SURGE-SUPPRESSOR-MOV-F0-CAT`: variant `var-switch-protection-surge-suppressor-mov` (Mov), fidelity `F0`, concern `CAT`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SWITCH-PROTECTION-SURGE-SUPPRESSOR-MOV-F0-CAT-NOMINAL`
- `TEST-CMP-SWITCH-PROTECTION-SURGE-SUPPRESSOR-MOV-F0-CAT-BOUNDARY`
- `TEST-CMP-SWITCH-PROTECTION-SURGE-SUPPRESSOR-MOV-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-SWITCH-PROTECTION-SURGE-SUPPRESSOR-MOV-F0-CAT` exists and is limited to variant `var-switch-protection-surge-suppressor-mov` (Mov), `F0`, and `CAT`.
