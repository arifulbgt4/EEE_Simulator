# CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F1-MODEL - Implement Independent current source F1 model
...
Implement one model tier, **F1**, for **Independent current source** using the family equations/behavior and no higher-fidelity claims.
...
- Exact F1 execution rule: Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: The oriented branch current is i(P->N) = I(t), with I(t) supplied by the selected DC/AC/transient profile.
...
- `docs/tasks/models/cmp-sources-loads-independent-current-shared-f1-model.md`
...
- One `ModelBinding` for `fam-sources-loads-independent-current` at `F1` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F1-MODEL`: shared family scope across `var-sources-loads-independent-current-dc`, `var-sources-loads-independent-current-ac-small-signal`, `var-sources-loads-independent-current-sine`, `var-sources-loads-independent-current-pulse`, fidelity `F1`, concern `MODEL`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F1-MODEL-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F1-MODEL-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F1-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F1-MODEL` exists and is limited to shared family scope across `var-sources-loads-independent-current-dc`, `var-sources-loads-independent-current-ac-small-signal`, `var-sources-loads-independent-current-sine`, `var-sources-loads-independent-current-pulse`, `F1`, and `MODEL`.
