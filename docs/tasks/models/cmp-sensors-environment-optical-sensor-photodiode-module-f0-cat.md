# CMP-SENSORS-ENVIRONMENT-OPTICAL-SENSOR-PHOTODIODE-MODULE-F0-CAT - Specify Photodiode Module catalog preset
...
| Variant | `var-sensors-environment-optical-sensor-photodiode-module` - Photodiode Module |
| Fidelity | F0 |
| Concern | CAT |
| Release | R7 |
...
Create the complete, immutable catalog definition for **Photodiode Module** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-sensors-environment-optical-sensor` (Optical sensor); task scope: variant `var-sensors-environment-optical-sensor-photodiode-module` (Photodiode Module); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Optical sensor family-specific implementation reference](../../catalog/families/fam-sensors-environment-optical-sensor.md#family-specific-implementation-reference), registry row `fam-sensors-environment-optical-sensor`, and variant `var-sensors-environment-optical-sensor-photodiode-module` (Photodiode Module).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Output follows the calibrated spectral/illuminance transfer plus declared saturation, response time, dark offset, and noise.
...
- `docs/tasks/models/cmp-sensors-environment-optical-sensor-photodiode-module-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-sensors-environment-optical-sensor-photodiode-module` (Photodiode Module), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-SENSORS-ENVIRONMENT-OPTICAL-SENSOR-PHOTODIODE-MODULE-F0-CAT`: variant `var-sensors-environment-optical-sensor-photodiode-module` (Photodiode Module), fidelity `F0`, concern `CAT`, and requirements REQ-014, REQ-015, REQ-018, REQ-020, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SENSORS-ENVIRONMENT-OPTICAL-SENSOR-PHOTODIODE-MODULE-F0-CAT-NOMINAL`
- `TEST-CMP-SENSORS-ENVIRONMENT-OPTICAL-SENSOR-PHOTODIODE-MODULE-F0-CAT-BOUNDARY`
- `TEST-CMP-SENSORS-ENVIRONMENT-OPTICAL-SENSOR-PHOTODIODE-MODULE-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-SENSORS-ENVIRONMENT-OPTICAL-SENSOR-PHOTODIODE-MODULE-F0-CAT` exists and is limited to variant `var-sensors-environment-optical-sensor-photodiode-module` (Photodiode Module), `F0`, and `CAT`.
