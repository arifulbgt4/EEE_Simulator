# CMP-MEMORY-STORAGE-EEPROM-FLASH-SHARED-F3-VALIDATION - Validate EEPROM and Flash F3
...
Produce independent validation evidence for **EEPROM and Flash** at **F3** without modifying the model under test.
...
- Stable family: `fam-memory-storage-eeprom-flash` (EEPROM and Flash); task scope: shared family scope across `var-memory-storage-eeprom-flash-eeprom`, `var-memory-storage-eeprom-flash-nor-flash`, `var-memory-storage-eeprom-flash-nand-flash`; fidelity `F3`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F3 rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: A persistent word array follows declared read/program/erase commands, timing, granularity, initialization, endurance metadata, and invalid-command behavior.
...
- `docs/tasks/models/cmp-memory-storage-eeprom-flash-shared-f3-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-MEM-EEPROM_FLASH-NOMINAL`, `GOLD-MEM-EEPROM_FLASH-BOUNDARY`, `GOLD-MEM-EEPROM_FLASH-FAILURE` at `F3`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-MEMORY-STORAGE-EEPROM-FLASH-SHARED-F3-VALIDATION`: shared family scope across `var-memory-storage-eeprom-flash-eeprom`, `var-memory-storage-eeprom-flash-nor-flash`, `var-memory-storage-eeprom-flash-nand-flash`, fidelity `F3`, concern `VALIDATION`, and requirements REQ-018, REQ-021, REQ-025, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MEMORY-STORAGE-EEPROM-FLASH-SHARED-F3-VALIDATION-NOMINAL`
- `TEST-CMP-MEMORY-STORAGE-EEPROM-FLASH-SHARED-F3-VALIDATION-BOUNDARY`
- `TEST-CMP-MEMORY-STORAGE-EEPROM-FLASH-SHARED-F3-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-MEMORY-STORAGE-EEPROM-FLASH-SHARED-F3-VALIDATION` exists and is limited to shared family scope across `var-memory-storage-eeprom-flash-eeprom`, `var-memory-storage-eeprom-flash-nor-flash`, `var-memory-storage-eeprom-flash-nand-flash`, `F3`, and `VALIDATION`.
