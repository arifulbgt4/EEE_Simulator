# CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F4-MODEL - Implement Digital pattern source F4 model
...
Implement one model tier, **F4**, for **Digital pattern source** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-sources-loads-digital-pattern-source` (Digital pattern source); task scope: shared family scope across `var-sources-loads-digital-pattern-source-pattern`; fidelity `F4`; concern `MODEL`.
...
- `docs/tasks/models/cmp-sources-loads-digital-pattern-source-shared-f4-model.md`
...
- One `ModelBinding` for `fam-sources-loads-digital-pattern-source` at `F4` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F4-MODEL`: shared family scope across `var-sources-loads-digital-pattern-source-pattern`, fidelity `F4`, concern `MODEL`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F4-MODEL-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F4-MODEL-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F4-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F4-MODEL` exists and is limited to shared family scope across `var-sources-loads-digital-pattern-source-pattern`, `F4`, and `MODEL`.
