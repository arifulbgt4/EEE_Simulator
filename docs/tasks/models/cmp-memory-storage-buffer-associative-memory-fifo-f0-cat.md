# CMP-MEMORY-STORAGE-BUFFER-ASSOCIATIVE-MEMORY-FIFO-F0-CAT - Specify Fifo catalog preset
...
| Variant | `var-memory-storage-buffer-associative-memory-fifo` - Fifo |
| Fidelity | F0 |
| Concern | CAT |
| Release | R8 |
...
Create the complete, immutable catalog definition for **Fifo** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-memory-storage-buffer-associative-memory` (Buffer and associative memory); task scope: variant `var-memory-storage-buffer-associative-memory-fifo` (Fifo); fidelity `F0`; concern `CAT`.
...
- `docs/tasks/models/cmp-memory-storage-buffer-associative-memory-fifo-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-memory-storage-buffer-associative-memory-fifo` (Fifo), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-MEMORY-STORAGE-BUFFER-ASSOCIATIVE-MEMORY-FIFO-F0-CAT`: variant `var-memory-storage-buffer-associative-memory-fifo` (Fifo), fidelity `F0`, concern `CAT`, and requirements REQ-018, REQ-021, REQ-025, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MEMORY-STORAGE-BUFFER-ASSOCIATIVE-MEMORY-FIFO-F0-CAT-NOMINAL`
- `TEST-CMP-MEMORY-STORAGE-BUFFER-ASSOCIATIVE-MEMORY-FIFO-F0-CAT-BOUNDARY`
- `TEST-CMP-MEMORY-STORAGE-BUFFER-ASSOCIATIVE-MEMORY-FIFO-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-MEMORY-STORAGE-BUFFER-ASSOCIATIVE-MEMORY-FIFO-F0-CAT` exists and is limited to variant `var-memory-storage-buffer-associative-memory-fifo` (Fifo), `F0`, and `CAT`.
