# CMP-INSTRUMENTS-MULTIMETER-CLAMP-F0-CAT - Specify Clamp catalog preset
...
| Variant | `var-instruments-multimeter-clamp` - Clamp |
| Fidelity | F0 |
| Concern | CAT |
| Release | R2 |
...
Create the complete, immutable catalog definition for **Clamp** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-instruments-multimeter` (Multimeter); task scope: variant `var-instruments-multimeter-clamp` (Clamp); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Multimeter family-specific implementation reference](../../catalog/families/fam-instruments-multimeter.md#family-specific-implementation-reference), registry row `fam-instruments-multimeter`, and variant `var-instruments-multimeter-clamp` (Clamp).
...
- `docs/tasks/models/cmp-instruments-multimeter-clamp-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-instruments-multimeter-clamp` (Clamp), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-INSTRUMENTS-MULTIMETER-CLAMP-F0-CAT`: variant `var-instruments-multimeter-clamp` (Clamp), fidelity `F0`, concern `CAT`, and requirements REQ-007, REQ-021, REQ-022, REQ-023, REQ-034, REQ-037.
...
- `TEST-CMP-INSTRUMENTS-MULTIMETER-CLAMP-F0-CAT-NOMINAL`
- `TEST-CMP-INSTRUMENTS-MULTIMETER-CLAMP-F0-CAT-BOUNDARY`
- `TEST-CMP-INSTRUMENTS-MULTIMETER-CLAMP-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-INSTRUMENTS-MULTIMETER-CLAMP-F0-CAT` exists and is limited to variant `var-instruments-multimeter-clamp` (Clamp), `F0`, and `CAT`.
