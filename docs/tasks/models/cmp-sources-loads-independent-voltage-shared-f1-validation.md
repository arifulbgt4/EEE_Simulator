# CMP-SOURCES-LOADS-INDEPENDENT-VOLTAGE-SHARED-F1-VALIDATION - Validate Independent voltage source F1
...
Produce independent validation evidence for **Independent voltage source** at **F1** without modifying the model under test.
...
- Stable family: `fam-sources-loads-independent-voltage` (Independent voltage source); task scope: shared family scope across `var-sources-loads-independent-voltage-dc`, `var-sources-loads-independent-voltage-ac-small-signal`, `var-sources-loads-independent-voltage-sine`, `var-sources-loads-independent-voltage-pulse`; fidelity `F1`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F1 rule: Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: The branch constraint is v(P) - v(N) = V(t), with V(t) supplied by the selected DC/AC/transient profile.
...
- `docs/tasks/models/cmp-sources-loads-independent-voltage-shared-f1-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SRC-INDEPENDENT_VOLTAGE-NOMINAL`, `GOLD-SRC-INDEPENDENT_VOLTAGE-BOUNDARY`, `GOLD-SRC-INDEPENDENT_VOLTAGE-FAILURE` at `F1`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-SOURCES-LOADS-INDEPENDENT-VOLTAGE-SHARED-F1-VALIDATION`: shared family scope across `var-sources-loads-independent-voltage-dc`, `var-sources-loads-independent-voltage-ac-small-signal`, `var-sources-loads-independent-voltage-sine`, `var-sources-loads-independent-voltage-pulse`, fidelity `F1`, concern `VALIDATION`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-VOLTAGE-SHARED-F1-VALIDATION-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-VOLTAGE-SHARED-F1-VALIDATION-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-VOLTAGE-SHARED-F1-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-INDEPENDENT-VOLTAGE-SHARED-F1-VALIDATION` exists and is limited to shared family scope across `var-sources-loads-independent-voltage-dc`, `var-sources-loads-independent-voltage-ac-small-signal`, `var-sources-loads-independent-voltage-sine`, `var-sources-loads-independent-voltage-pulse`, `F1`, and `VALIDATION`.
