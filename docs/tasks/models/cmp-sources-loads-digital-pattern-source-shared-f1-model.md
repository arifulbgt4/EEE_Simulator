# CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F1-MODEL - Implement Digital pattern source F1 model
...
Implement one model tier, **F1**, for **Digital pattern source** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-sources-loads-digital-pattern-source` (Digital pattern source); task scope: shared family scope across `var-sources-loads-digital-pattern-source-pattern`; fidelity `F1`; concern `MODEL`.
...
- Exact F1 execution rule: Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: The source emits the declared 0/1/X/Z vector at integer ticks; simultaneous changes use the scheduler tie order and never depend on host scheduling.
...
- `docs/tasks/models/cmp-sources-loads-digital-pattern-source-shared-f1-model.md`
...
- One `ModelBinding` for `fam-sources-loads-digital-pattern-source` at `F1` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F1-MODEL`: shared family scope across `var-sources-loads-digital-pattern-source-pattern`, fidelity `F1`, concern `MODEL`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F1-MODEL-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F1-MODEL-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F1-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F1-MODEL` exists and is limited to shared family scope across `var-sources-loads-digital-pattern-source-pattern`, `F1`, and `MODEL`.
