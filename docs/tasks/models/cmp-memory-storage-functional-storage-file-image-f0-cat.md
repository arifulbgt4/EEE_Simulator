# CMP-MEMORY-STORAGE-FUNCTIONAL-STORAGE-FILE-IMAGE-F0-CAT - Specify File Image catalog preset
...
| Variant | `var-memory-storage-functional-storage-file-image` - File Image |
| Fidelity | F0 |
| Concern | CAT |
| Release | R8 |
...
Create the complete, immutable catalog definition for **File Image** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-memory-storage-functional-storage` (Functional storage); task scope: variant `var-memory-storage-functional-storage-file-image` (File Image); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Functional storage family-specific implementation reference](../../catalog/families/fam-memory-storage-functional-storage.md#family-specific-implementation-reference), registry row `fam-memory-storage-functional-storage`, and variant `var-memory-storage-functional-storage-file-image` (File Image).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Functional block storage exposes versioned read/write/flush/error operations with deterministic data persistence and bounded latency abstraction.
...
- `docs/tasks/models/cmp-memory-storage-functional-storage-file-image-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-memory-storage-functional-storage-file-image` (File Image), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-MEMORY-STORAGE-FUNCTIONAL-STORAGE-FILE-IMAGE-F0-CAT`: variant `var-memory-storage-functional-storage-file-image` (File Image), fidelity `F0`, concern `CAT`, and requirements REQ-018, REQ-021, REQ-025, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MEMORY-STORAGE-FUNCTIONAL-STORAGE-FILE-IMAGE-F0-CAT-NOMINAL`
- `TEST-CMP-MEMORY-STORAGE-FUNCTIONAL-STORAGE-FILE-IMAGE-F0-CAT-BOUNDARY`
- `TEST-CMP-MEMORY-STORAGE-FUNCTIONAL-STORAGE-FILE-IMAGE-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-MEMORY-STORAGE-FUNCTIONAL-STORAGE-FILE-IMAGE-F0-CAT` exists and is limited to variant `var-memory-storage-functional-storage-file-image` (File Image), `F0`, and `CAT`.
