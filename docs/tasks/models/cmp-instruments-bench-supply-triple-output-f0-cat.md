# CMP-INSTRUMENTS-BENCH-SUPPLY-TRIPLE-OUTPUT-F0-CAT - Specify Triple Output catalog preset
...
| Variant | `var-instruments-bench-supply-triple-output` - Triple Output |
| Fidelity | F0 |
| Concern | CAT |
| Release | R2 |
...
Create the complete, immutable catalog definition for **Triple Output** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-instruments-bench-supply` (Bench supply); task scope: variant `var-instruments-bench-supply-triple-output` (Triple Output); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Bench supply family-specific implementation reference](../../catalog/families/fam-instruments-bench-supply.md#family-specific-implementation-reference), registry row `fam-instruments-bench-supply`, and variant `var-instruments-bench-supply-triple-output` (Triple Output).
...
- `docs/tasks/models/cmp-instruments-bench-supply-triple-output-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-instruments-bench-supply-triple-output` (Triple Output), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-INSTRUMENTS-BENCH-SUPPLY-TRIPLE-OUTPUT-F0-CAT`: variant `var-instruments-bench-supply-triple-output` (Triple Output), fidelity `F0`, concern `CAT`, and requirements REQ-007, REQ-021, REQ-022, REQ-023, REQ-034, REQ-037.
...
- `TEST-CMP-INSTRUMENTS-BENCH-SUPPLY-TRIPLE-OUTPUT-F0-CAT-NOMINAL`
- `TEST-CMP-INSTRUMENTS-BENCH-SUPPLY-TRIPLE-OUTPUT-F0-CAT-BOUNDARY`
- `TEST-CMP-INSTRUMENTS-BENCH-SUPPLY-TRIPLE-OUTPUT-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-INSTRUMENTS-BENCH-SUPPLY-TRIPLE-OUTPUT-F0-CAT` exists and is limited to variant `var-instruments-bench-supply-triple-output` (Triple Output), `F0`, and `CAT`.
