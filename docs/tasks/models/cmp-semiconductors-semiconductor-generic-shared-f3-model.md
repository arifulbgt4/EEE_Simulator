# CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-SHARED-F3-MODEL - Implement Generic semiconductor black box F3 model
...
Implement one model tier, **F3**, for **Generic semiconductor black box** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-semiconductors-semiconductor-generic` (Generic semiconductor black box); task scope: shared family scope across `var-semiconductors-semiconductor-generic-equation-model`; fidelity `F3`; concern `MODEL`.
...
- Exact F3 execution rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Behavior comes only from the selected validated black-box/compact model; its ordered pins, analyses, envelope, provenance, and unsupported behavior are mandatory inputs.
...
- `docs/tasks/models/cmp-semiconductors-semiconductor-generic-shared-f3-model.md`
...
- One `ModelBinding` for `fam-semiconductors-semiconductor-generic` at `F3` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-SHARED-F3-MODEL`: shared family scope across `var-semiconductors-semiconductor-generic-equation-model`, fidelity `F3`, concern `MODEL`, and requirements REQ-009, REQ-010, REQ-011, REQ-012, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-SHARED-F3-MODEL-NOMINAL`
- `TEST-CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-SHARED-F3-MODEL-BOUNDARY`
- `TEST-CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-SHARED-F3-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-SHARED-F3-MODEL` exists and is limited to shared family scope across `var-semiconductors-semiconductor-generic-equation-model`, `F3`, and `MODEL`.
