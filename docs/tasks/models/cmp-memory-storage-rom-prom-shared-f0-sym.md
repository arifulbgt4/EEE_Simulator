# CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F0-SYM - Create symbol and physical representation for ROM and PROM
...
Create original, electrically unambiguous schematic and recognizable physical representations for **ROM and PROM**, including accessible orientation/polarity cues and reusable package mapping.
...
- Stable family: `fam-memory-storage-rom-prom` (ROM and PROM); task scope: shared family scope across `var-memory-storage-rom-prom-mask-rom`, `var-memory-storage-rom-prom-prom`, `var-memory-storage-rom-prom-eprom`; fidelity `F0`; concern `SYM`.
...
- `docs/tasks/models/cmp-memory-storage-rom-prom-shared-f0-sym.md`
...
- One original `sym-memory-storage-rom-prom` schematic definition with anchors for `A:ADDRESS` (input; digital/storage/power), `D:DATA` (bidirectional; digital/storage/power), `C:CONTROL` (input; digital/storage/power), `VDD:VDD` (power; digital/storage/power), `VSS:VSS` (power; digital/storage/power).
...
- Scope is limited to `CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F0-SYM`: shared family scope across `var-memory-storage-rom-prom-mask-rom`, `var-memory-storage-rom-prom-prom`, `var-memory-storage-rom-prom-eprom`, fidelity `F0`, concern `SYM`, and requirements REQ-018, REQ-021, REQ-025, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F0-SYM-NOMINAL`
- `TEST-CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F0-SYM-BOUNDARY`
- `TEST-CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F0-SYM-FAILURE`

## Acceptance

1. The exact output for `CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F0-SYM` exists and is limited to shared family scope across `var-memory-storage-rom-prom-mask-rom`, `var-memory-storage-rom-prom-prom`, `var-memory-storage-rom-prom-eprom`, `F0`, and `SYM`.
