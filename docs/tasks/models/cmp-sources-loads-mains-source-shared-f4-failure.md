# CMP-SOURCES-LOADS-MAINS-SOURCE-SHARED-F4-FAILURE - Implement Mains source failure behavior
...
Implement only the declared F4 failure states and controlled failure injection for **Mains source**.
...
- Stable family: `fam-sources-loads-mains-source` (Mains source); task scope: shared family scope across `var-sources-loads-mains-source-single-phase`; fidelity `F4`; concern `FAILURE`.
...
- `docs/tasks/models/cmp-sources-loads-mains-source-shared-f4-failure.md`
...
- Scope is limited to `CMP-SOURCES-LOADS-MAINS-SOURCE-SHARED-F4-FAILURE`: shared family scope across `var-sources-loads-mains-source-single-phase`, fidelity `F4`, concern `FAILURE`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-MAINS-SOURCE-SHARED-F4-FAILURE-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-MAINS-SOURCE-SHARED-F4-FAILURE-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-MAINS-SOURCE-SHARED-F4-FAILURE-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-MAINS-SOURCE-SHARED-F4-FAILURE` exists and is limited to shared family scope across `var-sources-loads-mains-source-single-phase`, `F4`, and `FAILURE`.
