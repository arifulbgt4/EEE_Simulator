# CMP-DEFERRED-RESEARCH-VACUUM-TUBE-PENTODE-F5-CAT - Specify Pentode catalog preset
...
| Variant | `var-deferred-research-vacuum-tube-pentode` - Pentode |
| Fidelity | F5 |
| Concern | CAT |
| Release | R13 |
...
Create the complete, immutable catalog definition for **Pentode** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-deferred-research-vacuum-tube` (Vacuum tube); task scope: variant `var-deferred-research-vacuum-tube-pentode` (Pentode); fidelity `F5`; concern `CAT`.
...
- Canonical source: [Vacuum tube family-specific implementation reference](../../catalog/families/fam-deferred-research-vacuum-tube.md#family-specific-implementation-reference), registry row `fam-deferred-research-vacuum-tube`, and variant `var-deferred-research-vacuum-tube-pentode` (Pentode).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: F5 behavior is supplied only by an approved external vacuum-tube compact/physical model with pinned equations, parameters, provenance, and envelope.
...
- `docs/tasks/models/cmp-deferred-research-vacuum-tube-pentode-f5-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-deferred-research-vacuum-tube-pentode` (Pentode), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-DEFERRED-RESEARCH-VACUUM-TUBE-PENTODE-F5-CAT`: variant `var-deferred-research-vacuum-tube-pentode` (Pentode), fidelity `F5`, concern `CAT`, and requirements REQ-001, REQ-002, REQ-022, REQ-023, REQ-028, REQ-034, REQ-037, REQ-038.
...
- `TEST-CMP-DEFERRED-RESEARCH-VACUUM-TUBE-PENTODE-F5-CAT-NOMINAL`
- `TEST-CMP-DEFERRED-RESEARCH-VACUUM-TUBE-PENTODE-F5-CAT-BOUNDARY`
- `TEST-CMP-DEFERRED-RESEARCH-VACUUM-TUBE-PENTODE-F5-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-DEFERRED-RESEARCH-VACUUM-TUBE-PENTODE-F5-CAT` exists and is limited to variant `var-deferred-research-vacuum-tube-pentode` (Pentode), `F5`, and `CAT`.
