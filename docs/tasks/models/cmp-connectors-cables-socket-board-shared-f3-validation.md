# CMP-CONNECTORS-CABLES-SOCKET-BOARD-SHARED-F3-VALIDATION - Validate Socket and prototyping board F3
...
Produce independent validation evidence for **Socket and prototyping board** at **F3** without modifying the model under test.
...
- Candidate and independent reference must evaluate the same F3 rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: The selected socket/breadboard topology defines an explicit contact-connectivity matrix plus declared contact resistance and parasitics.
...
- `docs/tasks/models/cmp-connectors-cables-socket-board-shared-f3-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-CAB-SOCKET_BOARD-NOMINAL`, `GOLD-CAB-SOCKET_BOARD-BOUNDARY`, `GOLD-CAB-SOCKET_BOARD-FAILURE` at `F3`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-CONNECTORS-CABLES-SOCKET-BOARD-SHARED-F3-VALIDATION`: shared family scope across `var-connectors-cables-socket-board-ic-socket`, `var-connectors-cables-socket-board-breadboard`, `var-connectors-cables-socket-board-card-socket`, `var-connectors-cables-socket-board-screw-terminal`, `var-connectors-cables-socket-board-spring-terminal`, `var-connectors-cables-socket-board-pogo-array`, fidelity `F3`, concern `VALIDATION`, and requirements REQ-005, REQ-014, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-CONNECTORS-CABLES-SOCKET-BOARD-SHARED-F3-VALIDATION-NOMINAL`
- `TEST-CMP-CONNECTORS-CABLES-SOCKET-BOARD-SHARED-F3-VALIDATION-BOUNDARY`
- `TEST-CMP-CONNECTORS-CABLES-SOCKET-BOARD-SHARED-F3-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-CONNECTORS-CABLES-SOCKET-BOARD-SHARED-F3-VALIDATION` exists and is limited to shared family scope across `var-connectors-cables-socket-board-ic-socket`, `var-connectors-cables-socket-board-breadboard`, `var-connectors-cables-socket-board-card-socket`, `var-connectors-cables-socket-board-screw-terminal`, `var-connectors-cables-socket-board-spring-terminal`, `var-connectors-cables-socket-board-pogo-array`, `F3`, and `VALIDATION`.
