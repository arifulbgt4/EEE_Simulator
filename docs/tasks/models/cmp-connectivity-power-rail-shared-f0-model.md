# CMP-CONNECTIVITY-POWER-RAIL-SHARED-F0-MODEL - Implement Power rail F0 model
...
Implement one model tier, **F0**, for **Power rail** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-connectivity-power-rail` (Power rail); task scope: shared family scope across `var-connectivity-power-rail-parameterized-rail`; fidelity `F0`; concern `MODEL`.
...
- Exact F0 execution rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: A rail creates a named power net; it imposes a voltage equation only when a source/rail profile explicitly declares one, otherwise it is connectivity metadata.
...
- `docs/tasks/models/cmp-connectivity-power-rail-shared-f0-model.md`
...
- One `ModelBinding` for `fam-connectivity-power-rail` at `F0` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-CONNECTIVITY-POWER-RAIL-SHARED-F0-MODEL`: shared family scope across `var-connectivity-power-rail-parameterized-rail`, fidelity `F0`, concern `MODEL`, and requirements REQ-004, REQ-005, REQ-008, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-CONNECTIVITY-POWER-RAIL-SHARED-F0-MODEL-NOMINAL`
- `TEST-CMP-CONNECTIVITY-POWER-RAIL-SHARED-F0-MODEL-BOUNDARY`
- `TEST-CMP-CONNECTIVITY-POWER-RAIL-SHARED-F0-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-CONNECTIVITY-POWER-RAIL-SHARED-F0-MODEL` exists and is limited to shared family scope across `var-connectivity-power-rail-parameterized-rail`, `F0`, and `MODEL`.
