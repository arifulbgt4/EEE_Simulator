# CMP-MCU-FPGA-SERIAL-PERIPHERAL-UART-F0-CAT - Specify Uart catalog preset
...
| Variant | `var-mcu-fpga-serial-peripheral-uart` - Uart |
| Fidelity | F0 |
| Concern | CAT |
| Release | R7 |
...
Create the complete, immutable catalog definition for **Uart** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-mcu-fpga-serial-peripheral` (Serial peripheral); task scope: variant `var-mcu-fpga-serial-peripheral-uart` (Uart); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Serial peripheral family-specific implementation reference](../../catalog/families/fam-mcu-fpga-serial-peripheral.md#family-specific-implementation-reference), registry row `fam-mcu-fpga-serial-peripheral`, and variant `var-mcu-fpga-serial-peripheral-uart` (Uart).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: The selected UART/I2C/SPI/CAN/USB profile maps timed line events to framed protocol state with explicit arbitration, errors, buffering, and clock rules.
...
- `docs/tasks/models/cmp-mcu-fpga-serial-peripheral-uart-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-mcu-fpga-serial-peripheral-uart` (Uart), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-MCU-FPGA-SERIAL-PERIPHERAL-UART-F0-CAT`: variant `var-mcu-fpga-serial-peripheral-uart` (Uart), fidelity `F0`, concern `CAT`, and requirements REQ-020, REQ-021, REQ-024, REQ-025, REQ-026, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MCU-FPGA-SERIAL-PERIPHERAL-UART-F0-CAT-NOMINAL`
- `TEST-CMP-MCU-FPGA-SERIAL-PERIPHERAL-UART-F0-CAT-BOUNDARY`
- `TEST-CMP-MCU-FPGA-SERIAL-PERIPHERAL-UART-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-MCU-FPGA-SERIAL-PERIPHERAL-UART-F0-CAT` exists and is limited to variant `var-mcu-fpga-serial-peripheral-uart` (Uart), `F0`, and `CAT`.
