...
- Stable family: `fam-actuators-hmi-servo` (Servo); task scope: variant `var-actuators-hmi-servo-positional` (Positional); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Servo family-specific implementation reference](../../catalog/families/fam-actuators-hmi-servo.md#family-specific-implementation-reference), registry row `fam-actuators-hmi-servo`, and variant `var-actuators-hmi-servo-positional` (Positional).
...
- `docs/tasks/models/cmp-actuators-hmi-servo-positional-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-actuators-hmi-servo-positional` (Positional), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-ACTUATORS-HMI-SERVO-POSITIONAL-F0-CAT`: variant `var-actuators-hmi-servo-positional` (Positional), fidelity `F0`, concern `CAT`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ACTUATORS-HMI-SERVO-POSITIONAL-F0-CAT-NOMINAL`
- `TEST-CMP-ACTUATORS-HMI-SERVO-POSITIONAL-F0-CAT-BOUNDARY`
- `TEST-CMP-ACTUATORS-HMI-SERVO-POSITIONAL-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-ACTUATORS-HMI-SERVO-POSITIONAL-F0-CAT` exists and is limited to variant `var-actuators-hmi-servo-positional` (Positional), `F0`, and `CAT`.
