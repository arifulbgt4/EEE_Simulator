# CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-CONSTANT-STIMULUS-F0-CAT - Specify Constant Stimulus catalog preset
...
| Variant | `var-sensors-environment-environment-stimulus-constant-stimulus` - Constant Stimulus |
| Fidelity | F0 |
| Concern | CAT |
| Release | R7 |
...
Create the complete, immutable catalog definition for **Constant Stimulus** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-sensors-environment-environment-stimulus` (Environmental stimulus); task scope: variant `var-sensors-environment-environment-stimulus-constant-stimulus` (Constant Stimulus); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Environmental stimulus family-specific implementation reference](../../catalog/families/fam-sensors-environment-environment-stimulus.md#family-specific-implementation-reference), registry row `fam-sensors-environment-environment-stimulus`, and variant `var-sensors-environment-environment-stimulus-constant-stimulus` (Constant Stimulus).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: The primitive emits a deterministic SI-valued environmental field/time profile; interpolation, spatial scope, discontinuities, and seed are explicit.
...
- `docs/tasks/models/cmp-sensors-environment-environment-stimulus-constant-stimulus-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-sensors-environment-environment-stimulus-constant-stimulus` (Constant Stimulus), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-CONSTANT-STIMULUS-F0-CAT`: variant `var-sensors-environment-environment-stimulus-constant-stimulus` (Constant Stimulus), fidelity `F0`, concern `CAT`, and requirements REQ-014, REQ-015, REQ-018, REQ-020, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-CONSTANT-STIMULUS-F0-CAT-NOMINAL`
- `TEST-CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-CONSTANT-STIMULUS-F0-CAT-BOUNDARY`
- `TEST-CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-CONSTANT-STIMULUS-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-CONSTANT-STIMULUS-F0-CAT` exists and is limited to variant `var-sensors-environment-environment-stimulus-constant-stimulus` (Constant Stimulus), `F0`, and `CAT`.
