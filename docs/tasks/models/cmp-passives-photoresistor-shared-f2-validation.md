# CMP-PASSIVES-PHOTORESISTOR-SHARED-F2-VALIDATION - Validate Photoresistor F2
...
Produce independent validation evidence for **Photoresistor** at **F2** without modifying the model under test.
...
- Stable family: `fam-passives-photoresistor` (Photoresistor); task scope: shared family scope across `var-passives-photoresistor-ldr`; fidelity `F2`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F2 rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: Resistance is the declared bounded transfer R = f(illuminance,T) with response-time state; zero or out-of-range stimulus uses explicit saturation behavior.
...
- `docs/tasks/models/cmp-passives-photoresistor-shared-f2-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-PAS-PHOTORESISTOR-NOMINAL`, `GOLD-PAS-PHOTORESISTOR-BOUNDARY`, `GOLD-PAS-PHOTORESISTOR-FAILURE` at `F2`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-PASSIVES-PHOTORESISTOR-SHARED-F2-VALIDATION`: shared family scope across `var-passives-photoresistor-ldr`, fidelity `F2`, concern `VALIDATION`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-PHOTORESISTOR-SHARED-F2-VALIDATION-NOMINAL`
- `TEST-CMP-PASSIVES-PHOTORESISTOR-SHARED-F2-VALIDATION-BOUNDARY`
- `TEST-CMP-PASSIVES-PHOTORESISTOR-SHARED-F2-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-PHOTORESISTOR-SHARED-F2-VALIDATION` exists and is limited to shared family scope across `var-passives-photoresistor-ldr`, `F2`, and `VALIDATION`.
