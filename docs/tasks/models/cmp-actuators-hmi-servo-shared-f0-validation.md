# CMP-ACTUATORS-HMI-SERVO-SHARED-F0-VALIDATION - Validate Servo F0
...
Produce independent validation evidence for **Servo** at **F0** without modifying the model under test.
...
- Stable family: `fam-actuators-hmi-servo` (Servo); task scope: shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`; fidelity `F0`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F0 rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Command pulse/position drives a bounded closed-loop angle state with declared range, speed, dead band, torque/load, current, and stall behavior.
...
- `docs/tasks/models/cmp-actuators-hmi-servo-shared-f0-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-ACT-SERVO-NOMINAL`, `GOLD-ACT-SERVO-BOUNDARY`, `GOLD-ACT-SERVO-FAILURE` at `F0`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-ACTUATORS-HMI-SERVO-SHARED-F0-VALIDATION`: shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`, fidelity `F0`, concern `VALIDATION`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F0-VALIDATION-NOMINAL`
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F0-VALIDATION-BOUNDARY`
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F0-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-ACTUATORS-HMI-SERVO-SHARED-F0-VALIDATION` exists and is limited to shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`, `F0`, and `VALIDATION`.
