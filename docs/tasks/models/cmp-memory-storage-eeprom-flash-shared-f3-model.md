# CMP-MEMORY-STORAGE-EEPROM-FLASH-SHARED-F3-MODEL - Implement EEPROM and Flash F3 model
...
Implement one model tier, **F3**, for **EEPROM and Flash** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-memory-storage-eeprom-flash` (EEPROM and Flash); task scope: shared family scope across `var-memory-storage-eeprom-flash-eeprom`, `var-memory-storage-eeprom-flash-nor-flash`, `var-memory-storage-eeprom-flash-nand-flash`; fidelity `F3`; concern `MODEL`.
...
- Exact F3 execution rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: A persistent word array follows declared read/program/erase commands, timing, granularity, initialization, endurance metadata, and invalid-command behavior.
...
- `docs/tasks/models/cmp-memory-storage-eeprom-flash-shared-f3-model.md`
...
- One `ModelBinding` for `fam-memory-storage-eeprom-flash` at `F3` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-MEMORY-STORAGE-EEPROM-FLASH-SHARED-F3-MODEL`: shared family scope across `var-memory-storage-eeprom-flash-eeprom`, `var-memory-storage-eeprom-flash-nor-flash`, `var-memory-storage-eeprom-flash-nand-flash`, fidelity `F3`, concern `MODEL`, and requirements REQ-018, REQ-021, REQ-025, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MEMORY-STORAGE-EEPROM-FLASH-SHARED-F3-MODEL-NOMINAL`
- `TEST-CMP-MEMORY-STORAGE-EEPROM-FLASH-SHARED-F3-MODEL-BOUNDARY`
- `TEST-CMP-MEMORY-STORAGE-EEPROM-FLASH-SHARED-F3-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-MEMORY-STORAGE-EEPROM-FLASH-SHARED-F3-MODEL` exists and is limited to shared family scope across `var-memory-storage-eeprom-flash-eeprom`, `var-memory-storage-eeprom-flash-nor-flash`, `var-memory-storage-eeprom-flash-nand-flash`, `F3`, and `MODEL`.
