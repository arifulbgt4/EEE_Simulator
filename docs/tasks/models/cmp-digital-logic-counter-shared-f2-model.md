# CMP-DIGITAL-LOGIC-COUNTER-SHARED-F2-MODEL - Implement Counter F2 model
...
Implement one model tier, **F2**, for **Counter** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-digital-logic-counter` (Counter); task scope: shared family scope across `var-digital-logic-counter-ripple`, `var-digital-logic-counter-synchronous`, `var-digital-logic-counter-up-down`; fidelity `F2`; concern `MODEL`.
...
- Exact F2 execution rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: A bounded state value advances, decrements, loads, or resets on declared events with modulus, carry/borrow, enable, and timing rules.
...
- `docs/tasks/models/cmp-digital-logic-counter-shared-f2-model.md`
...
- One `ModelBinding` for `fam-digital-logic-counter` at `F2` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-DIGITAL-LOGIC-COUNTER-SHARED-F2-MODEL`: shared family scope across `var-digital-logic-counter-ripple`, `var-digital-logic-counter-synchronous`, `var-digital-logic-counter-up-down`, fidelity `F2`, concern `MODEL`, and requirements REQ-018, REQ-021, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-DIGITAL-LOGIC-COUNTER-SHARED-F2-MODEL-NOMINAL`
- `TEST-CMP-DIGITAL-LOGIC-COUNTER-SHARED-F2-MODEL-BOUNDARY`
- `TEST-CMP-DIGITAL-LOGIC-COUNTER-SHARED-F2-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-DIGITAL-LOGIC-COUNTER-SHARED-F2-MODEL` exists and is limited to shared family scope across `var-digital-logic-counter-ripple`, `var-digital-logic-counter-synchronous`, `var-digital-logic-counter-up-down`, `F2`, and `MODEL`.
