# CMP-SENSORS-ENVIRONMENT-FORCE-PRESSURE-SENSOR-DIFFERENTIAL-PRESSURE-F0-CAT - Specify Differential Pressure catalog preset
...
| Variant | `var-sensors-environment-force-pressure-sensor-differential-pressure` - Differential Pressure |
| Fidelity | F0 |
| Concern | CAT |
| Release | R7 |
...
Create the complete, immutable catalog definition for **Differential Pressure** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-sensors-environment-force-pressure-sensor` (Force and pressure sensor); task scope: variant `var-sensors-environment-force-pressure-sensor-differential-pressure` (Differential Pressure); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Force and pressure sensor family-specific implementation reference](../../catalog/families/fam-sensors-environment-force-pressure-sensor.md#family-specific-implementation-reference), registry row `fam-sensors-environment-force-pressure-sensor`, and variant `var-sensors-environment-force-pressure-sensor-differential-pressure` (Differential Pressure).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Output follows the calibrated force/strain/pressure transfer with bridge or transducer excitation, range, hysteresis, creep, and temperature terms.
...
- `docs/tasks/models/cmp-sensors-environment-force-pressure-sensor-differential-pressure-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-sensors-environment-force-pressure-sensor-differential-pressure` (Differential Pressure), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-SENSORS-ENVIRONMENT-FORCE-PRESSURE-SENSOR-DIFFERENTIAL-PRESSURE-F0-CAT`: variant `var-sensors-environment-force-pressure-sensor-differential-pressure` (Differential Pressure), fidelity `F0`, concern `CAT`, and requirements REQ-014, REQ-015, REQ-018, REQ-020, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SENSORS-ENVIRONMENT-FORCE-PRESSURE-SENSOR-DIFFERENTIAL-PRESSURE-F0-CAT-NOMINAL`
- `TEST-CMP-SENSORS-ENVIRONMENT-FORCE-PRESSURE-SENSOR-DIFFERENTIAL-PRESSURE-F0-CAT-BOUNDARY`
- `TEST-CMP-SENSORS-ENVIRONMENT-FORCE-PRESSURE-SENSOR-DIFFERENTIAL-PRESSURE-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-SENSORS-ENVIRONMENT-FORCE-PRESSURE-SENSOR-DIFFERENTIAL-PRESSURE-F0-CAT` exists and is limited to variant `var-sensors-environment-force-pressure-sensor-differential-pressure` (Differential Pressure), `F0`, and `CAT`.
