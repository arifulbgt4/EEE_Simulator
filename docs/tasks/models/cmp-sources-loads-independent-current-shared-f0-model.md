# CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F0-MODEL - Implement Independent current source F0 model
...
Implement one model tier, **F0**, for **Independent current source** using the family equations/behavior and no higher-fidelity claims.
...
- Exact F0 execution rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The oriented branch current is i(P->N) = I(t), with I(t) supplied by the selected DC/AC/transient profile.
...
- `docs/tasks/models/cmp-sources-loads-independent-current-shared-f0-model.md`
...
- One `ModelBinding` for `fam-sources-loads-independent-current` at `F0` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F0-MODEL`: shared family scope across `var-sources-loads-independent-current-dc`, `var-sources-loads-independent-current-ac-small-signal`, `var-sources-loads-independent-current-sine`, `var-sources-loads-independent-current-pulse`, fidelity `F0`, concern `MODEL`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F0-MODEL-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F0-MODEL-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F0-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F0-MODEL` exists and is limited to shared family scope across `var-sources-loads-independent-current-dc`, `var-sources-loads-independent-current-ac-small-signal`, `var-sources-loads-independent-current-sine`, `var-sources-loads-independent-current-pulse`, `F0`, and `MODEL`.
