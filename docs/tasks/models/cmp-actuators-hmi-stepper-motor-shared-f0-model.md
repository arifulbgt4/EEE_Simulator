# CMP-ACTUATORS-HMI-STEPPER-MOTOR-SHARED-F0-MODEL - Implement Stepper motor F0 model
...
Implement one model tier, **F0**, for **Stepper motor** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-actuators-hmi-stepper-motor` (Stepper motor); task scope: shared family scope across `var-actuators-hmi-stepper-motor-unipolar`, `var-actuators-hmi-stepper-motor-bipolar`, `var-actuators-hmi-stepper-motor-hybrid`; fidelity `F0`; concern `MODEL`.
...
- Exact F0 execution rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Phase-current and rotor angle/speed states follow the declared step/torque/detent/inertia model with missed-step and limit behavior.
...
- `docs/tasks/models/cmp-actuators-hmi-stepper-motor-shared-f0-model.md`
...
- One `ModelBinding` for `fam-actuators-hmi-stepper-motor` at `F0` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-ACTUATORS-HMI-STEPPER-MOTOR-SHARED-F0-MODEL`: shared family scope across `var-actuators-hmi-stepper-motor-unipolar`, `var-actuators-hmi-stepper-motor-bipolar`, `var-actuators-hmi-stepper-motor-hybrid`, fidelity `F0`, concern `MODEL`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ACTUATORS-HMI-STEPPER-MOTOR-SHARED-F0-MODEL-NOMINAL`
- `TEST-CMP-ACTUATORS-HMI-STEPPER-MOTOR-SHARED-F0-MODEL-BOUNDARY`
- `TEST-CMP-ACTUATORS-HMI-STEPPER-MOTOR-SHARED-F0-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-ACTUATORS-HMI-STEPPER-MOTOR-SHARED-F0-MODEL` exists and is limited to shared family scope across `var-actuators-hmi-stepper-motor-unipolar`, `var-actuators-hmi-stepper-motor-bipolar`, `var-actuators-hmi-stepper-motor-hybrid`, `F0`, and `MODEL`.
