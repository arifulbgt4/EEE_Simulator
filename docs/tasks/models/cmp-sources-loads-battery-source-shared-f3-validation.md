# CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F3-VALIDATION - Validate Battery source F3
...
Produce independent validation evidence for **Battery source** at **F3** without modifying the model under test.
...
- Stable family: `fam-sources-loads-battery-source` (Battery source); task scope: shared family scope across `var-sources-loads-battery-source-battery-pack`; fidelity `F3`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F3 rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Terminal voltage is OCV(SOC,T) - I*R_internal(SOC,T), with SOC advanced by charge balance dSOC/dt = -I/Q_capacity under the declared sign convention.
...
- `docs/tasks/models/cmp-sources-loads-battery-source-shared-f3-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SRC-BATTERY_SOURCE-NOMINAL`, `GOLD-SRC-BATTERY_SOURCE-BOUNDARY`, `GOLD-SRC-BATTERY_SOURCE-FAILURE` at `F3`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F3-VALIDATION`: shared family scope across `var-sources-loads-battery-source-battery-pack`, fidelity `F3`, concern `VALIDATION`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F3-VALIDATION-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F3-VALIDATION-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F3-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F3-VALIDATION` exists and is limited to shared family scope across `var-sources-loads-battery-source-battery-pack`, `F3`, and `VALIDATION`.
