# CMP-RF-COMMUNICATIONS-RF-PORT-NETWORK-S-PARAMETER-BLACK-BOX-F1-CAT - Specify S Parameter Black Box catalog preset
...
| Variant | `var-rf-communications-rf-port-network-s-parameter-black-box` - S Parameter Black Box |
| Fidelity | F1 |
| Concern | CAT |
| Release | R7 |
...
Create the complete, immutable catalog definition for **S Parameter Black Box** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-rf-communications-rf-port-network` (RF port and network model); task scope: variant `var-rf-communications-rf-port-network-s-parameter-black-box` (S Parameter Black Box); fidelity `F1`; concern `CAT`.
...
- Canonical source: [RF port and network model family-specific implementation reference](../../catalog/families/fam-rf-communications-rf-port-network.md#family-specific-implementation-reference), registry row `fam-rf-communications-rf-port-network`, and variant `var-rf-communications-rf-port-network-s-parameter-black-box` (S Parameter Black Box).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: At each declared frequency, incident/reflected waves satisfy b = S(f)*a at the stated reference impedance; passivity, causality, and port order are validated.
...
- `docs/tasks/models/cmp-rf-communications-rf-port-network-s-parameter-black-box-f1-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-rf-communications-rf-port-network-s-parameter-black-box` (S Parameter Black Box), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-RF-COMMUNICATIONS-RF-PORT-NETWORK-S-PARAMETER-BLACK-BOX-F1-CAT`: variant `var-rf-communications-rf-port-network-s-parameter-black-box` (S Parameter Black Box), fidelity `F1`, concern `CAT`, and requirements REQ-011, REQ-012, REQ-024, REQ-028, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-RF-COMMUNICATIONS-RF-PORT-NETWORK-S-PARAMETER-BLACK-BOX-F1-CAT-NOMINAL`
- `TEST-CMP-RF-COMMUNICATIONS-RF-PORT-NETWORK-S-PARAMETER-BLACK-BOX-F1-CAT-BOUNDARY`
- `TEST-CMP-RF-COMMUNICATIONS-RF-PORT-NETWORK-S-PARAMETER-BLACK-BOX-F1-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-RF-COMMUNICATIONS-RF-PORT-NETWORK-S-PARAMETER-BLACK-BOX-F1-CAT` exists and is limited to variant `var-rf-communications-rf-port-network-s-parameter-black-box` (S Parameter Black Box), `F1`, and `CAT`.
