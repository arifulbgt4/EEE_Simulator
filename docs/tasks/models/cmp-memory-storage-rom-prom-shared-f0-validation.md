# CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F0-VALIDATION - Validate ROM and PROM F0
...
Produce independent validation evidence for **ROM and PROM** at **F0** without modifying the model under test.
...
- Stable family: `fam-memory-storage-rom-prom` (ROM and PROM); task scope: shared family scope across `var-memory-storage-rom-prom-mask-rom`, `var-memory-storage-rom-prom-prom`, `var-memory-storage-rom-prom-eprom`; fidelity `F0`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F0 rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Address selects an immutable/programmed word; range, enable, output delay, initialization, and unknown-address behavior are explicit.
...
- `docs/tasks/models/cmp-memory-storage-rom-prom-shared-f0-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-MEM-ROM_PROM-NOMINAL`, `GOLD-MEM-ROM_PROM-BOUNDARY`, `GOLD-MEM-ROM_PROM-FAILURE` at `F0`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F0-VALIDATION`: shared family scope across `var-memory-storage-rom-prom-mask-rom`, `var-memory-storage-rom-prom-prom`, `var-memory-storage-rom-prom-eprom`, fidelity `F0`, concern `VALIDATION`, and requirements REQ-018, REQ-021, REQ-025, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F0-VALIDATION-NOMINAL`
- `TEST-CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F0-VALIDATION-BOUNDARY`
- `TEST-CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F0-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F0-VALIDATION` exists and is limited to shared family scope across `var-memory-storage-rom-prom-mask-rom`, `var-memory-storage-rom-prom-prom`, `var-memory-storage-rom-prom-eprom`, `F0`, and `VALIDATION`.
