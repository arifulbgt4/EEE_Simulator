# CMP-POWER-ELECTRONICS-PWM-CONTROLLER-CONFIGURABLE-PWM-F0-CAT - Specify Configurable Pwm catalog preset
...
| Variant | `var-power-electronics-pwm-controller-configurable-pwm` - Configurable Pwm |
| Fidelity | F0 |
| Concern | CAT |
| Release | R7 |
...
Create the complete, immutable catalog definition for **Configurable Pwm** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-power-electronics-pwm-controller` (PWM controller); task scope: variant `var-power-electronics-pwm-controller-configurable-pwm` (Configurable Pwm); fidelity `F0`; concern `CAT`.
...
- Canonical source: [PWM controller family-specific implementation reference](../../catalog/families/fam-power-electronics-pwm-controller.md#family-specific-implementation-reference), registry row `fam-power-electronics-pwm-controller`, and variant `var-power-electronics-pwm-controller-configurable-pwm` (Configurable Pwm).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: The deterministic modulator compares command and carrier/control state to emit bounded PWM events with declared frequency, dead time, and saturation.
...
- `docs/tasks/models/cmp-power-electronics-pwm-controller-configurable-pwm-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-power-electronics-pwm-controller-configurable-pwm` (Configurable Pwm), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-POWER-ELECTRONICS-PWM-CONTROLLER-CONFIGURABLE-PWM-F0-CAT`: variant `var-power-electronics-pwm-controller-configurable-pwm` (Configurable Pwm), fidelity `F0`, concern `CAT`, and requirements REQ-009, REQ-010, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-POWER-ELECTRONICS-PWM-CONTROLLER-CONFIGURABLE-PWM-F0-CAT-NOMINAL`
- `TEST-CMP-POWER-ELECTRONICS-PWM-CONTROLLER-CONFIGURABLE-PWM-F0-CAT-BOUNDARY`
- `TEST-CMP-POWER-ELECTRONICS-PWM-CONTROLLER-CONFIGURABLE-PWM-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-POWER-ELECTRONICS-PWM-CONTROLLER-CONFIGURABLE-PWM-F0-CAT` exists and is limited to variant `var-power-electronics-pwm-controller-configurable-pwm` (Configurable Pwm), `F0`, and `CAT`.
