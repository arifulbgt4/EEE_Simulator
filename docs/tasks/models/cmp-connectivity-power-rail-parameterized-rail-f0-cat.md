# CMP-CONNECTIVITY-POWER-RAIL-PARAMETERIZED-RAIL-F0-CAT - Specify Parameterized Rail catalog preset
...
| Variant | `var-connectivity-power-rail-parameterized-rail` - Parameterized Rail |
| Fidelity | F0 |
| Concern | CAT |
| Release | R1 |
...
Create the complete, immutable catalog definition for **Parameterized Rail** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-connectivity-power-rail` (Power rail); task scope: variant `var-connectivity-power-rail-parameterized-rail` (Parameterized Rail); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Power rail family-specific implementation reference](../../catalog/families/fam-connectivity-power-rail.md#family-specific-implementation-reference), registry row `fam-connectivity-power-rail`, and variant `var-connectivity-power-rail-parameterized-rail` (Parameterized Rail).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: A rail creates a named power net; it imposes a voltage equation only when a source/rail profile explicitly declares one, otherwise it is connectivity metadata.
...
- `docs/tasks/models/cmp-connectivity-power-rail-parameterized-rail-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-connectivity-power-rail-parameterized-rail` (Parameterized Rail), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-CONNECTIVITY-POWER-RAIL-PARAMETERIZED-RAIL-F0-CAT`: variant `var-connectivity-power-rail-parameterized-rail` (Parameterized Rail), fidelity `F0`, concern `CAT`, and requirements REQ-004, REQ-005, REQ-008, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-CONNECTIVITY-POWER-RAIL-PARAMETERIZED-RAIL-F0-CAT-NOMINAL`
- `TEST-CMP-CONNECTIVITY-POWER-RAIL-PARAMETERIZED-RAIL-F0-CAT-BOUNDARY`
- `TEST-CMP-CONNECTIVITY-POWER-RAIL-PARAMETERIZED-RAIL-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-CONNECTIVITY-POWER-RAIL-PARAMETERIZED-RAIL-F0-CAT` exists and is limited to variant `var-connectivity-power-rail-parameterized-rail` (Parameterized Rail), `F0`, and `CAT`.
