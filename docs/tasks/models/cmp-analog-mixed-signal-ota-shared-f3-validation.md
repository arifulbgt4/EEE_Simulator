# CMP-ANALOG-MIXED-SIGNAL-OTA-SHARED-F3-VALIDATION - Validate Operational transconductance amplifier F3
...
Produce independent validation evidence for **Operational transconductance amplifier** at **F3** without modifying the model under test.
...
- Stable family: `fam-analog-mixed-signal-ota` (Operational transconductance amplifier); task scope: shared family scope across `var-analog-mixed-signal-ota-single-ended`, `var-analog-mixed-signal-ota-differential`; fidelity `F3`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F3 rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Output current is Iout = gm*(Vplus-Vminus), bounded by bias, compliance, bandwidth, slew, and supply limits.
...
- `docs/tasks/models/cmp-analog-mixed-signal-ota-shared-f3-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-AMS-OTA-NOMINAL`, `GOLD-AMS-OTA-BOUNDARY`, `GOLD-AMS-OTA-FAILURE` at `F3`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-ANALOG-MIXED-SIGNAL-OTA-SHARED-F3-VALIDATION`: shared family scope across `var-analog-mixed-signal-ota-single-ended`, `var-analog-mixed-signal-ota-differential`, fidelity `F3`, concern `VALIDATION`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-018, REQ-020, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ANALOG-MIXED-SIGNAL-OTA-SHARED-F3-VALIDATION-NOMINAL`
- `TEST-CMP-ANALOG-MIXED-SIGNAL-OTA-SHARED-F3-VALIDATION-BOUNDARY`
- `TEST-CMP-ANALOG-MIXED-SIGNAL-OTA-SHARED-F3-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-ANALOG-MIXED-SIGNAL-OTA-SHARED-F3-VALIDATION` exists and is limited to shared family scope across `var-analog-mixed-signal-ota-single-ended`, `var-analog-mixed-signal-ota-differential`, `F3`, and `VALIDATION`.
