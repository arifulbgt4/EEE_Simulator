# CMP-DIGITAL-LOGIC-STATE-MACHINE-SHARED-F3-VALIDATION - Validate State machine F3
...
Produce independent validation evidence for **State machine** at **F3** without modifying the model under test.
...
- Stable family: `fam-digital-logic-state-machine` (State machine); task scope: shared family scope across `var-digital-logic-state-machine-moore`, `var-digital-logic-state-machine-mealy`; fidelity `F3`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F3 rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: At each declared event, next_state = F(state,input) and output = G(state,input) using the versioned transition/output tables.
...
- `docs/tasks/models/cmp-digital-logic-state-machine-shared-f3-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-DIG-STATE_MACHINE-NOMINAL`, `GOLD-DIG-STATE_MACHINE-BOUNDARY`, `GOLD-DIG-STATE_MACHINE-FAILURE` at `F3`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-DIGITAL-LOGIC-STATE-MACHINE-SHARED-F3-VALIDATION`: shared family scope across `var-digital-logic-state-machine-moore`, `var-digital-logic-state-machine-mealy`, fidelity `F3`, concern `VALIDATION`, and requirements REQ-018, REQ-021, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-DIGITAL-LOGIC-STATE-MACHINE-SHARED-F3-VALIDATION-NOMINAL`
- `TEST-CMP-DIGITAL-LOGIC-STATE-MACHINE-SHARED-F3-VALIDATION-BOUNDARY`
- `TEST-CMP-DIGITAL-LOGIC-STATE-MACHINE-SHARED-F3-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-DIGITAL-LOGIC-STATE-MACHINE-SHARED-F3-VALIDATION` exists and is limited to shared family scope across `var-digital-logic-state-machine-moore`, `var-digital-logic-state-machine-mealy`, `F3`, and `VALIDATION`.
