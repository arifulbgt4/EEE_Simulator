# CMP-SENSORS-ENVIRONMENT-CLIMATE-GAS-SENSOR-SHARED-F3-VALIDATION - Validate Climate and gas sensor F3
...
Produce independent validation evidence for **Climate and gas sensor** at **F3** without modifying the model under test.
...
- Stable family: `fam-sensors-environment-climate-gas-sensor` (Climate and gas sensor); task scope: shared family scope across `var-sensors-environment-climate-gas-sensor-humidity`, `var-sensors-environment-climate-gas-sensor-barometric`, `var-sensors-environment-climate-gas-sensor-carbon-dioxide`, `var-sensors-environment-climate-gas-sensor-voc`, `var-sensors-environment-climate-gas-sensor-generic-gas`; fidelity `F3`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F3 rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Output follows the declared humidity/gas concentration transfer with warm-up, cross-sensitivity, saturation, response/recovery, and environmental terms.
...
- `docs/tasks/models/cmp-sensors-environment-climate-gas-sensor-shared-f3-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SEN-CLIMATE_GAS_SENSOR-NOMINAL`, `GOLD-SEN-CLIMATE_GAS_SENSOR-BOUNDARY`, `GOLD-SEN-CLIMATE_GAS_SENSOR-FAILURE` at `F3`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-SENSORS-ENVIRONMENT-CLIMATE-GAS-SENSOR-SHARED-F3-VALIDATION`: shared family scope across `var-sensors-environment-climate-gas-sensor-humidity`, `var-sensors-environment-climate-gas-sensor-barometric`, `var-sensors-environment-climate-gas-sensor-carbon-dioxide`, `var-sensors-environment-climate-gas-sensor-voc`, `var-sensors-environment-climate-gas-sensor-generic-gas`, fidelity `F3`, concern `VALIDATION`, and requirements REQ-014, REQ-015, REQ-018, REQ-020, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SENSORS-ENVIRONMENT-CLIMATE-GAS-SENSOR-SHARED-F3-VALIDATION-NOMINAL`
- `TEST-CMP-SENSORS-ENVIRONMENT-CLIMATE-GAS-SENSOR-SHARED-F3-VALIDATION-BOUNDARY`
- `TEST-CMP-SENSORS-ENVIRONMENT-CLIMATE-GAS-SENSOR-SHARED-F3-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SENSORS-ENVIRONMENT-CLIMATE-GAS-SENSOR-SHARED-F3-VALIDATION` exists and is limited to shared family scope across `var-sensors-environment-climate-gas-sensor-humidity`, `var-sensors-environment-climate-gas-sensor-barometric`, `var-sensors-environment-climate-gas-sensor-carbon-dioxide`, `var-sensors-environment-climate-gas-sensor-voc`, `var-sensors-environment-climate-gas-sensor-generic-gas`, `F3`, and `VALIDATION`.
