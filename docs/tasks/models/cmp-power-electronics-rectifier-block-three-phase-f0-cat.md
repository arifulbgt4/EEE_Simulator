# CMP-POWER-ELECTRONICS-RECTIFIER-BLOCK-THREE-PHASE-F0-CAT - Specify Three Phase catalog preset
...
| Variant | `var-power-electronics-rectifier-block-three-phase` - Three Phase |
| Fidelity | F0 |
| Concern | CAT |
| Release | R7 |
...
Create the complete, immutable catalog definition for **Three Phase** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-power-electronics-rectifier-block` (Rectifier block); task scope: variant `var-power-electronics-rectifier-block-three-phase` (Three Phase); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Rectifier block family-specific implementation reference](../../catalog/families/fam-power-electronics-rectifier-block.md#family-specific-implementation-reference), registry row `fam-power-electronics-rectifier-block`, and variant `var-power-electronics-rectifier-block-three-phase` (Three Phase).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: The block expands to the selected diode/controlled-switch topology and preserves its explicit branch equations, conduction order, and DC/ripple outputs.
...
- `docs/tasks/models/cmp-power-electronics-rectifier-block-three-phase-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-power-electronics-rectifier-block-three-phase` (Three Phase), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-POWER-ELECTRONICS-RECTIFIER-BLOCK-THREE-PHASE-F0-CAT`: variant `var-power-electronics-rectifier-block-three-phase` (Three Phase), fidelity `F0`, concern `CAT`, and requirements REQ-009, REQ-010, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-POWER-ELECTRONICS-RECTIFIER-BLOCK-THREE-PHASE-F0-CAT-NOMINAL`
- `TEST-CMP-POWER-ELECTRONICS-RECTIFIER-BLOCK-THREE-PHASE-F0-CAT-BOUNDARY`
- `TEST-CMP-POWER-ELECTRONICS-RECTIFIER-BLOCK-THREE-PHASE-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-POWER-ELECTRONICS-RECTIFIER-BLOCK-THREE-PHASE-F0-CAT` exists and is limited to variant `var-power-electronics-rectifier-block-three-phase` (Three Phase), `F0`, and `CAT`.
