# CMP-COMPUTER-GPU-SYSTEM-BUS-PERIPHERAL-ADDRESS-BUS-F0-CAT - Specify Address Bus catalog preset
...
| Variant | `var-computer-gpu-system-bus-peripheral-address-bus` - Address Bus |
| Fidelity | F0 |
| Concern | CAT |
| Release | R8 |
...
Create the complete, immutable catalog definition for **Address Bus** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-computer-gpu-system-bus-peripheral` (System bus and peripheral); task scope: variant `var-computer-gpu-system-bus-peripheral-address-bus` (Address Bus); fidelity `F0`; concern `CAT`.
...
- `docs/tasks/models/cmp-computer-gpu-system-bus-peripheral-address-bus-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-computer-gpu-system-bus-peripheral-address-bus` (Address Bus), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-COMPUTER-GPU-SYSTEM-BUS-PERIPHERAL-ADDRESS-BUS-F0-CAT`: variant `var-computer-gpu-system-bus-peripheral-address-bus` (Address Bus), fidelity `F0`, concern `CAT`, and requirements REQ-025, REQ-027, REQ-028, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-COMPUTER-GPU-SYSTEM-BUS-PERIPHERAL-ADDRESS-BUS-F0-CAT-NOMINAL`
- `TEST-CMP-COMPUTER-GPU-SYSTEM-BUS-PERIPHERAL-ADDRESS-BUS-F0-CAT-BOUNDARY`
- `TEST-CMP-COMPUTER-GPU-SYSTEM-BUS-PERIPHERAL-ADDRESS-BUS-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-COMPUTER-GPU-SYSTEM-BUS-PERIPHERAL-ADDRESS-BUS-F0-CAT` exists and is limited to variant `var-computer-gpu-system-bus-peripheral-address-bus` (Address Bus), `F0`, and `CAT`.
