# CMP-INSTRUMENTS-BENCH-SUPPLY-SHARED-F0-MODEL - Implement Bench supply F0 model
...
Implement one model tier, **F0**, for **Bench supply** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-instruments-bench-supply` (Bench supply); task scope: shared family scope across `var-instruments-bench-supply-single-output`, `var-instruments-bench-supply-triple-output`; fidelity `F0`; concern `MODEL`.
...
- Exact F0 execution rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The output follows CV/CC setpoints and transitions with declared source impedance, current limit, ramp, protection, and channel coupling.
...
- `docs/tasks/models/cmp-instruments-bench-supply-shared-f0-model.md`
...
- One `ModelBinding` for `fam-instruments-bench-supply` at `F0` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-INSTRUMENTS-BENCH-SUPPLY-SHARED-F0-MODEL`: shared family scope across `var-instruments-bench-supply-single-output`, `var-instruments-bench-supply-triple-output`, fidelity `F0`, concern `MODEL`, and requirements REQ-007, REQ-021, REQ-022, REQ-023, REQ-034, REQ-037.
...
- `TEST-CMP-INSTRUMENTS-BENCH-SUPPLY-SHARED-F0-MODEL-NOMINAL`
- `TEST-CMP-INSTRUMENTS-BENCH-SUPPLY-SHARED-F0-MODEL-BOUNDARY`
- `TEST-CMP-INSTRUMENTS-BENCH-SUPPLY-SHARED-F0-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-INSTRUMENTS-BENCH-SUPPLY-SHARED-F0-MODEL` exists and is limited to shared family scope across `var-instruments-bench-supply-single-output`, `var-instruments-bench-supply-triple-output`, `F0`, and `MODEL`.
