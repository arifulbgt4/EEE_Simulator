# CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F1-VALIDATION - Validate Digital pattern source F1
...
Produce independent validation evidence for **Digital pattern source** at **F1** without modifying the model under test.
...
- Stable family: `fam-sources-loads-digital-pattern-source` (Digital pattern source); task scope: shared family scope across `var-sources-loads-digital-pattern-source-pattern`; fidelity `F1`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F1 rule: Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: The source emits the declared 0/1/X/Z vector at integer ticks; simultaneous changes use the scheduler tie order and never depend on host scheduling.
...
- `docs/tasks/models/cmp-sources-loads-digital-pattern-source-shared-f1-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SRC-DIGITAL_PATTERN_SOURCE-NOMINAL`, `GOLD-SRC-DIGITAL_PATTERN_SOURCE-BOUNDARY`, `GOLD-SRC-DIGITAL_PATTERN_SOURCE-FAILURE` at `F1`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F1-VALIDATION`: shared family scope across `var-sources-loads-digital-pattern-source-pattern`, fidelity `F1`, concern `VALIDATION`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F1-VALIDATION-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F1-VALIDATION-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F1-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F1-VALIDATION` exists and is limited to shared family scope across `var-sources-loads-digital-pattern-source-pattern`, `F1`, and `VALIDATION`.
