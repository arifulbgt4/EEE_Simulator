# CMP-DIGITAL-LOGIC-LATCH-SHARED-F0-VALIDATION - Validate Latch F0
...
Produce independent validation evidence for **Latch** at **F0** without modifying the model under test.
...
- Stable family: `fam-digital-logic-latch` (Latch); task scope: shared family scope across `var-digital-logic-latch-sr`, `var-digital-logic-latch-d`, `var-digital-logic-latch-gated`; fidelity `F0`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F0 rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The latch updates stored state while enable is active and holds it otherwise, with explicit illegal SR, delay, setup/hold, and metastability diagnostics.
...
- `docs/tasks/models/cmp-digital-logic-latch-shared-f0-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-DIG-LATCH-NOMINAL`, `GOLD-DIG-LATCH-BOUNDARY`, `GOLD-DIG-LATCH-FAILURE` at `F0`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-DIGITAL-LOGIC-LATCH-SHARED-F0-VALIDATION`: shared family scope across `var-digital-logic-latch-sr`, `var-digital-logic-latch-d`, `var-digital-logic-latch-gated`, fidelity `F0`, concern `VALIDATION`, and requirements REQ-018, REQ-021, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-DIGITAL-LOGIC-LATCH-SHARED-F0-VALIDATION-NOMINAL`
- `TEST-CMP-DIGITAL-LOGIC-LATCH-SHARED-F0-VALIDATION-BOUNDARY`
- `TEST-CMP-DIGITAL-LOGIC-LATCH-SHARED-F0-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-DIGITAL-LOGIC-LATCH-SHARED-F0-VALIDATION` exists and is limited to shared family scope across `var-digital-logic-latch-sr`, `var-digital-logic-latch-d`, `var-digital-logic-latch-gated`, `F0`, and `VALIDATION`.
