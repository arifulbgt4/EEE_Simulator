# CMP-ACTUATORS-HMI-SERVO-SHARED-F2-MODEL - Implement Servo F2 model
...
Implement one model tier, **F2**, for **Servo** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-actuators-hmi-servo` (Servo); task scope: shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`; fidelity `F2`; concern `MODEL`.
...
- Exact F2 execution rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: Command pulse/position drives a bounded closed-loop angle state with declared range, speed, dead band, torque/load, current, and stall behavior.
...
- `docs/tasks/models/cmp-actuators-hmi-servo-shared-f2-model.md`
...
- One `ModelBinding` for `fam-actuators-hmi-servo` at `F2` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-ACTUATORS-HMI-SERVO-SHARED-F2-MODEL`: shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`, fidelity `F2`, concern `MODEL`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F2-MODEL-NOMINAL`
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F2-MODEL-BOUNDARY`
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F2-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-ACTUATORS-HMI-SERVO-SHARED-F2-MODEL` exists and is limited to shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`, `F2`, and `MODEL`.
