# CMP-POWER-ELECTRONICS-DC-CONVERTER-CHARGE-PUMP-F0-CAT - Specify Charge Pump catalog preset
...
| Variant | `var-power-electronics-dc-converter-charge-pump` - Charge Pump |
| Fidelity | F0 |
| Concern | CAT |
| Release | R7 |
...
Create the complete, immutable catalog definition for **Charge Pump** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-power-electronics-dc-converter` (DC converter); task scope: variant `var-power-electronics-dc-converter-charge-pump` (Charge Pump); fidelity `F0`; concern `CAT`.
...
- Canonical source: [DC converter family-specific implementation reference](../../catalog/families/fam-power-electronics-dc-converter.md#family-specific-implementation-reference), registry row `fam-power-electronics-dc-converter`, and variant `var-power-electronics-dc-converter-charge-pump` (Charge Pump).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: The selected buck/boost/buck-boost/flyback/forward/charge-pump profile uses either an explicit switching topology or declared averaged state-space equations.
...
- `docs/tasks/models/cmp-power-electronics-dc-converter-charge-pump-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-power-electronics-dc-converter-charge-pump` (Charge Pump), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-POWER-ELECTRONICS-DC-CONVERTER-CHARGE-PUMP-F0-CAT`: variant `var-power-electronics-dc-converter-charge-pump` (Charge Pump), fidelity `F0`, concern `CAT`, and requirements REQ-009, REQ-010, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-POWER-ELECTRONICS-DC-CONVERTER-CHARGE-PUMP-F0-CAT-NOMINAL`
- `TEST-CMP-POWER-ELECTRONICS-DC-CONVERTER-CHARGE-PUMP-F0-CAT-BOUNDARY`
- `TEST-CMP-POWER-ELECTRONICS-DC-CONVERTER-CHARGE-PUMP-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-POWER-ELECTRONICS-DC-CONVERTER-CHARGE-PUMP-F0-CAT` exists and is limited to variant `var-power-electronics-dc-converter-charge-pump` (Charge Pump), `F0`, and `CAT`.
