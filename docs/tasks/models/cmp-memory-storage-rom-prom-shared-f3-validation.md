# CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F3-VALIDATION - Validate ROM and PROM F3
...
Produce independent validation evidence for **ROM and PROM** at **F3** without modifying the model under test.
...
- Stable family: `fam-memory-storage-rom-prom` (ROM and PROM); task scope: shared family scope across `var-memory-storage-rom-prom-mask-rom`, `var-memory-storage-rom-prom-prom`, `var-memory-storage-rom-prom-eprom`; fidelity `F3`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F3 rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Address selects an immutable/programmed word; range, enable, output delay, initialization, and unknown-address behavior are explicit.
...
- `docs/tasks/models/cmp-memory-storage-rom-prom-shared-f3-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-MEM-ROM_PROM-NOMINAL`, `GOLD-MEM-ROM_PROM-BOUNDARY`, `GOLD-MEM-ROM_PROM-FAILURE` at `F3`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F3-VALIDATION`: shared family scope across `var-memory-storage-rom-prom-mask-rom`, `var-memory-storage-rom-prom-prom`, `var-memory-storage-rom-prom-eprom`, fidelity `F3`, concern `VALIDATION`, and requirements REQ-018, REQ-021, REQ-025, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F3-VALIDATION-NOMINAL`
- `TEST-CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F3-VALIDATION-BOUNDARY`
- `TEST-CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F3-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F3-VALIDATION` exists and is limited to shared family scope across `var-memory-storage-rom-prom-mask-rom`, `var-memory-storage-rom-prom-prom`, `var-memory-storage-rom-prom-eprom`, `F3`, and `VALIDATION`.
