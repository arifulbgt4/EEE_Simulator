# CMP-MEMORY-STORAGE-DRAM-SHARED-F3-VALIDATION - Validate Dynamic RAM F3
...
Produce independent validation evidence for **Dynamic RAM** at **F3** without modifying the model under test.
...
- Stable family: `fam-memory-storage-dram` (Dynamic RAM); task scope: shared family scope across `var-memory-storage-dram-sdram`, `var-memory-storage-dram-ddr`, `var-memory-storage-dram-mobile-dram`; fidelity `F3`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F3 rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: A bank/row/column state model enforces activate/read/write/precharge/refresh timing, retention, initialization, and conflict rules.
...
- `docs/tasks/models/cmp-memory-storage-dram-shared-f3-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-MEM-DRAM-NOMINAL`, `GOLD-MEM-DRAM-BOUNDARY`, `GOLD-MEM-DRAM-FAILURE` at `F3`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-MEMORY-STORAGE-DRAM-SHARED-F3-VALIDATION`: shared family scope across `var-memory-storage-dram-sdram`, `var-memory-storage-dram-ddr`, `var-memory-storage-dram-mobile-dram`, fidelity `F3`, concern `VALIDATION`, and requirements REQ-018, REQ-021, REQ-025, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MEMORY-STORAGE-DRAM-SHARED-F3-VALIDATION-NOMINAL`
- `TEST-CMP-MEMORY-STORAGE-DRAM-SHARED-F3-VALIDATION-BOUNDARY`
- `TEST-CMP-MEMORY-STORAGE-DRAM-SHARED-F3-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-MEMORY-STORAGE-DRAM-SHARED-F3-VALIDATION` exists and is limited to shared family scope across `var-memory-storage-dram-sdram`, `var-memory-storage-dram-ddr`, `var-memory-storage-dram-mobile-dram`, `F3`, and `VALIDATION`.
