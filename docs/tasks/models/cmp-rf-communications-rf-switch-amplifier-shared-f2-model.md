# CMP-RF-COMMUNICATIONS-RF-SWITCH-AMPLIFIER-SHARED-F2-MODEL - Implement RF switch and amplifier F2 model
...
Implement one model tier, **F2**, for **RF switch and amplifier** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-rf-communications-rf-switch-amplifier` (RF switch and amplifier); task scope: shared family scope across `var-rf-communications-rf-switch-amplifier-rf-switch`, `var-rf-communications-rf-switch-amplifier-lna`, `var-rf-communications-rf-switch-amplifier-power-amplifier`; fidelity `F2`; concern `MODEL`.
...
- Exact F2 execution rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: The selected state maps ports through declared gain/loss, match, noise, compression, isolation, and switching behavior over frequency and bias.
...
- `docs/tasks/models/cmp-rf-communications-rf-switch-amplifier-shared-f2-model.md`
...
- One `ModelBinding` for `fam-rf-communications-rf-switch-amplifier` at `F2` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-RF-COMMUNICATIONS-RF-SWITCH-AMPLIFIER-SHARED-F2-MODEL`: shared family scope across `var-rf-communications-rf-switch-amplifier-rf-switch`, `var-rf-communications-rf-switch-amplifier-lna`, `var-rf-communications-rf-switch-amplifier-power-amplifier`, fidelity `F2`, concern `MODEL`, and requirements REQ-011, REQ-012, REQ-024, REQ-028, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-RF-COMMUNICATIONS-RF-SWITCH-AMPLIFIER-SHARED-F2-MODEL-NOMINAL`
- `TEST-CMP-RF-COMMUNICATIONS-RF-SWITCH-AMPLIFIER-SHARED-F2-MODEL-BOUNDARY`
- `TEST-CMP-RF-COMMUNICATIONS-RF-SWITCH-AMPLIFIER-SHARED-F2-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-RF-COMMUNICATIONS-RF-SWITCH-AMPLIFIER-SHARED-F2-MODEL` exists and is limited to shared family scope across `var-rf-communications-rf-switch-amplifier-rf-switch`, `var-rf-communications-rf-switch-amplifier-lna`, `var-rf-communications-rf-switch-amplifier-power-amplifier`, `F2`, and `MODEL`.
