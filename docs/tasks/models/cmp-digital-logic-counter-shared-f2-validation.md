# CMP-DIGITAL-LOGIC-COUNTER-SHARED-F2-VALIDATION - Validate Counter F2
...
Produce independent validation evidence for **Counter** at **F2** without modifying the model under test.
...
- Stable family: `fam-digital-logic-counter` (Counter); task scope: shared family scope across `var-digital-logic-counter-ripple`, `var-digital-logic-counter-synchronous`, `var-digital-logic-counter-up-down`; fidelity `F2`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F2 rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: A bounded state value advances, decrements, loads, or resets on declared events with modulus, carry/borrow, enable, and timing rules.
...
- `docs/tasks/models/cmp-digital-logic-counter-shared-f2-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-DIG-COUNTER-NOMINAL`, `GOLD-DIG-COUNTER-BOUNDARY`, `GOLD-DIG-COUNTER-FAILURE` at `F2`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-DIGITAL-LOGIC-COUNTER-SHARED-F2-VALIDATION`: shared family scope across `var-digital-logic-counter-ripple`, `var-digital-logic-counter-synchronous`, `var-digital-logic-counter-up-down`, fidelity `F2`, concern `VALIDATION`, and requirements REQ-018, REQ-021, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-DIGITAL-LOGIC-COUNTER-SHARED-F2-VALIDATION-NOMINAL`
- `TEST-CMP-DIGITAL-LOGIC-COUNTER-SHARED-F2-VALIDATION-BOUNDARY`
- `TEST-CMP-DIGITAL-LOGIC-COUNTER-SHARED-F2-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-DIGITAL-LOGIC-COUNTER-SHARED-F2-VALIDATION` exists and is limited to shared family scope across `var-digital-logic-counter-ripple`, `var-digital-logic-counter-synchronous`, `var-digital-logic-counter-up-down`, `F2`, and `VALIDATION`.
