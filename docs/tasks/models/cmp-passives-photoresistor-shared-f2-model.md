# CMP-PASSIVES-PHOTORESISTOR-SHARED-F2-MODEL - Implement Photoresistor F2 model
...
Implement one model tier, **F2**, for **Photoresistor** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-passives-photoresistor` (Photoresistor); task scope: shared family scope across `var-passives-photoresistor-ldr`; fidelity `F2`; concern `MODEL`.
...
- `docs/tasks/models/cmp-passives-photoresistor-shared-f2-model.md`
...
- One `ModelBinding` for `fam-passives-photoresistor` at `F2` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-PASSIVES-PHOTORESISTOR-SHARED-F2-MODEL`: shared family scope across `var-passives-photoresistor-ldr`, fidelity `F2`, concern `MODEL`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-PHOTORESISTOR-SHARED-F2-MODEL-NOMINAL`
- `TEST-CMP-PASSIVES-PHOTORESISTOR-SHARED-F2-MODEL-BOUNDARY`
- `TEST-CMP-PASSIVES-PHOTORESISTOR-SHARED-F2-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-PHOTORESISTOR-SHARED-F2-MODEL` exists and is limited to shared family scope across `var-passives-photoresistor-ldr`, `F2`, and `MODEL`.
