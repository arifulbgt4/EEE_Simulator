# CMP-PASSIVES-FERRITE-SHARED-F0-VALIDATION - Validate Ferrite suppressor F0
...
Produce independent validation evidence for **Ferrite suppressor** at **F0** without modifying the model under test.
...
- Stable family: `fam-passives-ferrite` (Ferrite suppressor); task scope: shared family scope across `var-passives-ferrite-bead`; fidelity `F0`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F0 rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The branch uses the declared frequency-dependent complex impedance Z(f,T,bias); time-domain use requires an approved causal fitted model.
...
- `docs/tasks/models/cmp-passives-ferrite-shared-f0-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-PAS-FERRITE-NOMINAL`, `GOLD-PAS-FERRITE-BOUNDARY`, `GOLD-PAS-FERRITE-FAILURE` at `F0`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-PASSIVES-FERRITE-SHARED-F0-VALIDATION`: shared family scope across `var-passives-ferrite-bead`, fidelity `F0`, concern `VALIDATION`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-FERRITE-SHARED-F0-VALIDATION-NOMINAL`
- `TEST-CMP-PASSIVES-FERRITE-SHARED-F0-VALIDATION-BOUNDARY`
- `TEST-CMP-PASSIVES-FERRITE-SHARED-F0-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-FERRITE-SHARED-F0-VALIDATION` exists and is limited to shared family scope across `var-passives-ferrite-bead`, `F0`, and `VALIDATION`.
