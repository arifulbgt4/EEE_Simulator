# CMP-SEMICONDUCTORS-BJT-SHARED-F3-MODEL - Implement Bipolar junction transistor F3 model
...
Implement one model tier, **F3**, for **Bipolar junction transistor** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-semiconductors-bjt` (Bipolar junction transistor); task scope: shared family scope across `var-semiconductors-bjt-npn`, `var-semiconductors-bjt-pnp`, `var-semiconductors-bjt-darlington-npn`, `var-semiconductors-bjt-darlington-pnp`; fidelity `F3`; concern `MODEL`.
...
- Exact F3 execution rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: F1 uses a documented Ebers-Moll-style junction/current-gain relation with NPN/PNP polarity; F3 binds a pinned compact model and parameter envelope.
...
- `docs/tasks/models/cmp-semiconductors-bjt-shared-f3-model.md`
...
- One `ModelBinding` for `fam-semiconductors-bjt` at `F3` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-SEMICONDUCTORS-BJT-SHARED-F3-MODEL`: shared family scope across `var-semiconductors-bjt-npn`, `var-semiconductors-bjt-pnp`, `var-semiconductors-bjt-darlington-npn`, `var-semiconductors-bjt-darlington-pnp`, fidelity `F3`, concern `MODEL`, and requirements REQ-009, REQ-010, REQ-011, REQ-012, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SEMICONDUCTORS-BJT-SHARED-F3-MODEL-NOMINAL`
- `TEST-CMP-SEMICONDUCTORS-BJT-SHARED-F3-MODEL-BOUNDARY`
- `TEST-CMP-SEMICONDUCTORS-BJT-SHARED-F3-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-SEMICONDUCTORS-BJT-SHARED-F3-MODEL` exists and is limited to shared family scope across `var-semiconductors-bjt-npn`, `var-semiconductors-bjt-pnp`, `var-semiconductors-bjt-darlington-npn`, `var-semiconductors-bjt-darlington-pnp`, `F3`, and `MODEL`.
