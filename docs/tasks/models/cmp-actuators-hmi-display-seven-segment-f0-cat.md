# CMP-ACTUATORS-HMI-DISPLAY-SEVEN-SEGMENT-F0-CAT - Specify Seven Segment catalog preset
...
| Variant | `var-actuators-hmi-display-seven-segment` - Seven Segment |
| Fidelity | F0 |
| Concern | CAT |
| Release | R7 |
...
Create the complete, immutable catalog definition for **Seven Segment** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-actuators-hmi-display` (Display); task scope: variant `var-actuators-hmi-display-seven-segment` (Seven Segment); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Display family-specific implementation reference](../../catalog/families/fam-actuators-hmi-display.md#family-specific-implementation-reference), registry row `fam-actuators-hmi-display`, and variant `var-actuators-hmi-display-seven-segment` (Seven Segment).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Electrical/digital inputs update the selected segment/pixel state with declared addressing, refresh, luminance, timing, power, and persistence.
...
- `docs/tasks/models/cmp-actuators-hmi-display-seven-segment-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-actuators-hmi-display-seven-segment` (Seven Segment), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-ACTUATORS-HMI-DISPLAY-SEVEN-SEGMENT-F0-CAT`: variant `var-actuators-hmi-display-seven-segment` (Seven Segment), fidelity `F0`, concern `CAT`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ACTUATORS-HMI-DISPLAY-SEVEN-SEGMENT-F0-CAT-NOMINAL`
- `TEST-CMP-ACTUATORS-HMI-DISPLAY-SEVEN-SEGMENT-F0-CAT-BOUNDARY`
- `TEST-CMP-ACTUATORS-HMI-DISPLAY-SEVEN-SEGMENT-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-ACTUATORS-HMI-DISPLAY-SEVEN-SEGMENT-F0-CAT` exists and is limited to variant `var-actuators-hmi-display-seven-segment` (Seven Segment), `F0`, and `CAT`.
