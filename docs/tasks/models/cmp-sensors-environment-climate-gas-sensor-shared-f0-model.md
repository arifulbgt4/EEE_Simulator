# CMP-SENSORS-ENVIRONMENT-CLIMATE-GAS-SENSOR-SHARED-F0-MODEL - Implement Climate and gas sensor F0 model
...
Implement one model tier, **F0**, for **Climate and gas sensor** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-sensors-environment-climate-gas-sensor` (Climate and gas sensor); task scope: shared family scope across `var-sensors-environment-climate-gas-sensor-humidity`, `var-sensors-environment-climate-gas-sensor-barometric`, `var-sensors-environment-climate-gas-sensor-carbon-dioxide`, `var-sensors-environment-climate-gas-sensor-voc`, `var-sensors-environment-climate-gas-sensor-generic-gas`; fidelity `F0`; concern `MODEL`.
...
- Exact F0 execution rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Output follows the declared humidity/gas concentration transfer with warm-up, cross-sensitivity, saturation, response/recovery, and environmental terms.
...
- `docs/tasks/models/cmp-sensors-environment-climate-gas-sensor-shared-f0-model.md`
...
- One `ModelBinding` for `fam-sensors-environment-climate-gas-sensor` at `F0` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-SENSORS-ENVIRONMENT-CLIMATE-GAS-SENSOR-SHARED-F0-MODEL`: shared family scope across `var-sensors-environment-climate-gas-sensor-humidity`, `var-sensors-environment-climate-gas-sensor-barometric`, `var-sensors-environment-climate-gas-sensor-carbon-dioxide`, `var-sensors-environment-climate-gas-sensor-voc`, `var-sensors-environment-climate-gas-sensor-generic-gas`, fidelity `F0`, concern `MODEL`, and requirements REQ-014, REQ-015, REQ-018, REQ-020, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SENSORS-ENVIRONMENT-CLIMATE-GAS-SENSOR-SHARED-F0-MODEL-NOMINAL`
- `TEST-CMP-SENSORS-ENVIRONMENT-CLIMATE-GAS-SENSOR-SHARED-F0-MODEL-BOUNDARY`
- `TEST-CMP-SENSORS-ENVIRONMENT-CLIMATE-GAS-SENSOR-SHARED-F0-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-SENSORS-ENVIRONMENT-CLIMATE-GAS-SENSOR-SHARED-F0-MODEL` exists and is limited to shared family scope across `var-sensors-environment-climate-gas-sensor-humidity`, `var-sensors-environment-climate-gas-sensor-barometric`, `var-sensors-environment-climate-gas-sensor-carbon-dioxide`, `var-sensors-environment-climate-gas-sensor-voc`, `var-sensors-environment-climate-gas-sensor-generic-gas`, `F0`, and `MODEL`.
