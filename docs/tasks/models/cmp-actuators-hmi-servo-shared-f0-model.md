# CMP-ACTUATORS-HMI-SERVO-SHARED-F0-MODEL - Implement Servo F0 model
...
Implement one model tier, **F0**, for **Servo** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-actuators-hmi-servo` (Servo); task scope: shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`; fidelity `F0`; concern `MODEL`.
...
- Exact F0 execution rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Command pulse/position drives a bounded closed-loop angle state with declared range, speed, dead band, torque/load, current, and stall behavior.
...
- `docs/tasks/models/cmp-actuators-hmi-servo-shared-f0-model.md`
...
- One `ModelBinding` for `fam-actuators-hmi-servo` at `F0` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-ACTUATORS-HMI-SERVO-SHARED-F0-MODEL`: shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`, fidelity `F0`, concern `MODEL`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F0-MODEL-NOMINAL`
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F0-MODEL-BOUNDARY`
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F0-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-ACTUATORS-HMI-SERVO-SHARED-F0-MODEL` exists and is limited to shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`, `F0`, and `MODEL`.
