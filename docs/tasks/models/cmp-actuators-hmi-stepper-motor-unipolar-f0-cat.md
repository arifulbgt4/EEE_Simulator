# CMP-ACTUATORS-HMI-STEPPER-MOTOR-UNIPOLAR-F0-CAT - Specify Unipolar catalog preset
...
| Variant | `var-actuators-hmi-stepper-motor-unipolar` - Unipolar |
| Fidelity | F0 |
| Concern | CAT |
| Release | R7 |
...
Create the complete, immutable catalog definition for **Unipolar** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-actuators-hmi-stepper-motor` (Stepper motor); task scope: variant `var-actuators-hmi-stepper-motor-unipolar` (Unipolar); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Stepper motor family-specific implementation reference](../../catalog/families/fam-actuators-hmi-stepper-motor.md#family-specific-implementation-reference), registry row `fam-actuators-hmi-stepper-motor`, and variant `var-actuators-hmi-stepper-motor-unipolar` (Unipolar).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Phase-current and rotor angle/speed states follow the declared step/torque/detent/inertia model with missed-step and limit behavior.
...
- `docs/tasks/models/cmp-actuators-hmi-stepper-motor-unipolar-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-actuators-hmi-stepper-motor-unipolar` (Unipolar), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-ACTUATORS-HMI-STEPPER-MOTOR-UNIPOLAR-F0-CAT`: variant `var-actuators-hmi-stepper-motor-unipolar` (Unipolar), fidelity `F0`, concern `CAT`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ACTUATORS-HMI-STEPPER-MOTOR-UNIPOLAR-F0-CAT-NOMINAL`
- `TEST-CMP-ACTUATORS-HMI-STEPPER-MOTOR-UNIPOLAR-F0-CAT-BOUNDARY`
- `TEST-CMP-ACTUATORS-HMI-STEPPER-MOTOR-UNIPOLAR-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-ACTUATORS-HMI-STEPPER-MOTOR-UNIPOLAR-F0-CAT` exists and is limited to variant `var-actuators-hmi-stepper-motor-unipolar` (Unipolar), `F0`, and `CAT`.
