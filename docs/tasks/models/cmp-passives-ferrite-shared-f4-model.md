# CMP-PASSIVES-FERRITE-SHARED-F4-MODEL - Implement Ferrite suppressor F4 model
...
Implement one model tier, **F4**, for **Ferrite suppressor** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-passives-ferrite` (Ferrite suppressor); task scope: shared family scope across `var-passives-ferrite-bead`; fidelity `F4`; concern `MODEL`.
...
- `docs/tasks/models/cmp-passives-ferrite-shared-f4-model.md`
...
- One `ModelBinding` for `fam-passives-ferrite` at `F4` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-PASSIVES-FERRITE-SHARED-F4-MODEL`: shared family scope across `var-passives-ferrite-bead`, fidelity `F4`, concern `MODEL`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-FERRITE-SHARED-F4-MODEL-NOMINAL`
- `TEST-CMP-PASSIVES-FERRITE-SHARED-F4-MODEL-BOUNDARY`
- `TEST-CMP-PASSIVES-FERRITE-SHARED-F4-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-FERRITE-SHARED-F4-MODEL` exists and is limited to shared family scope across `var-passives-ferrite-bead`, `F4`, and `MODEL`.
