# CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F0-MODEL - Implement ROM and PROM F0 model
...
Implement one model tier, **F0**, for **ROM and PROM** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-memory-storage-rom-prom` (ROM and PROM); task scope: shared family scope across `var-memory-storage-rom-prom-mask-rom`, `var-memory-storage-rom-prom-prom`, `var-memory-storage-rom-prom-eprom`; fidelity `F0`; concern `MODEL`.
...
- Exact F0 execution rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Address selects an immutable/programmed word; range, enable, output delay, initialization, and unknown-address behavior are explicit.
...
- `docs/tasks/models/cmp-memory-storage-rom-prom-shared-f0-model.md`
...
- One `ModelBinding` for `fam-memory-storage-rom-prom` at `F0` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F0-MODEL`: shared family scope across `var-memory-storage-rom-prom-mask-rom`, `var-memory-storage-rom-prom-prom`, `var-memory-storage-rom-prom-eprom`, fidelity `F0`, concern `MODEL`, and requirements REQ-018, REQ-021, REQ-025, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F0-MODEL-NOMINAL`
- `TEST-CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F0-MODEL-BOUNDARY`
- `TEST-CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F0-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-MEMORY-STORAGE-ROM-PROM-SHARED-F0-MODEL` exists and is limited to shared family scope across `var-memory-storage-rom-prom-mask-rom`, `var-memory-storage-rom-prom-prom`, `var-memory-storage-rom-prom-eprom`, `F0`, and `MODEL`.
