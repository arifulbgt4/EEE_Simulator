# CMP-RF-COMMUNICATIONS-ANTENNA-MONOPOLE-F1-CAT - Specify Monopole catalog preset
...
| Variant | `var-rf-communications-antenna-monopole` - Monopole |
| Fidelity | F1 |
| Concern | CAT |
| Release | R7 |
...
Create the complete, immutable catalog definition for **Monopole** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-rf-communications-antenna` (Antenna); task scope: variant `var-rf-communications-antenna-monopole` (Monopole); fidelity `F1`; concern `CAT`.
...
- Canonical source: [Antenna family-specific implementation reference](../../catalog/families/fam-rf-communications-antenna.md#family-specific-implementation-reference), registry row `fam-rf-communications-antenna`, and variant `var-rf-communications-antenna-monopole` (Monopole).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: The antenna uses a declared port impedance/S-parameter plus radiation gain/pattern/polarization/efficiency profile over its validated frequency envelope.
...
- `docs/tasks/models/cmp-rf-communications-antenna-monopole-f1-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-rf-communications-antenna-monopole` (Monopole), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-RF-COMMUNICATIONS-ANTENNA-MONOPOLE-F1-CAT`: variant `var-rf-communications-antenna-monopole` (Monopole), fidelity `F1`, concern `CAT`, and requirements REQ-011, REQ-012, REQ-024, REQ-028, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-RF-COMMUNICATIONS-ANTENNA-MONOPOLE-F1-CAT-NOMINAL`
- `TEST-CMP-RF-COMMUNICATIONS-ANTENNA-MONOPOLE-F1-CAT-BOUNDARY`
- `TEST-CMP-RF-COMMUNICATIONS-ANTENNA-MONOPOLE-F1-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-RF-COMMUNICATIONS-ANTENNA-MONOPOLE-F1-CAT` exists and is limited to variant `var-rf-communications-antenna-monopole` (Monopole), `F1`, and `CAT`.
