# CMP-CONNECTIVITY-WIRE-SHARED-F0-MODEL - Implement Wire and conductor F0 model
...
Implement one model tier, **F0**, for **Wire and conductor** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-connectivity-wire` (Wire and conductor); task scope: shared family scope across `var-connectivity-wire-conductor`; fidelity `F0`; concern `MODEL`.
...
- Exact F0 execution rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Ideal connectivity enforces v(P) - v(N) = 0; any F1 non-ideal profile replaces the ideal branch with an explicitly parameterized series impedance.
...
- `docs/tasks/models/cmp-connectivity-wire-shared-f0-model.md`
...
- One `ModelBinding` for `fam-connectivity-wire` at `F0` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-CONNECTIVITY-WIRE-SHARED-F0-MODEL`: shared family scope across `var-connectivity-wire-conductor`, fidelity `F0`, concern `MODEL`, and requirements REQ-004, REQ-005, REQ-008, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-CONNECTIVITY-WIRE-SHARED-F0-MODEL-NOMINAL`
- `TEST-CMP-CONNECTIVITY-WIRE-SHARED-F0-MODEL-BOUNDARY`
- `TEST-CMP-CONNECTIVITY-WIRE-SHARED-F0-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-CONNECTIVITY-WIRE-SHARED-F0-MODEL` exists and is limited to shared family scope across `var-connectivity-wire-conductor`, `F0`, and `MODEL`.
