...
- Stable family: `fam-instruments-bench-supply` (Bench supply); task scope: shared family scope across `var-instruments-bench-supply-single-output`, `var-instruments-bench-supply-triple-output`; fidelity `F0`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F0 rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The output follows CV/CC setpoints and transitions with declared source impedance, current limit, ramp, protection, and channel coupling.
...
- `docs/tasks/models/cmp-instruments-bench-supply-shared-f0-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-INS-BENCH_SUPPLY-NOMINAL`, `GOLD-INS-BENCH_SUPPLY-BOUNDARY`, `GOLD-INS-BENCH_SUPPLY-FAILURE` at `F0`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-INSTRUMENTS-BENCH-SUPPLY-SHARED-F0-VALIDATION`: shared family scope across `var-instruments-bench-supply-single-output`, `var-instruments-bench-supply-triple-output`, fidelity `F0`, concern `VALIDATION`, and requirements REQ-007, REQ-021, REQ-022, REQ-023, REQ-034, REQ-037.
...
- `TEST-CMP-INSTRUMENTS-BENCH-SUPPLY-SHARED-F0-VALIDATION-NOMINAL`
- `TEST-CMP-INSTRUMENTS-BENCH-SUPPLY-SHARED-F0-VALIDATION-BOUNDARY`
- `TEST-CMP-INSTRUMENTS-BENCH-SUPPLY-SHARED-F0-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-INSTRUMENTS-BENCH-SUPPLY-SHARED-F0-VALIDATION` exists and is limited to shared family scope across `var-instruments-bench-supply-single-output`, `var-instruments-bench-supply-triple-output`, `F0`, and `VALIDATION`.
