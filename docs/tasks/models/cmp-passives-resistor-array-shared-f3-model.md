# CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F3-MODEL - Implement Resistor array F3 model
...
Implement one model tier, **F3**, for **Resistor array** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-passives-resistor-array` (Resistor array); task scope: shared family scope across `var-passives-resistor-array-isolated`, `var-passives-resistor-array-bussed`; fidelity `F3`; concern `MODEL`.
...
- Exact F3 execution rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Each declared resistor branch obeys v_k = R_k*i_k; shared-terminal and matching/correlation rules come only from the selected array topology.
...
- `docs/tasks/models/cmp-passives-resistor-array-shared-f3-model.md`
...
- One `ModelBinding` for `fam-passives-resistor-array` at `F3` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F3-MODEL`: shared family scope across `var-passives-resistor-array-isolated`, `var-passives-resistor-array-bussed`, fidelity `F3`, concern `MODEL`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F3-MODEL-NOMINAL`
- `TEST-CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F3-MODEL-BOUNDARY`
- `TEST-CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F3-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F3-MODEL` exists and is limited to shared family scope across `var-passives-resistor-array-isolated`, `var-passives-resistor-array-bussed`, `F3`, and `MODEL`.
