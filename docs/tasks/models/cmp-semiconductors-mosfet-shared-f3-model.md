# CMP-SEMICONDUCTORS-MOSFET-SHARED-F3-MODEL - Implement MOSFET F3 model
...
Implement one model tier, **F3**, for **MOSFET** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-semiconductors-mosfet` (MOSFET); task scope: shared family scope across `var-semiconductors-mosfet-n-enhancement`, `var-semiconductors-mosfet-p-enhancement`, `var-semiconductors-mosfet-n-depletion`, `var-semiconductors-mosfet-p-depletion`, `var-semiconductors-mosfet-gan-power`, `var-semiconductors-mosfet-sic-power`; fidelity `F3`; concern `MODEL`.
...
- Exact F3 execution rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: F1 uses explicit cutoff/linear/saturation channel equations with body diode; F3 binds a pinned compact model and preserves terminal orientation.
...
- `docs/tasks/models/cmp-semiconductors-mosfet-shared-f3-model.md`
...
- One `ModelBinding` for `fam-semiconductors-mosfet` at `F3` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-SEMICONDUCTORS-MOSFET-SHARED-F3-MODEL`: shared family scope across `var-semiconductors-mosfet-n-enhancement`, `var-semiconductors-mosfet-p-enhancement`, `var-semiconductors-mosfet-n-depletion`, `var-semiconductors-mosfet-p-depletion`, `var-semiconductors-mosfet-gan-power`, `var-semiconductors-mosfet-sic-power`, fidelity `F3`, concern `MODEL`, and requirements REQ-009, REQ-010, REQ-011, REQ-012, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SEMICONDUCTORS-MOSFET-SHARED-F3-MODEL-NOMINAL`
- `TEST-CMP-SEMICONDUCTORS-MOSFET-SHARED-F3-MODEL-BOUNDARY`
- `TEST-CMP-SEMICONDUCTORS-MOSFET-SHARED-F3-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-SEMICONDUCTORS-MOSFET-SHARED-F3-MODEL` exists and is limited to shared family scope across `var-semiconductors-mosfet-n-enhancement`, `var-semiconductors-mosfet-p-enhancement`, `var-semiconductors-mosfet-n-depletion`, `var-semiconductors-mosfet-p-depletion`, `var-semiconductors-mosfet-gan-power`, `var-semiconductors-mosfet-sic-power`, `F3`, and `MODEL`.
