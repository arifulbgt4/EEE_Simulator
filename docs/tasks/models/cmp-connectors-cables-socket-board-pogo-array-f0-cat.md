# CMP-CONNECTORS-CABLES-SOCKET-BOARD-POGO-ARRAY-F0-CAT - Specify Pogo Array catalog preset
...
| Variant | `var-connectors-cables-socket-board-pogo-array` - Pogo Array |
| Fidelity | F0 |
| Concern | CAT |
| Release | R7 |
...
Create the complete, immutable catalog definition for **Pogo Array** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-connectors-cables-socket-board` (Socket and prototyping board); task scope: variant `var-connectors-cables-socket-board-pogo-array` (Pogo Array); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Socket and prototyping board family-specific implementation reference](../../catalog/families/fam-connectors-cables-socket-board.md#family-specific-implementation-reference), registry row `fam-connectors-cables-socket-board`, and variant `var-connectors-cables-socket-board-pogo-array` (Pogo Array).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: The selected socket/breadboard topology defines an explicit contact-connectivity matrix plus declared contact resistance and parasitics.
...
- `docs/tasks/models/cmp-connectors-cables-socket-board-pogo-array-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-connectors-cables-socket-board-pogo-array` (Pogo Array), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-CONNECTORS-CABLES-SOCKET-BOARD-POGO-ARRAY-F0-CAT`: variant `var-connectors-cables-socket-board-pogo-array` (Pogo Array), fidelity `F0`, concern `CAT`, and requirements REQ-005, REQ-014, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-CONNECTORS-CABLES-SOCKET-BOARD-POGO-ARRAY-F0-CAT-NOMINAL`
- `TEST-CMP-CONNECTORS-CABLES-SOCKET-BOARD-POGO-ARRAY-F0-CAT-BOUNDARY`
- `TEST-CMP-CONNECTORS-CABLES-SOCKET-BOARD-POGO-ARRAY-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-CONNECTORS-CABLES-SOCKET-BOARD-POGO-ARRAY-F0-CAT` exists and is limited to variant `var-connectors-cables-socket-board-pogo-array` (Pogo Array), `F0`, and `CAT`.
