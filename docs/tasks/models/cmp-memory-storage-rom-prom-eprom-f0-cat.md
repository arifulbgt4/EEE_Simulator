# CMP-MEMORY-STORAGE-ROM-PROM-EPROM-F0-CAT - Specify Eprom catalog preset
...
| Variant | `var-memory-storage-rom-prom-eprom` - Eprom |
| Fidelity | F0 |
| Concern | CAT |
| Release | R8 |
...
Create the complete, immutable catalog definition for **Eprom** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-memory-storage-rom-prom` (ROM and PROM); task scope: variant `var-memory-storage-rom-prom-eprom` (Eprom); fidelity `F0`; concern `CAT`.
...
- Canonical source: [ROM and PROM family-specific implementation reference](../../catalog/families/fam-memory-storage-rom-prom.md#family-specific-implementation-reference), registry row `fam-memory-storage-rom-prom`, and variant `var-memory-storage-rom-prom-eprom` (Eprom).
...
- `docs/tasks/models/cmp-memory-storage-rom-prom-eprom-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-memory-storage-rom-prom-eprom` (Eprom), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-MEMORY-STORAGE-ROM-PROM-EPROM-F0-CAT`: variant `var-memory-storage-rom-prom-eprom` (Eprom), fidelity `F0`, concern `CAT`, and requirements REQ-018, REQ-021, REQ-025, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MEMORY-STORAGE-ROM-PROM-EPROM-F0-CAT-NOMINAL`
- `TEST-CMP-MEMORY-STORAGE-ROM-PROM-EPROM-F0-CAT-BOUNDARY`
- `TEST-CMP-MEMORY-STORAGE-ROM-PROM-EPROM-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-MEMORY-STORAGE-ROM-PROM-EPROM-F0-CAT` exists and is limited to variant `var-memory-storage-rom-prom-eprom` (Eprom), `F0`, and `CAT`.
