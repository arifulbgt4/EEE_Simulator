# CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F0-MODEL - Implement Resistor array F0 model
...
Implement one model tier, **F0**, for **Resistor array** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-passives-resistor-array` (Resistor array); task scope: shared family scope across `var-passives-resistor-array-isolated`, `var-passives-resistor-array-bussed`; fidelity `F0`; concern `MODEL`.
...
- Exact F0 execution rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Each declared resistor branch obeys v_k = R_k*i_k; shared-terminal and matching/correlation rules come only from the selected array topology.
...
- `docs/tasks/models/cmp-passives-resistor-array-shared-f0-model.md`
...
- One `ModelBinding` for `fam-passives-resistor-array` at `F0` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F0-MODEL`: shared family scope across `var-passives-resistor-array-isolated`, `var-passives-resistor-array-bussed`, fidelity `F0`, concern `MODEL`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F0-MODEL-NOMINAL`
- `TEST-CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F0-MODEL-BOUNDARY`
- `TEST-CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F0-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F0-MODEL` exists and is limited to shared family scope across `var-passives-resistor-array-isolated`, `var-passives-resistor-array-bussed`, `F0`, and `MODEL`.
