# CMP-ACTUATORS-HMI-DISPLAY-SHARED-F3-VALIDATION - Validate Display F3
...
Produce independent validation evidence for **Display** at **F3** without modifying the model under test.
...
- Stable family: `fam-actuators-hmi-display` (Display); task scope: shared family scope across `var-actuators-hmi-display-seven-segment`, `var-actuators-hmi-display-dot-matrix`, `var-actuators-hmi-display-lcd`, `var-actuators-hmi-display-oled`, `var-actuators-hmi-display-e-paper`; fidelity `F3`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F3 rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Electrical/digital inputs update the selected segment/pixel state with declared addressing, refresh, luminance, timing, power, and persistence.
...
- `docs/tasks/models/cmp-actuators-hmi-display-shared-f3-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-ACT-DISPLAY-NOMINAL`, `GOLD-ACT-DISPLAY-BOUNDARY`, `GOLD-ACT-DISPLAY-FAILURE` at `F3`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-ACTUATORS-HMI-DISPLAY-SHARED-F3-VALIDATION`: shared family scope across `var-actuators-hmi-display-seven-segment`, `var-actuators-hmi-display-dot-matrix`, `var-actuators-hmi-display-lcd`, `var-actuators-hmi-display-oled`, `var-actuators-hmi-display-e-paper`, fidelity `F3`, concern `VALIDATION`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ACTUATORS-HMI-DISPLAY-SHARED-F3-VALIDATION-NOMINAL`
- `TEST-CMP-ACTUATORS-HMI-DISPLAY-SHARED-F3-VALIDATION-BOUNDARY`
- `TEST-CMP-ACTUATORS-HMI-DISPLAY-SHARED-F3-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-ACTUATORS-HMI-DISPLAY-SHARED-F3-VALIDATION` exists and is limited to shared family scope across `var-actuators-hmi-display-seven-segment`, `var-actuators-hmi-display-dot-matrix`, `var-actuators-hmi-display-lcd`, `var-actuators-hmi-display-oled`, `var-actuators-hmi-display-e-paper`, `F3`, and `VALIDATION`.
