# CMP-PASSIVES-CHOKE-DIFFERENTIAL-MODE-F0-CAT - Specify Differential Mode catalog preset
...
| Variant | `var-passives-choke-differential-mode` - Differential Mode |
| Fidelity | F0 |
| Concern | CAT |
| Release | R2 |
...
Create the complete, immutable catalog definition for **Differential Mode** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-passives-choke` (Choke); task scope: variant `var-passives-choke-differential-mode` (Differential Mode); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Choke family-specific implementation reference](../../catalog/families/fam-passives-choke.md#family-specific-implementation-reference), registry row `fam-passives-choke`, and variant `var-passives-choke-differential-mode` (Differential Mode).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Each winding follows v = d(lambda)/dt with the declared differential/common-mode inductance matrix, winding loss, and saturation envelope.
...
- `docs/tasks/models/cmp-passives-choke-differential-mode-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-passives-choke-differential-mode` (Differential Mode), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-PASSIVES-CHOKE-DIFFERENTIAL-MODE-F0-CAT`: variant `var-passives-choke-differential-mode` (Differential Mode), fidelity `F0`, concern `CAT`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-CHOKE-DIFFERENTIAL-MODE-F0-CAT-NOMINAL`
- `TEST-CMP-PASSIVES-CHOKE-DIFFERENTIAL-MODE-F0-CAT-BOUNDARY`
- `TEST-CMP-PASSIVES-CHOKE-DIFFERENTIAL-MODE-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-CHOKE-DIFFERENTIAL-MODE-F0-CAT` exists and is limited to variant `var-passives-choke-differential-mode` (Differential Mode), `F0`, and `CAT`.
