# CMP-ACTUATORS-HMI-DISPLAY-SHARED-F2-VALIDATION - Validate Display F2
...
Produce independent validation evidence for **Display** at **F2** without modifying the model under test.
...
- Stable family: `fam-actuators-hmi-display` (Display); task scope: shared family scope across `var-actuators-hmi-display-seven-segment`, `var-actuators-hmi-display-dot-matrix`, `var-actuators-hmi-display-lcd`, `var-actuators-hmi-display-oled`, `var-actuators-hmi-display-e-paper`; fidelity `F2`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F2 rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: Electrical/digital inputs update the selected segment/pixel state with declared addressing, refresh, luminance, timing, power, and persistence.
...
- `docs/tasks/models/cmp-actuators-hmi-display-shared-f2-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-ACT-DISPLAY-NOMINAL`, `GOLD-ACT-DISPLAY-BOUNDARY`, `GOLD-ACT-DISPLAY-FAILURE` at `F2`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-ACTUATORS-HMI-DISPLAY-SHARED-F2-VALIDATION`: shared family scope across `var-actuators-hmi-display-seven-segment`, `var-actuators-hmi-display-dot-matrix`, `var-actuators-hmi-display-lcd`, `var-actuators-hmi-display-oled`, `var-actuators-hmi-display-e-paper`, fidelity `F2`, concern `VALIDATION`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ACTUATORS-HMI-DISPLAY-SHARED-F2-VALIDATION-NOMINAL`
- `TEST-CMP-ACTUATORS-HMI-DISPLAY-SHARED-F2-VALIDATION-BOUNDARY`
- `TEST-CMP-ACTUATORS-HMI-DISPLAY-SHARED-F2-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-ACTUATORS-HMI-DISPLAY-SHARED-F2-VALIDATION` exists and is limited to shared family scope across `var-actuators-hmi-display-seven-segment`, `var-actuators-hmi-display-dot-matrix`, `var-actuators-hmi-display-lcd`, `var-actuators-hmi-display-oled`, `var-actuators-hmi-display-e-paper`, `F2`, and `VALIDATION`.
