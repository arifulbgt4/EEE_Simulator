# CMP-DIGITAL-LOGIC-STATE-MACHINE-SHARED-F3-MODEL - Implement State machine F3 model
...
Implement one model tier, **F3**, for **State machine** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-digital-logic-state-machine` (State machine); task scope: shared family scope across `var-digital-logic-state-machine-moore`, `var-digital-logic-state-machine-mealy`; fidelity `F3`; concern `MODEL`.
...
- Exact F3 execution rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: At each declared event, next_state = F(state,input) and output = G(state,input) using the versioned transition/output tables.
...
- `docs/tasks/models/cmp-digital-logic-state-machine-shared-f3-model.md`
...
- One `ModelBinding` for `fam-digital-logic-state-machine` at `F3` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-DIGITAL-LOGIC-STATE-MACHINE-SHARED-F3-MODEL`: shared family scope across `var-digital-logic-state-machine-moore`, `var-digital-logic-state-machine-mealy`, fidelity `F3`, concern `MODEL`, and requirements REQ-018, REQ-021, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-DIGITAL-LOGIC-STATE-MACHINE-SHARED-F3-MODEL-NOMINAL`
- `TEST-CMP-DIGITAL-LOGIC-STATE-MACHINE-SHARED-F3-MODEL-BOUNDARY`
- `TEST-CMP-DIGITAL-LOGIC-STATE-MACHINE-SHARED-F3-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-DIGITAL-LOGIC-STATE-MACHINE-SHARED-F3-MODEL` exists and is limited to shared family scope across `var-digital-logic-state-machine-moore`, `var-digital-logic-state-machine-mealy`, `F3`, and `MODEL`.
