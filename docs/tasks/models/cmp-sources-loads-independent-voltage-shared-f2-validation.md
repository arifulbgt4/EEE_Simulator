# CMP-SOURCES-LOADS-INDEPENDENT-VOLTAGE-SHARED-F2-VALIDATION - Validate Independent voltage source F2
...
Produce independent validation evidence for **Independent voltage source** at **F2** without modifying the model under test.
...
- Stable family: `fam-sources-loads-independent-voltage` (Independent voltage source); task scope: shared family scope across `var-sources-loads-independent-voltage-dc`, `var-sources-loads-independent-voltage-ac-small-signal`, `var-sources-loads-independent-voltage-sine`, `var-sources-loads-independent-voltage-pulse`; fidelity `F2`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F2 rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: The branch constraint is v(P) - v(N) = V(t), with V(t) supplied by the selected DC/AC/transient profile.
...
- `docs/tasks/models/cmp-sources-loads-independent-voltage-shared-f2-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SRC-INDEPENDENT_VOLTAGE-NOMINAL`, `GOLD-SRC-INDEPENDENT_VOLTAGE-BOUNDARY`, `GOLD-SRC-INDEPENDENT_VOLTAGE-FAILURE` at `F2`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-SOURCES-LOADS-INDEPENDENT-VOLTAGE-SHARED-F2-VALIDATION`: shared family scope across `var-sources-loads-independent-voltage-dc`, `var-sources-loads-independent-voltage-ac-small-signal`, `var-sources-loads-independent-voltage-sine`, `var-sources-loads-independent-voltage-pulse`, fidelity `F2`, concern `VALIDATION`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-VOLTAGE-SHARED-F2-VALIDATION-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-VOLTAGE-SHARED-F2-VALIDATION-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-VOLTAGE-SHARED-F2-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-INDEPENDENT-VOLTAGE-SHARED-F2-VALIDATION` exists and is limited to shared family scope across `var-sources-loads-independent-voltage-dc`, `var-sources-loads-independent-voltage-ac-small-signal`, `var-sources-loads-independent-voltage-sine`, `var-sources-loads-independent-voltage-pulse`, `F2`, and `VALIDATION`.
