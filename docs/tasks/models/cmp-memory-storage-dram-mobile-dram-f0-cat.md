# CMP-MEMORY-STORAGE-DRAM-MOBILE-DRAM-F0-CAT - Specify Mobile Dram catalog preset
...
| Variant | `var-memory-storage-dram-mobile-dram` - Mobile Dram |
| Fidelity | F0 |
| Concern | CAT |
| Release | R8 |
...
Create the complete, immutable catalog definition for **Mobile Dram** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-memory-storage-dram` (Dynamic RAM); task scope: variant `var-memory-storage-dram-mobile-dram` (Mobile Dram); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Dynamic RAM family-specific implementation reference](../../catalog/families/fam-memory-storage-dram.md#family-specific-implementation-reference), registry row `fam-memory-storage-dram`, and variant `var-memory-storage-dram-mobile-dram` (Mobile Dram).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: A bank/row/column state model enforces activate/read/write/precharge/refresh timing, retention, initialization, and conflict rules.
...
- `docs/tasks/models/cmp-memory-storage-dram-mobile-dram-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-memory-storage-dram-mobile-dram` (Mobile Dram), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-MEMORY-STORAGE-DRAM-MOBILE-DRAM-F0-CAT`: variant `var-memory-storage-dram-mobile-dram` (Mobile Dram), fidelity `F0`, concern `CAT`, and requirements REQ-018, REQ-021, REQ-025, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MEMORY-STORAGE-DRAM-MOBILE-DRAM-F0-CAT-NOMINAL`
- `TEST-CMP-MEMORY-STORAGE-DRAM-MOBILE-DRAM-F0-CAT-BOUNDARY`
- `TEST-CMP-MEMORY-STORAGE-DRAM-MOBILE-DRAM-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-MEMORY-STORAGE-DRAM-MOBILE-DRAM-F0-CAT` exists and is limited to variant `var-memory-storage-dram-mobile-dram` (Mobile Dram), `F0`, and `CAT`.
