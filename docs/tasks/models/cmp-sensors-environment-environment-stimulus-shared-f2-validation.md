# CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-SHARED-F2-VALIDATION - Validate Environmental stimulus F2
...
Produce independent validation evidence for **Environmental stimulus** at **F2** without modifying the model under test.
...
- Stable family: `fam-sensors-environment-environment-stimulus` (Environmental stimulus); task scope: shared family scope across `var-sensors-environment-environment-stimulus-constant-stimulus`, `var-sensors-environment-environment-stimulus-time-profile`; fidelity `F2`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F2 rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: The primitive emits a deterministic SI-valued environmental field/time profile; interpolation, spatial scope, discontinuities, and seed are explicit.
...
- `docs/tasks/models/cmp-sensors-environment-environment-stimulus-shared-f2-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SEN-ENVIRONMENT_STIMULUS-NOMINAL`, `GOLD-SEN-ENVIRONMENT_STIMULUS-BOUNDARY`, `GOLD-SEN-ENVIRONMENT_STIMULUS-FAILURE` at `F2`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-SHARED-F2-VALIDATION`: shared family scope across `var-sensors-environment-environment-stimulus-constant-stimulus`, `var-sensors-environment-environment-stimulus-time-profile`, fidelity `F2`, concern `VALIDATION`, and requirements REQ-014, REQ-015, REQ-018, REQ-020, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-SHARED-F2-VALIDATION-NOMINAL`
- `TEST-CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-SHARED-F2-VALIDATION-BOUNDARY`
- `TEST-CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-SHARED-F2-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-SHARED-F2-VALIDATION` exists and is limited to shared family scope across `var-sensors-environment-environment-stimulus-constant-stimulus`, `var-sensors-environment-environment-stimulus-time-profile`, `F2`, and `VALIDATION`.
