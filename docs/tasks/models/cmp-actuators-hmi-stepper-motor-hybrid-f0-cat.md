# CMP-ACTUATORS-HMI-STEPPER-MOTOR-HYBRID-F0-CAT - Specify Hybrid catalog preset
...
| Variant | `var-actuators-hmi-stepper-motor-hybrid` - Hybrid |
| Fidelity | F0 |
| Concern | CAT |
| Release | R7 |
...
Create the complete, immutable catalog definition for **Hybrid** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-actuators-hmi-stepper-motor` (Stepper motor); task scope: variant `var-actuators-hmi-stepper-motor-hybrid` (Hybrid); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Stepper motor family-specific implementation reference](../../catalog/families/fam-actuators-hmi-stepper-motor.md#family-specific-implementation-reference), registry row `fam-actuators-hmi-stepper-motor`, and variant `var-actuators-hmi-stepper-motor-hybrid` (Hybrid).
...
- `docs/tasks/models/cmp-actuators-hmi-stepper-motor-hybrid-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-actuators-hmi-stepper-motor-hybrid` (Hybrid), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-ACTUATORS-HMI-STEPPER-MOTOR-HYBRID-F0-CAT`: variant `var-actuators-hmi-stepper-motor-hybrid` (Hybrid), fidelity `F0`, concern `CAT`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ACTUATORS-HMI-STEPPER-MOTOR-HYBRID-F0-CAT-NOMINAL`
- `TEST-CMP-ACTUATORS-HMI-STEPPER-MOTOR-HYBRID-F0-CAT-BOUNDARY`
- `TEST-CMP-ACTUATORS-HMI-STEPPER-MOTOR-HYBRID-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-ACTUATORS-HMI-STEPPER-MOTOR-HYBRID-F0-CAT` exists and is limited to variant `var-actuators-hmi-stepper-motor-hybrid` (Hybrid), `F0`, and `CAT`.
