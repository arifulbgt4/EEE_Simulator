# CMP-MEMORY-STORAGE-FUNCTIONAL-STORAGE-SHARED-F2-VALIDATION - Validate Functional storage F2
...
Produce independent validation evidence for **Functional storage** at **F2** without modifying the model under test.
...
- Stable family: `fam-memory-storage-functional-storage` (Functional storage); task scope: shared family scope across `var-memory-storage-functional-storage-block-device`, `var-memory-storage-functional-storage-file-image`, `var-memory-storage-functional-storage-removable-media`; fidelity `F2`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F2 rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: Functional block storage exposes versioned read/write/flush/error operations with deterministic data persistence and bounded latency abstraction.
...
- `docs/tasks/models/cmp-memory-storage-functional-storage-shared-f2-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-MEM-FUNCTIONAL_STORAGE-NOMINAL`, `GOLD-MEM-FUNCTIONAL_STORAGE-BOUNDARY`, `GOLD-MEM-FUNCTIONAL_STORAGE-FAILURE` at `F2`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-MEMORY-STORAGE-FUNCTIONAL-STORAGE-SHARED-F2-VALIDATION`: shared family scope across `var-memory-storage-functional-storage-block-device`, `var-memory-storage-functional-storage-file-image`, `var-memory-storage-functional-storage-removable-media`, fidelity `F2`, concern `VALIDATION`, and requirements REQ-018, REQ-021, REQ-025, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MEMORY-STORAGE-FUNCTIONAL-STORAGE-SHARED-F2-VALIDATION-NOMINAL`
- `TEST-CMP-MEMORY-STORAGE-FUNCTIONAL-STORAGE-SHARED-F2-VALIDATION-BOUNDARY`
- `TEST-CMP-MEMORY-STORAGE-FUNCTIONAL-STORAGE-SHARED-F2-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-MEMORY-STORAGE-FUNCTIONAL-STORAGE-SHARED-F2-VALIDATION` exists and is limited to shared family scope across `var-memory-storage-functional-storage-block-device`, `var-memory-storage-functional-storage-file-image`, `var-memory-storage-functional-storage-removable-media`, `F2`, and `VALIDATION`.
