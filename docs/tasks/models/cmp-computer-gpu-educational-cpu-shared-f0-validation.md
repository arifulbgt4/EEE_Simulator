# CMP-COMPUTER-GPU-EDUCATIONAL-CPU-SHARED-F0-VALIDATION - Validate Educational CPU F0
...
Produce independent validation evidence for **Educational CPU** at **F0** without modifying the model under test.
...
- Candidate and independent reference must evaluate the same F0 rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The CPU advances the declared ISA/microarchitecture state per instruction or cycle, including registers, flags, PC, memory/bus transactions, interrupts, reset, and traps.
...
- `docs/tasks/models/cmp-computer-gpu-educational-cpu-shared-f0-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-CPU-EDUCATIONAL_CPU-NOMINAL`, `GOLD-CPU-EDUCATIONAL_CPU-BOUNDARY`, `GOLD-CPU-EDUCATIONAL_CPU-FAILURE` at `F0`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-COMPUTER-GPU-EDUCATIONAL-CPU-SHARED-F0-VALIDATION`: shared family scope across `var-computer-gpu-educational-cpu-one-bit`, `var-computer-gpu-educational-cpu-four-bit`, `var-computer-gpu-educational-cpu-eight-bit`, `var-computer-gpu-educational-cpu-sixteen-bit`, `var-computer-gpu-educational-cpu-small-risc-v`, fidelity `F0`, concern `VALIDATION`, and requirements REQ-025, REQ-027, REQ-028, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-COMPUTER-GPU-EDUCATIONAL-CPU-SHARED-F0-VALIDATION-NOMINAL`
- `TEST-CMP-COMPUTER-GPU-EDUCATIONAL-CPU-SHARED-F0-VALIDATION-BOUNDARY`
- `TEST-CMP-COMPUTER-GPU-EDUCATIONAL-CPU-SHARED-F0-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-COMPUTER-GPU-EDUCATIONAL-CPU-SHARED-F0-VALIDATION` exists and is limited to shared family scope across `var-computer-gpu-educational-cpu-one-bit`, `var-computer-gpu-educational-cpu-four-bit`, `var-computer-gpu-educational-cpu-eight-bit`, `var-computer-gpu-educational-cpu-sixteen-bit`, `var-computer-gpu-educational-cpu-small-risc-v`, `F0`, and `VALIDATION`.
