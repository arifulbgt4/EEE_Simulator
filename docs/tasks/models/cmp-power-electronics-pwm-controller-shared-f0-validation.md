# CMP-POWER-ELECTRONICS-PWM-CONTROLLER-SHARED-F0-VALIDATION - Validate PWM controller F0
...
Produce independent validation evidence for **PWM controller** at **F0** without modifying the model under test.
...
- Stable family: `fam-power-electronics-pwm-controller` (PWM controller); task scope: shared family scope across `var-power-electronics-pwm-controller-configurable-pwm`; fidelity `F0`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F0 rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The deterministic modulator compares command and carrier/control state to emit bounded PWM events with declared frequency, dead time, and saturation.
...
- `docs/tasks/models/cmp-power-electronics-pwm-controller-shared-f0-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-PWR-PWM_CONTROLLER-NOMINAL`, `GOLD-PWR-PWM_CONTROLLER-BOUNDARY`, `GOLD-PWR-PWM_CONTROLLER-FAILURE` at `F0`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-POWER-ELECTRONICS-PWM-CONTROLLER-SHARED-F0-VALIDATION`: shared family scope across `var-power-electronics-pwm-controller-configurable-pwm`, fidelity `F0`, concern `VALIDATION`, and requirements REQ-009, REQ-010, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-POWER-ELECTRONICS-PWM-CONTROLLER-SHARED-F0-VALIDATION-NOMINAL`
- `TEST-CMP-POWER-ELECTRONICS-PWM-CONTROLLER-SHARED-F0-VALIDATION-BOUNDARY`
- `TEST-CMP-POWER-ELECTRONICS-PWM-CONTROLLER-SHARED-F0-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-POWER-ELECTRONICS-PWM-CONTROLLER-SHARED-F0-VALIDATION` exists and is limited to shared family scope across `var-power-electronics-pwm-controller-configurable-pwm`, `F0`, and `VALIDATION`.
