# CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-SHARED-F2-MODEL - Implement Environmental stimulus F2 model
...
Implement one model tier, **F2**, for **Environmental stimulus** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-sensors-environment-environment-stimulus` (Environmental stimulus); task scope: shared family scope across `var-sensors-environment-environment-stimulus-constant-stimulus`, `var-sensors-environment-environment-stimulus-time-profile`; fidelity `F2`; concern `MODEL`.
...
- Exact F2 execution rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: The primitive emits a deterministic SI-valued environmental field/time profile; interpolation, spatial scope, discontinuities, and seed are explicit.
...
- `docs/tasks/models/cmp-sensors-environment-environment-stimulus-shared-f2-model.md`
...
- One `ModelBinding` for `fam-sensors-environment-environment-stimulus` at `F2` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-SHARED-F2-MODEL`: shared family scope across `var-sensors-environment-environment-stimulus-constant-stimulus`, `var-sensors-environment-environment-stimulus-time-profile`, fidelity `F2`, concern `MODEL`, and requirements REQ-014, REQ-015, REQ-018, REQ-020, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-SHARED-F2-MODEL-NOMINAL`
- `TEST-CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-SHARED-F2-MODEL-BOUNDARY`
- `TEST-CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-SHARED-F2-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-SHARED-F2-MODEL` exists and is limited to shared family scope across `var-sensors-environment-environment-stimulus-constant-stimulus`, `var-sensors-environment-environment-stimulus-time-profile`, `F2`, and `MODEL`.
