# CMP-PASSIVES-PHOTORESISTOR-LDR-F0-CAT - Specify Ldr catalog preset
...
| Variant | `var-passives-photoresistor-ldr` - Ldr |
| Fidelity | F0 |
| Concern | CAT |
| Release | R2 |
...
Create the complete, immutable catalog definition for **Ldr** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-passives-photoresistor` (Photoresistor); task scope: variant `var-passives-photoresistor-ldr` (Ldr); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Photoresistor family-specific implementation reference](../../catalog/families/fam-passives-photoresistor.md#family-specific-implementation-reference), registry row `fam-passives-photoresistor`, and variant `var-passives-photoresistor-ldr` (Ldr).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Resistance is the declared bounded transfer R = f(illuminance,T) with response-time state; zero or out-of-range stimulus uses explicit saturation behavior.
...
- `docs/tasks/models/cmp-passives-photoresistor-ldr-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-passives-photoresistor-ldr` (Ldr), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-PASSIVES-PHOTORESISTOR-LDR-F0-CAT`: variant `var-passives-photoresistor-ldr` (Ldr), fidelity `F0`, concern `CAT`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-PHOTORESISTOR-LDR-F0-CAT-NOMINAL`
- `TEST-CMP-PASSIVES-PHOTORESISTOR-LDR-F0-CAT-BOUNDARY`
- `TEST-CMP-PASSIVES-PHOTORESISTOR-LDR-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-PHOTORESISTOR-LDR-F0-CAT` exists and is limited to variant `var-passives-photoresistor-ldr` (Ldr), `F0`, and `CAT`.
