# CMP-SOURCES-LOADS-BATTERY-SOURCE-BATTERY-PACK-F0-CAT - Specify Battery Pack catalog preset
...
| Variant | `var-sources-loads-battery-source-battery-pack` - Battery Pack |
| Fidelity | F0 |
| Concern | CAT |
| Release | R2 |
...
Create the complete, immutable catalog definition for **Battery Pack** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-sources-loads-battery-source` (Battery source); task scope: variant `var-sources-loads-battery-source-battery-pack` (Battery Pack); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Battery source family-specific implementation reference](../../catalog/families/fam-sources-loads-battery-source.md#family-specific-implementation-reference), registry row `fam-sources-loads-battery-source`, and variant `var-sources-loads-battery-source-battery-pack` (Battery Pack).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Terminal voltage is OCV(SOC,T) - I*R_internal(SOC,T), with SOC advanced by charge balance dSOC/dt = -I/Q_capacity under the declared sign convention.
...
- `docs/tasks/models/cmp-sources-loads-battery-source-battery-pack-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-sources-loads-battery-source-battery-pack` (Battery Pack), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-SOURCES-LOADS-BATTERY-SOURCE-BATTERY-PACK-F0-CAT`: variant `var-sources-loads-battery-source-battery-pack` (Battery Pack), fidelity `F0`, concern `CAT`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-BATTERY-SOURCE-BATTERY-PACK-F0-CAT-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-BATTERY-SOURCE-BATTERY-PACK-F0-CAT-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-BATTERY-SOURCE-BATTERY-PACK-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-BATTERY-SOURCE-BATTERY-PACK-F0-CAT` exists and is limited to variant `var-sources-loads-battery-source-battery-pack` (Battery Pack), `F0`, and `CAT`.
