# CMP-SWITCH-PROTECTION-TRANSMISSION-GATE-SHARED-F3-VALIDATION - Validate Transmission gate F3
...
Produce independent validation evidence for **Transmission gate** at **F3** without modifying the model under test.
...
- Stable family: `fam-switch-protection-transmission-gate` (Transmission gate); task scope: shared family scope across `var-switch-protection-transmission-gate-cmos`, `var-switch-protection-transmission-gate-bidirectional`; fidelity `F3`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F3 rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Complementary control selects a bidirectional finite-conductance path; conflicting/unknown controls yield the documented X or high-impedance behavior.
...
- `docs/tasks/models/cmp-switch-protection-transmission-gate-shared-f3-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SWP-TRANSMISSION_GATE-NOMINAL`, `GOLD-SWP-TRANSMISSION_GATE-BOUNDARY`, `GOLD-SWP-TRANSMISSION_GATE-FAILURE` at `F3`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-SWITCH-PROTECTION-TRANSMISSION-GATE-SHARED-F3-VALIDATION`: shared family scope across `var-switch-protection-transmission-gate-cmos`, `var-switch-protection-transmission-gate-bidirectional`, fidelity `F3`, concern `VALIDATION`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SWITCH-PROTECTION-TRANSMISSION-GATE-SHARED-F3-VALIDATION-NOMINAL`
- `TEST-CMP-SWITCH-PROTECTION-TRANSMISSION-GATE-SHARED-F3-VALIDATION-BOUNDARY`
- `TEST-CMP-SWITCH-PROTECTION-TRANSMISSION-GATE-SHARED-F3-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SWITCH-PROTECTION-TRANSMISSION-GATE-SHARED-F3-VALIDATION` exists and is limited to shared family scope across `var-switch-protection-transmission-gate-cmos`, `var-switch-protection-transmission-gate-bidirectional`, `F3`, and `VALIDATION`.
