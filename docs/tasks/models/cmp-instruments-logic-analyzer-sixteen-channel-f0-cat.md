# CMP-INSTRUMENTS-LOGIC-ANALYZER-SIXTEEN-CHANNEL-F0-CAT - Specify Sixteen Channel catalog preset
...
| Variant | `var-instruments-logic-analyzer-sixteen-channel` - Sixteen Channel |
| Fidelity | F0 |
| Concern | CAT |
| Release | R2 |
...
Create the complete, immutable catalog definition for **Sixteen Channel** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-instruments-logic-analyzer` (Logic analyzer); task scope: variant `var-instruments-logic-analyzer-sixteen-channel` (Sixteen Channel); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Logic analyzer family-specific implementation reference](../../catalog/families/fam-instruments-logic-analyzer.md#family-specific-implementation-reference), registry row `fam-instruments-logic-analyzer`, and variant `var-instruments-logic-analyzer-sixteen-channel` (Sixteen Channel).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Channels resolve sampled thresholds to 0/1/X/Z and capture deterministic timestamped events under declared trigger and memory rules.
...
- `docs/tasks/models/cmp-instruments-logic-analyzer-sixteen-channel-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-instruments-logic-analyzer-sixteen-channel` (Sixteen Channel), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-INSTRUMENTS-LOGIC-ANALYZER-SIXTEEN-CHANNEL-F0-CAT`: variant `var-instruments-logic-analyzer-sixteen-channel` (Sixteen Channel), fidelity `F0`, concern `CAT`, and requirements REQ-007, REQ-021, REQ-022, REQ-023, REQ-034, REQ-037.
...
- `TEST-CMP-INSTRUMENTS-LOGIC-ANALYZER-SIXTEEN-CHANNEL-F0-CAT-NOMINAL`
- `TEST-CMP-INSTRUMENTS-LOGIC-ANALYZER-SIXTEEN-CHANNEL-F0-CAT-BOUNDARY`
- `TEST-CMP-INSTRUMENTS-LOGIC-ANALYZER-SIXTEEN-CHANNEL-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-INSTRUMENTS-LOGIC-ANALYZER-SIXTEEN-CHANNEL-F0-CAT` exists and is limited to variant `var-instruments-logic-analyzer-sixteen-channel` (Sixteen Channel), `F0`, and `CAT`.
