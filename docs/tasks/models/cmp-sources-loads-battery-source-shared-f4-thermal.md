# CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F4-THERMAL - Implement Battery source electrothermal behavior
...
Implement only the F4 power, temperature and electrothermal feedback concern for **Battery source**.
...
- Stable family: `fam-sources-loads-battery-source` (Battery source); task scope: shared family scope across `var-sources-loads-battery-source-battery-pack`; fidelity `F4`; concern `THERMAL`.
...
- `docs/tasks/models/cmp-sources-loads-battery-source-shared-f4-thermal.md`
...
- One F4 thermal-state contract for `fam-sources-loads-battery-source` naming P, T, Tamb, Rth, Cth, initial state, valid range, update order, and unsupported coupling behavior.
- Deterministic steady/transient thermal and runaway/limit evidence tied to the exact family reference vectors.
- Scope is limited to `CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F4-THERMAL`: shared family scope across `var-sources-loads-battery-source-battery-pack`, fidelity `F4`, concern `THERMAL`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F4-THERMAL-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F4-THERMAL-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F4-THERMAL-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F4-THERMAL` exists and is limited to shared family scope across `var-sources-loads-battery-source-battery-pack`, `F4`, and `THERMAL`.
