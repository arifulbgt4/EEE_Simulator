# CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-EQUATION-MODEL-F0-CAT - Specify Equation Model catalog preset
...
| Variant | `var-semiconductors-semiconductor-generic-equation-model` - Equation Model |
| Fidelity | F0 |
| Concern | CAT |
| Release | R3 |
...
Create the complete, immutable catalog definition for **Equation Model** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-semiconductors-semiconductor-generic` (Generic semiconductor black box); task scope: variant `var-semiconductors-semiconductor-generic-equation-model` (Equation Model); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Generic semiconductor black box family-specific implementation reference](../../catalog/families/fam-semiconductors-semiconductor-generic.md#family-specific-implementation-reference), registry row `fam-semiconductors-semiconductor-generic`, and variant `var-semiconductors-semiconductor-generic-equation-model` (Equation Model).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Behavior comes only from the selected validated black-box/compact model; its ordered pins, analyses, envelope, provenance, and unsupported behavior are mandatory inputs.
...
- `docs/tasks/models/cmp-semiconductors-semiconductor-generic-equation-model-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-semiconductors-semiconductor-generic-equation-model` (Equation Model), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-EQUATION-MODEL-F0-CAT`: variant `var-semiconductors-semiconductor-generic-equation-model` (Equation Model), fidelity `F0`, concern `CAT`, and requirements REQ-009, REQ-010, REQ-011, REQ-012, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-EQUATION-MODEL-F0-CAT-NOMINAL`
- `TEST-CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-EQUATION-MODEL-F0-CAT-BOUNDARY`
- `TEST-CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-EQUATION-MODEL-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-SEMICONDUCTORS-SEMICONDUCTOR-GENERIC-EQUATION-MODEL-F0-CAT` exists and is limited to variant `var-semiconductors-semiconductor-generic-equation-model` (Equation Model), `F0`, and `CAT`.
