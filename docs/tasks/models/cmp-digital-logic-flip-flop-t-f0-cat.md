# CMP-DIGITAL-LOGIC-FLIP-FLOP-T-F0-CAT - Specify T catalog preset
...
| Variant | `var-digital-logic-flip-flop-t` - T |
| Fidelity | F0 |
| Concern | CAT |
| Release | R5 |
...
Create the complete, immutable catalog definition for **T** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-digital-logic-flip-flop` (Flip-flop); task scope: variant `var-digital-logic-flip-flop-t` (T); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Flip-flop family-specific implementation reference](../../catalog/families/fam-digital-logic-flip-flop.md#family-specific-implementation-reference), registry row `fam-digital-logic-flip-flop`, and variant `var-digital-logic-flip-flop-t` (T).
...
- `docs/tasks/models/cmp-digital-logic-flip-flop-t-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-digital-logic-flip-flop-t` (T), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-DIGITAL-LOGIC-FLIP-FLOP-T-F0-CAT`: variant `var-digital-logic-flip-flop-t` (T), fidelity `F0`, concern `CAT`, and requirements REQ-018, REQ-021, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-DIGITAL-LOGIC-FLIP-FLOP-T-F0-CAT-NOMINAL`
- `TEST-CMP-DIGITAL-LOGIC-FLIP-FLOP-T-F0-CAT-BOUNDARY`
- `TEST-CMP-DIGITAL-LOGIC-FLIP-FLOP-T-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-DIGITAL-LOGIC-FLIP-FLOP-T-F0-CAT` exists and is limited to variant `var-digital-logic-flip-flop-t` (T), `F0`, and `CAT`.
