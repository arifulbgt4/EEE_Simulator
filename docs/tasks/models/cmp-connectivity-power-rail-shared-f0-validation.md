# CMP-CONNECTIVITY-POWER-RAIL-SHARED-F0-VALIDATION - Validate Power rail F0
...
Produce independent validation evidence for **Power rail** at **F0** without modifying the model under test.
...
- Stable family: `fam-connectivity-power-rail` (Power rail); task scope: shared family scope across `var-connectivity-power-rail-parameterized-rail`; fidelity `F0`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F0 rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: A rail creates a named power net; it imposes a voltage equation only when a source/rail profile explicitly declares one, otherwise it is connectivity metadata.
...
- `docs/tasks/models/cmp-connectivity-power-rail-shared-f0-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-CON-POWER_RAIL-NOMINAL`, `GOLD-CON-POWER_RAIL-BOUNDARY`, `GOLD-CON-POWER_RAIL-FAILURE` at `F0`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-CONNECTIVITY-POWER-RAIL-SHARED-F0-VALIDATION`: shared family scope across `var-connectivity-power-rail-parameterized-rail`, fidelity `F0`, concern `VALIDATION`, and requirements REQ-004, REQ-005, REQ-008, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-CONNECTIVITY-POWER-RAIL-SHARED-F0-VALIDATION-NOMINAL`
- `TEST-CMP-CONNECTIVITY-POWER-RAIL-SHARED-F0-VALIDATION-BOUNDARY`
- `TEST-CMP-CONNECTIVITY-POWER-RAIL-SHARED-F0-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-CONNECTIVITY-POWER-RAIL-SHARED-F0-VALIDATION` exists and is limited to shared family scope across `var-connectivity-power-rail-parameterized-rail`, `F0`, and `VALIDATION`.
