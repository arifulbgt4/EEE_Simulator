# CMP-ACTUATORS-HMI-SERVO-SHARED-F4-THERMAL - Implement Servo electrothermal behavior
...
Implement only the F4 power, temperature and electrothermal feedback concern for **Servo**.
...
- Stable family: `fam-actuators-hmi-servo` (Servo); task scope: shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`; fidelity `F4`; concern `THERMAL`.
...
- `docs/tasks/models/cmp-actuators-hmi-servo-shared-f4-thermal.md`
...
- One F4 thermal-state contract for `fam-actuators-hmi-servo` naming P, T, Tamb, Rth, Cth, initial state, valid range, update order, and unsupported coupling behavior.
- Deterministic steady/transient thermal and runaway/limit evidence tied to the exact family reference vectors.
- Scope is limited to `CMP-ACTUATORS-HMI-SERVO-SHARED-F4-THERMAL`: shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`, fidelity `F4`, concern `THERMAL`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F4-THERMAL-NOMINAL`
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F4-THERMAL-BOUNDARY`
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F4-THERMAL-FAILURE`

## Acceptance

1. The exact output for `CMP-ACTUATORS-HMI-SERVO-SHARED-F4-THERMAL` exists and is limited to shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`, `F4`, and `THERMAL`.
