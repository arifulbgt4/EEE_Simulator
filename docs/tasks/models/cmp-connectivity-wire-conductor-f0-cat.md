# CMP-CONNECTIVITY-WIRE-CONDUCTOR-F0-CAT - Specify Conductor catalog preset
...
| Variant | `var-connectivity-wire-conductor` - Conductor |
| Fidelity | F0 |
| Concern | CAT |
| Release | R1 |
...
Create the complete, immutable catalog definition for **Conductor** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-connectivity-wire` (Wire and conductor); task scope: variant `var-connectivity-wire-conductor` (Conductor); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Wire and conductor family-specific implementation reference](../../catalog/families/fam-connectivity-wire.md#family-specific-implementation-reference), registry row `fam-connectivity-wire`, and variant `var-connectivity-wire-conductor` (Conductor).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Ideal connectivity enforces v(P) - v(N) = 0; any F1 non-ideal profile replaces the ideal branch with an explicitly parameterized series impedance.
...
- `docs/tasks/models/cmp-connectivity-wire-conductor-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-connectivity-wire-conductor` (Conductor), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-CONNECTIVITY-WIRE-CONDUCTOR-F0-CAT`: variant `var-connectivity-wire-conductor` (Conductor), fidelity `F0`, concern `CAT`, and requirements REQ-004, REQ-005, REQ-008, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-CONNECTIVITY-WIRE-CONDUCTOR-F0-CAT-NOMINAL`
- `TEST-CMP-CONNECTIVITY-WIRE-CONDUCTOR-F0-CAT-BOUNDARY`
- `TEST-CMP-CONNECTIVITY-WIRE-CONDUCTOR-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-CONNECTIVITY-WIRE-CONDUCTOR-F0-CAT` exists and is limited to variant `var-connectivity-wire-conductor` (Conductor), `F0`, and `CAT`.
