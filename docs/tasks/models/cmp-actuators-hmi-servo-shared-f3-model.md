# CMP-ACTUATORS-HMI-SERVO-SHARED-F3-MODEL - Implement Servo F3 model
...
Implement one model tier, **F3**, for **Servo** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-actuators-hmi-servo` (Servo); task scope: shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`; fidelity `F3`; concern `MODEL`.
...
- Exact F3 execution rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Command pulse/position drives a bounded closed-loop angle state with declared range, speed, dead band, torque/load, current, and stall behavior.
...
- `docs/tasks/models/cmp-actuators-hmi-servo-shared-f3-model.md`
...
- One `ModelBinding` for `fam-actuators-hmi-servo` at `F3` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-ACTUATORS-HMI-SERVO-SHARED-F3-MODEL`: shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`, fidelity `F3`, concern `MODEL`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F3-MODEL-NOMINAL`
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F3-MODEL-BOUNDARY`
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F3-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-ACTUATORS-HMI-SERVO-SHARED-F3-MODEL` exists and is limited to shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`, `F3`, and `MODEL`.
