# CMP-SEMICONDUCTORS-OPTO-ISOLATOR-SHARED-F3-MODEL - Implement Optically isolated semiconductor F3 model
...
Implement one model tier, **F3**, for **Optically isolated semiconductor** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-semiconductors-opto-isolator` (Optically isolated semiconductor); task scope: shared family scope across `var-semiconductors-opto-isolator-phototransistor-output`, `var-semiconductors-opto-isolator-triac-output`; fidelity `F3`; concern `MODEL`.
...
- Exact F3 execution rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Input diode current produces output photocurrent or logic transfer through declared CTR, delay, saturation, isolation leakage, and temperature behavior.
...
- `docs/tasks/models/cmp-semiconductors-opto-isolator-shared-f3-model.md`
...
- One `ModelBinding` for `fam-semiconductors-opto-isolator` at `F3` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-SEMICONDUCTORS-OPTO-ISOLATOR-SHARED-F3-MODEL`: shared family scope across `var-semiconductors-opto-isolator-phototransistor-output`, `var-semiconductors-opto-isolator-triac-output`, fidelity `F3`, concern `MODEL`, and requirements REQ-009, REQ-010, REQ-011, REQ-012, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SEMICONDUCTORS-OPTO-ISOLATOR-SHARED-F3-MODEL-NOMINAL`
- `TEST-CMP-SEMICONDUCTORS-OPTO-ISOLATOR-SHARED-F3-MODEL-BOUNDARY`
- `TEST-CMP-SEMICONDUCTORS-OPTO-ISOLATOR-SHARED-F3-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-SEMICONDUCTORS-OPTO-ISOLATOR-SHARED-F3-MODEL` exists and is limited to shared family scope across `var-semiconductors-opto-isolator-phototransistor-output`, `var-semiconductors-opto-isolator-triac-output`, `F3`, and `MODEL`.
