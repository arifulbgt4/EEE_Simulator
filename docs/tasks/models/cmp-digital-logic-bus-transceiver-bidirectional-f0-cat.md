# CMP-DIGITAL-LOGIC-BUS-TRANSCEIVER-BIDIRECTIONAL-F0-CAT - Specify Bidirectional catalog preset
...
| Variant | `var-digital-logic-bus-transceiver-bidirectional` - Bidirectional |
| Fidelity | F0 |
| Concern | CAT |
| Release | R5 |
...
- Stable family: `fam-digital-logic-bus-transceiver` (Bus transceiver); task scope: variant `var-digital-logic-bus-transceiver-bidirectional` (Bidirectional); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Bus transceiver family-specific implementation reference](../../catalog/families/fam-digital-logic-bus-transceiver.md#family-specific-implementation-reference), registry row `fam-digital-logic-bus-transceiver`, and variant `var-digital-logic-bus-transceiver-bidirectional` (Bidirectional).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Direction and enable map fixed-width data between buses through declared drive strengths; conflicts, Z, and X resolve deterministically.
...
- `docs/tasks/models/cmp-digital-logic-bus-transceiver-bidirectional-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-digital-logic-bus-transceiver-bidirectional` (Bidirectional), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-DIGITAL-LOGIC-BUS-TRANSCEIVER-BIDIRECTIONAL-F0-CAT`: variant `var-digital-logic-bus-transceiver-bidirectional` (Bidirectional), fidelity `F0`, concern `CAT`, and requirements REQ-018, REQ-021, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-DIGITAL-LOGIC-BUS-TRANSCEIVER-BIDIRECTIONAL-F0-CAT-NOMINAL`
- `TEST-CMP-DIGITAL-LOGIC-BUS-TRANSCEIVER-BIDIRECTIONAL-F0-CAT-BOUNDARY`
- `TEST-CMP-DIGITAL-LOGIC-BUS-TRANSCEIVER-BIDIRECTIONAL-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-DIGITAL-LOGIC-BUS-TRANSCEIVER-BIDIRECTIONAL-F0-CAT` exists and is limited to variant `var-digital-logic-bus-transceiver-bidirectional` (Bidirectional), `F0`, and `CAT`.
