# CMP-SENSORS-ENVIRONMENT-FORCE-PRESSURE-SENSOR-SHARED-F2-VALIDATION - Validate Force and pressure sensor F2
...
Produce independent validation evidence for **Force and pressure sensor** at **F2** without modifying the model under test.
...
- Stable family: `fam-sensors-environment-force-pressure-sensor` (Force and pressure sensor); task scope: shared family scope across `var-sensors-environment-force-pressure-sensor-strain-gauge`, `var-sensors-environment-force-pressure-sensor-load-cell`, `var-sensors-environment-force-pressure-sensor-absolute-pressure`, `var-sensors-environment-force-pressure-sensor-differential-pressure`; fidelity `F2`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F2 rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: Output follows the calibrated force/strain/pressure transfer with bridge or transducer excitation, range, hysteresis, creep, and temperature terms.
...
- `docs/tasks/models/cmp-sensors-environment-force-pressure-sensor-shared-f2-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SEN-FORCE_PRESSURE_SENSOR-NOMINAL`, `GOLD-SEN-FORCE_PRESSURE_SENSOR-BOUNDARY`, `GOLD-SEN-FORCE_PRESSURE_SENSOR-FAILURE` at `F2`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-SENSORS-ENVIRONMENT-FORCE-PRESSURE-SENSOR-SHARED-F2-VALIDATION`: shared family scope across `var-sensors-environment-force-pressure-sensor-strain-gauge`, `var-sensors-environment-force-pressure-sensor-load-cell`, `var-sensors-environment-force-pressure-sensor-absolute-pressure`, `var-sensors-environment-force-pressure-sensor-differential-pressure`, fidelity `F2`, concern `VALIDATION`, and requirements REQ-014, REQ-015, REQ-018, REQ-020, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SENSORS-ENVIRONMENT-FORCE-PRESSURE-SENSOR-SHARED-F2-VALIDATION-NOMINAL`
- `TEST-CMP-SENSORS-ENVIRONMENT-FORCE-PRESSURE-SENSOR-SHARED-F2-VALIDATION-BOUNDARY`
- `TEST-CMP-SENSORS-ENVIRONMENT-FORCE-PRESSURE-SENSOR-SHARED-F2-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SENSORS-ENVIRONMENT-FORCE-PRESSURE-SENSOR-SHARED-F2-VALIDATION` exists and is limited to shared family scope across `var-sensors-environment-force-pressure-sensor-strain-gauge`, `var-sensors-environment-force-pressure-sensor-load-cell`, `var-sensors-environment-force-pressure-sensor-absolute-pressure`, `var-sensors-environment-force-pressure-sensor-differential-pressure`, `F2`, and `VALIDATION`.
