# CMP-POWER-ELECTRONICS-CHARGER-BMS-SHARED-F2-VALIDATION - Validate Charger and battery management F2
...
Produce independent validation evidence for **Charger and battery management** at **F2** without modifying the model under test.
...
- Stable family: `fam-power-electronics-charger-bms` (Charger and battery management); task scope: shared family scope across `var-power-electronics-charger-bms-battery-charger`, `var-power-electronics-charger-bms-bms`; fidelity `F2`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F2 rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: Charging and protection follow the declared CC/CV or chemistry profile plus cell-voltage/current/temperature state machine and balancing rules.
...
- `docs/tasks/models/cmp-power-electronics-charger-bms-shared-f2-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-PWR-CHARGER_BMS-NOMINAL`, `GOLD-PWR-CHARGER_BMS-BOUNDARY`, `GOLD-PWR-CHARGER_BMS-FAILURE` at `F2`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-POWER-ELECTRONICS-CHARGER-BMS-SHARED-F2-VALIDATION`: shared family scope across `var-power-electronics-charger-bms-battery-charger`, `var-power-electronics-charger-bms-bms`, fidelity `F2`, concern `VALIDATION`, and requirements REQ-009, REQ-010, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-POWER-ELECTRONICS-CHARGER-BMS-SHARED-F2-VALIDATION-NOMINAL`
- `TEST-CMP-POWER-ELECTRONICS-CHARGER-BMS-SHARED-F2-VALIDATION-BOUNDARY`
- `TEST-CMP-POWER-ELECTRONICS-CHARGER-BMS-SHARED-F2-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-POWER-ELECTRONICS-CHARGER-BMS-SHARED-F2-VALIDATION` exists and is limited to shared family scope across `var-power-electronics-charger-bms-battery-charger`, `var-power-electronics-charger-bms-bms`, `F2`, and `VALIDATION`.
