# CMP-MEMORY-STORAGE-SRAM-SHARED-F2-VALIDATION - Validate Static RAM F2
...
Produce independent validation evidence for **Static RAM** at **F2** without modifying the model under test.
...
- Stable family: `fam-memory-storage-sram` (Static RAM); task scope: shared family scope across `var-memory-storage-sram-asynchronous`, `var-memory-storage-sram-synchronous`, `var-memory-storage-sram-dual-port`; fidelity `F2`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F2 rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: A volatile word array follows declared address/data/control and clock rules with read/write timing, port conflict, initialization, and X behavior.
...
- `docs/tasks/models/cmp-memory-storage-sram-shared-f2-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-MEM-SRAM-NOMINAL`, `GOLD-MEM-SRAM-BOUNDARY`, `GOLD-MEM-SRAM-FAILURE` at `F2`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-MEMORY-STORAGE-SRAM-SHARED-F2-VALIDATION`: shared family scope across `var-memory-storage-sram-asynchronous`, `var-memory-storage-sram-synchronous`, `var-memory-storage-sram-dual-port`, fidelity `F2`, concern `VALIDATION`, and requirements REQ-018, REQ-021, REQ-025, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MEMORY-STORAGE-SRAM-SHARED-F2-VALIDATION-NOMINAL`
- `TEST-CMP-MEMORY-STORAGE-SRAM-SHARED-F2-VALIDATION-BOUNDARY`
- `TEST-CMP-MEMORY-STORAGE-SRAM-SHARED-F2-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-MEMORY-STORAGE-SRAM-SHARED-F2-VALIDATION` exists and is limited to shared family scope across `var-memory-storage-sram-asynchronous`, `var-memory-storage-sram-synchronous`, `var-memory-storage-sram-dual-port`, `F2`, and `VALIDATION`.
