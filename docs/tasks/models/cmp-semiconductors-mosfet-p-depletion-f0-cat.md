# CMP-SEMICONDUCTORS-MOSFET-P-DEPLETION-F0-CAT - Specify P Depletion catalog preset
...
| Variant | `var-semiconductors-mosfet-p-depletion` - P Depletion |
| Fidelity | F0 |
| Concern | CAT |
| Release | R3 |
...
Create the complete, immutable catalog definition for **P Depletion** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-semiconductors-mosfet` (MOSFET); task scope: variant `var-semiconductors-mosfet-p-depletion` (P Depletion); fidelity `F0`; concern `CAT`.
...
- Canonical source: [MOSFET family-specific implementation reference](../../catalog/families/fam-semiconductors-mosfet.md#family-specific-implementation-reference), registry row `fam-semiconductors-mosfet`, and variant `var-semiconductors-mosfet-p-depletion` (P Depletion).
...
- `docs/tasks/models/cmp-semiconductors-mosfet-p-depletion-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-semiconductors-mosfet-p-depletion` (P Depletion), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-SEMICONDUCTORS-MOSFET-P-DEPLETION-F0-CAT`: variant `var-semiconductors-mosfet-p-depletion` (P Depletion), fidelity `F0`, concern `CAT`, and requirements REQ-009, REQ-010, REQ-011, REQ-012, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SEMICONDUCTORS-MOSFET-P-DEPLETION-F0-CAT-NOMINAL`
- `TEST-CMP-SEMICONDUCTORS-MOSFET-P-DEPLETION-F0-CAT-BOUNDARY`
- `TEST-CMP-SEMICONDUCTORS-MOSFET-P-DEPLETION-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-SEMICONDUCTORS-MOSFET-P-DEPLETION-F0-CAT` exists and is limited to variant `var-semiconductors-mosfet-p-depletion` (P Depletion), `F0`, and `CAT`.
