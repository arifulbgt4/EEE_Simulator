# CMP-ACTUATORS-HMI-SERVO-SHARED-F1-VALIDATION - Validate Servo F1
...
Produce independent validation evidence for **Servo** at **F1** without modifying the model under test.
...
- Stable family: `fam-actuators-hmi-servo` (Servo); task scope: shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`; fidelity `F1`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F1 rule: Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: Command pulse/position drives a bounded closed-loop angle state with declared range, speed, dead band, torque/load, current, and stall behavior.
...
- `docs/tasks/models/cmp-actuators-hmi-servo-shared-f1-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-ACT-SERVO-NOMINAL`, `GOLD-ACT-SERVO-BOUNDARY`, `GOLD-ACT-SERVO-FAILURE` at `F1`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-ACTUATORS-HMI-SERVO-SHARED-F1-VALIDATION`: shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`, fidelity `F1`, concern `VALIDATION`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F1-VALIDATION-NOMINAL`
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F1-VALIDATION-BOUNDARY`
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F1-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-ACTUATORS-HMI-SERVO-SHARED-F1-VALIDATION` exists and is limited to shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`, `F1`, and `VALIDATION`.
