# CMP-SENSORS-ENVIRONMENT-SOUND-DISTANCE-SENSOR-SHARED-F3-VALIDATION - Validate Sound and distance sensor F3
...
Produce independent validation evidence for **Sound and distance sensor** at **F3** without modifying the model under test.
...
- Candidate and independent reference must evaluate the same F3 rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Output follows the acoustic pressure or time-of-flight relation with propagation speed, range, blanking, echo/noise, and timeout behavior.
...
- `docs/tasks/models/cmp-sensors-environment-sound-distance-sensor-shared-f3-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SEN-SOUND_DISTANCE_SENSOR-NOMINAL`, `GOLD-SEN-SOUND_DISTANCE_SENSOR-BOUNDARY`, `GOLD-SEN-SOUND_DISTANCE_SENSOR-FAILURE` at `F3`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- `TEST-CMP-SENSORS-ENVIRONMENT-SOUND-DISTANCE-SENSOR-SHARED-F3-VALIDATION-NOMINAL`
- `TEST-CMP-SENSORS-ENVIRONMENT-SOUND-DISTANCE-SENSOR-SHARED-F3-VALIDATION-BOUNDARY`
- `TEST-CMP-SENSORS-ENVIRONMENT-SOUND-DISTANCE-SENSOR-SHARED-F3-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SENSORS-ENVIRONMENT-SOUND-DISTANCE-SENSOR-SHARED-F3-VALIDATION` exists and is limited to shared family scope across `var-sensors-environment-sound-distance-sensor-microphone`, `var-sensors-environment-sound-distance-sensor-ultrasonic`, `var-sensors-environment-sound-distance-sensor-infrared-distance`, `var-sensors-environment-sound-distance-sensor-lidar`, `F3`, and `VALIDATION`.
