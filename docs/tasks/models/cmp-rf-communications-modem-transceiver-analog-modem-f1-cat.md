# CMP-RF-COMMUNICATIONS-MODEM-TRANSCEIVER-ANALOG-MODEM-F1-CAT - Specify Analog Modem catalog preset
...
| Variant | `var-rf-communications-modem-transceiver-analog-modem` - Analog Modem |
| Fidelity | F1 |
| Concern | CAT |
| Release | R7 |
...
Create the complete, immutable catalog definition for **Analog Modem** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-rf-communications-modem-transceiver` (Modem and transceiver); task scope: variant `var-rf-communications-modem-transceiver-analog-modem` (Analog Modem); fidelity `F1`; concern `CAT`.
...
- Canonical source: [Modem and transceiver family-specific implementation reference](../../catalog/families/fam-rf-communications-modem-transceiver.md#family-specific-implementation-reference), registry row `fam-rf-communications-modem-transceiver`, and variant `var-rf-communications-modem-transceiver-analog-modem` (Analog Modem).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Bits/symbols map through the selected modulation, coding, framing, timing, RF/baseband, synchronization, and error state machine.
...
- `docs/tasks/models/cmp-rf-communications-modem-transceiver-analog-modem-f1-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-rf-communications-modem-transceiver-analog-modem` (Analog Modem), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-RF-COMMUNICATIONS-MODEM-TRANSCEIVER-ANALOG-MODEM-F1-CAT`: variant `var-rf-communications-modem-transceiver-analog-modem` (Analog Modem), fidelity `F1`, concern `CAT`, and requirements REQ-011, REQ-012, REQ-024, REQ-028, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-RF-COMMUNICATIONS-MODEM-TRANSCEIVER-ANALOG-MODEM-F1-CAT-NOMINAL`
- `TEST-CMP-RF-COMMUNICATIONS-MODEM-TRANSCEIVER-ANALOG-MODEM-F1-CAT-BOUNDARY`
- `TEST-CMP-RF-COMMUNICATIONS-MODEM-TRANSCEIVER-ANALOG-MODEM-F1-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-RF-COMMUNICATIONS-MODEM-TRANSCEIVER-ANALOG-MODEM-F1-CAT` exists and is limited to variant `var-rf-communications-modem-transceiver-analog-modem` (Analog Modem), `F1`, and `CAT`.
