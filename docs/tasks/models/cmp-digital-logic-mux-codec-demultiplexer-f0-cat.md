# CMP-DIGITAL-LOGIC-MUX-CODEC-DEMULTIPLEXER-F0-CAT - Specify Demultiplexer catalog preset
...
| Variant | `var-digital-logic-mux-codec-demultiplexer` - Demultiplexer |
| Fidelity | F0 |
| Concern | CAT |
| Release | R5 |
...
Create the complete, immutable catalog definition for **Demultiplexer** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-digital-logic-mux-codec` (Multiplexer and code converter); task scope: variant `var-digital-logic-mux-codec-demultiplexer` (Demultiplexer); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Multiplexer and code converter family-specific implementation reference](../../catalog/families/fam-digital-logic-mux-codec.md#family-specific-implementation-reference), registry row `fam-digital-logic-mux-codec`, and variant `var-digital-logic-mux-codec-demultiplexer` (Demultiplexer).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Selection/code inputs choose the declared mux/demux/encoder/decoder conversion; invalid and X/Z codes produce the documented outputs.
...
- `docs/tasks/models/cmp-digital-logic-mux-codec-demultiplexer-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-digital-logic-mux-codec-demultiplexer` (Demultiplexer), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-DIGITAL-LOGIC-MUX-CODEC-DEMULTIPLEXER-F0-CAT`: variant `var-digital-logic-mux-codec-demultiplexer` (Demultiplexer), fidelity `F0`, concern `CAT`, and requirements REQ-018, REQ-021, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-DIGITAL-LOGIC-MUX-CODEC-DEMULTIPLEXER-F0-CAT-NOMINAL`
- `TEST-CMP-DIGITAL-LOGIC-MUX-CODEC-DEMULTIPLEXER-F0-CAT-BOUNDARY`
- `TEST-CMP-DIGITAL-LOGIC-MUX-CODEC-DEMULTIPLEXER-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-DIGITAL-LOGIC-MUX-CODEC-DEMULTIPLEXER-F0-CAT` exists and is limited to variant `var-digital-logic-mux-codec-demultiplexer` (Demultiplexer), `F0`, and `CAT`.
