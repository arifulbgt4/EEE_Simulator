# CMP-PASSIVES-CHOKE-SHARED-F4-MODEL - Implement Choke F4 model
...
Implement one model tier, **F4**, for **Choke** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-passives-choke` (Choke); task scope: shared family scope across `var-passives-choke-common-mode`, `var-passives-choke-differential-mode`; fidelity `F4`; concern `MODEL`.
...
- `docs/tasks/models/cmp-passives-choke-shared-f4-model.md`
...
- One `ModelBinding` for `fam-passives-choke` at `F4` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-PASSIVES-CHOKE-SHARED-F4-MODEL`: shared family scope across `var-passives-choke-common-mode`, `var-passives-choke-differential-mode`, fidelity `F4`, concern `MODEL`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-CHOKE-SHARED-F4-MODEL-NOMINAL`
- `TEST-CMP-PASSIVES-CHOKE-SHARED-F4-MODEL-BOUNDARY`
- `TEST-CMP-PASSIVES-CHOKE-SHARED-F4-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-CHOKE-SHARED-F4-MODEL` exists and is limited to shared family scope across `var-passives-choke-common-mode`, `var-passives-choke-differential-mode`, `F4`, and `MODEL`.
