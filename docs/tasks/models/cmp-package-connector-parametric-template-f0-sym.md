# CMP-PACKAGE-CONNECTOR-PARAMETRIC-TEMPLATE-F0-SYM - Create Parametric connector physical renderer
...
|---|---|
| Status | Planned |
| Family | `pkg-connector-parametric` - Parametric connector |
| Variant | `template` |
| Fidelity | F0 |
| Concern | SYM |
| Release | R1 |
...
Create recognizable original procedural geometry at all required levels of detail with non-color orientation cues.

## Exact prerequisites

- `CMP-PACKAGE-CONNECTOR-PARAMETRIC-TEMPLATE-F0-CAT`
- `PLAT-SYM-007`
...
- Aliases: Parametric connector
- Pins: family-defined parameterized pins
- Parameters: none
- Supported analyses: physical-view, pin-map-validation
- Package mappings: pkg-connector-parametric
- Golden references: GRC-054
...
- Package registry entry `pkg-connector-parametric` revision `1.0.0` is the only geometry/default source for this task.
- Concern-specific golden references: GRC-054. `NUMERICAL_ACCURACY_TARGETS.md` defines comparison and evidence rules.
- This concern has no electrical model equation; geometry and numbering use the declarative package constraints only.

## Allowed files

- `docs/tasks/models/cmp-package-connector-parametric-template-f0-sym.md`
...
- `TEST-CMP-PACKAGE-CONNECTOR-PARAMETRIC-TEMPLATE-F0-SYM-NOMINAL`
- `TEST-CMP-PACKAGE-CONNECTOR-PARAMETRIC-TEMPLATE-F0-SYM-BOUNDARY`
- `TEST-CMP-PACKAGE-CONNECTOR-PARAMETRIC-TEMPLATE-F0-SYM-FAILURE`
