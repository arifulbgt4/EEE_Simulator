# CMP-ACTUATORS-HMI-SERVO-SHARED-F4-MODEL - Implement Servo F4 model
...
Implement one model tier, **F4**, for **Servo** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-actuators-hmi-servo` (Servo); task scope: shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`; fidelity `F4`; concern `MODEL`.
...
- `docs/tasks/models/cmp-actuators-hmi-servo-shared-f4-model.md`
...
- One `ModelBinding` for `fam-actuators-hmi-servo` at `F4` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-ACTUATORS-HMI-SERVO-SHARED-F4-MODEL`: shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`, fidelity `F4`, concern `MODEL`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F4-MODEL-NOMINAL`
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F4-MODEL-BOUNDARY`
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F4-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-ACTUATORS-HMI-SERVO-SHARED-F4-MODEL` exists and is limited to shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`, `F4`, and `MODEL`.
