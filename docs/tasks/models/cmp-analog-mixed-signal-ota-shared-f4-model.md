# CMP-ANALOG-MIXED-SIGNAL-OTA-SHARED-F4-MODEL - Implement Operational transconductance amplifier F4 model
...
Implement one model tier, **F4**, for **Operational transconductance amplifier** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-analog-mixed-signal-ota` (Operational transconductance amplifier); task scope: shared family scope across `var-analog-mixed-signal-ota-single-ended`, `var-analog-mixed-signal-ota-differential`; fidelity `F4`; concern `MODEL`.
...
- Exact F4 execution rule: Electrothermal/tolerance/failure tier: extend the lower-tier relation with declared sampling, power-to-heat state Cth*dT/dt = P-(T-Tamb)/Rth, derating, and deterministic failure transitions; base relation: Output current is Iout = gm*(Vplus-Vminus), bounded by bias, compliance, bandwidth, slew, and supply limits.
...
- `docs/tasks/models/cmp-analog-mixed-signal-ota-shared-f4-model.md`
...
- One `ModelBinding` for `fam-analog-mixed-signal-ota` at `F4` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-ANALOG-MIXED-SIGNAL-OTA-SHARED-F4-MODEL`: shared family scope across `var-analog-mixed-signal-ota-single-ended`, `var-analog-mixed-signal-ota-differential`, fidelity `F4`, concern `MODEL`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-018, REQ-020, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ANALOG-MIXED-SIGNAL-OTA-SHARED-F4-MODEL-NOMINAL`
- `TEST-CMP-ANALOG-MIXED-SIGNAL-OTA-SHARED-F4-MODEL-BOUNDARY`
- `TEST-CMP-ANALOG-MIXED-SIGNAL-OTA-SHARED-F4-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-ANALOG-MIXED-SIGNAL-OTA-SHARED-F4-MODEL` exists and is limited to shared family scope across `var-analog-mixed-signal-ota-single-ended`, `var-analog-mixed-signal-ota-differential`, `F4`, and `MODEL`.
