# CMP-PASSIVES-CHOKE-SHARED-F1-VALIDATION - Validate Choke F1
...
Produce independent validation evidence for **Choke** at **F1** without modifying the model under test.
...
- Stable family: `fam-passives-choke` (Choke); task scope: shared family scope across `var-passives-choke-common-mode`, `var-passives-choke-differential-mode`; fidelity `F1`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F1 rule: Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: Each winding follows v = d(lambda)/dt with the declared differential/common-mode inductance matrix, winding loss, and saturation envelope.
...
- `docs/tasks/models/cmp-passives-choke-shared-f1-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-PAS-CHOKE-NOMINAL`, `GOLD-PAS-CHOKE-BOUNDARY`, `GOLD-PAS-CHOKE-FAILURE` at `F1`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-PASSIVES-CHOKE-SHARED-F1-VALIDATION`: shared family scope across `var-passives-choke-common-mode`, `var-passives-choke-differential-mode`, fidelity `F1`, concern `VALIDATION`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-CHOKE-SHARED-F1-VALIDATION-NOMINAL`
- `TEST-CMP-PASSIVES-CHOKE-SHARED-F1-VALIDATION-BOUNDARY`
- `TEST-CMP-PASSIVES-CHOKE-SHARED-F1-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-CHOKE-SHARED-F1-VALIDATION` exists and is limited to shared family scope across `var-passives-choke-common-mode`, `var-passives-choke-differential-mode`, `F1`, and `VALIDATION`.
