# CMP-PASSIVES-RESISTOR-ARRAY-ISOLATED-F0-CAT - Specify Isolated catalog preset
...
| Variant | `var-passives-resistor-array-isolated` - Isolated |
| Fidelity | F0 |
| Concern | CAT |
| Release | R2 |
...
Create the complete, immutable catalog definition for **Isolated** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-passives-resistor-array` (Resistor array); task scope: variant `var-passives-resistor-array-isolated` (Isolated); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Resistor array family-specific implementation reference](../../catalog/families/fam-passives-resistor-array.md#family-specific-implementation-reference), registry row `fam-passives-resistor-array`, and variant `var-passives-resistor-array-isolated` (Isolated).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Each declared resistor branch obeys v_k = R_k*i_k; shared-terminal and matching/correlation rules come only from the selected array topology.
...
- `docs/tasks/models/cmp-passives-resistor-array-isolated-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-passives-resistor-array-isolated` (Isolated), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-PASSIVES-RESISTOR-ARRAY-ISOLATED-F0-CAT`: variant `var-passives-resistor-array-isolated` (Isolated), fidelity `F0`, concern `CAT`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-RESISTOR-ARRAY-ISOLATED-F0-CAT-NOMINAL`
- `TEST-CMP-PASSIVES-RESISTOR-ARRAY-ISOLATED-F0-CAT-BOUNDARY`
- `TEST-CMP-PASSIVES-RESISTOR-ARRAY-ISOLATED-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-RESISTOR-ARRAY-ISOLATED-F0-CAT` exists and is limited to variant `var-passives-resistor-array-isolated` (Isolated), `F0`, and `CAT`.
