# CMP-SEMICONDUCTORS-OPTO-ISOLATOR-SHARED-F0-VALIDATION - Validate Optically isolated semiconductor F0
...
Produce independent validation evidence for **Optically isolated semiconductor** at **F0** without modifying the model under test.
...
- Stable family: `fam-semiconductors-opto-isolator` (Optically isolated semiconductor); task scope: shared family scope across `var-semiconductors-opto-isolator-phototransistor-output`, `var-semiconductors-opto-isolator-triac-output`; fidelity `F0`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F0 rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Input diode current produces output photocurrent or logic transfer through declared CTR, delay, saturation, isolation leakage, and temperature behavior.
...
- `docs/tasks/models/cmp-semiconductors-opto-isolator-shared-f0-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SEM-OPTO_ISOLATOR-NOMINAL`, `GOLD-SEM-OPTO_ISOLATOR-BOUNDARY`, `GOLD-SEM-OPTO_ISOLATOR-FAILURE` at `F0`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-SEMICONDUCTORS-OPTO-ISOLATOR-SHARED-F0-VALIDATION`: shared family scope across `var-semiconductors-opto-isolator-phototransistor-output`, `var-semiconductors-opto-isolator-triac-output`, fidelity `F0`, concern `VALIDATION`, and requirements REQ-009, REQ-010, REQ-011, REQ-012, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SEMICONDUCTORS-OPTO-ISOLATOR-SHARED-F0-VALIDATION-NOMINAL`
- `TEST-CMP-SEMICONDUCTORS-OPTO-ISOLATOR-SHARED-F0-VALIDATION-BOUNDARY`
- `TEST-CMP-SEMICONDUCTORS-OPTO-ISOLATOR-SHARED-F0-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SEMICONDUCTORS-OPTO-ISOLATOR-SHARED-F0-VALIDATION` exists and is limited to shared family scope across `var-semiconductors-opto-isolator-phototransistor-output`, `var-semiconductors-opto-isolator-triac-output`, `F0`, and `VALIDATION`.
