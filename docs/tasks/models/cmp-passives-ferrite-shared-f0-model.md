# CMP-PASSIVES-FERRITE-SHARED-F0-MODEL - Implement Ferrite suppressor F0 model
...
Implement one model tier, **F0**, for **Ferrite suppressor** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-passives-ferrite` (Ferrite suppressor); task scope: shared family scope across `var-passives-ferrite-bead`; fidelity `F0`; concern `MODEL`.
...
- Exact F0 execution rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The branch uses the declared frequency-dependent complex impedance Z(f,T,bias); time-domain use requires an approved causal fitted model.
...
- `docs/tasks/models/cmp-passives-ferrite-shared-f0-model.md`
...
- One `ModelBinding` for `fam-passives-ferrite` at `F0` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-PASSIVES-FERRITE-SHARED-F0-MODEL`: shared family scope across `var-passives-ferrite-bead`, fidelity `F0`, concern `MODEL`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-FERRITE-SHARED-F0-MODEL-NOMINAL`
- `TEST-CMP-PASSIVES-FERRITE-SHARED-F0-MODEL-BOUNDARY`
- `TEST-CMP-PASSIVES-FERRITE-SHARED-F0-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-FERRITE-SHARED-F0-MODEL` exists and is limited to shared family scope across `var-passives-ferrite-bead`, `F0`, and `MODEL`.
