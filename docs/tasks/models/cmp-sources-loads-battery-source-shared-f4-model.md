# CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F4-MODEL - Implement Battery source F4 model
...
Implement one model tier, **F4**, for **Battery source** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-sources-loads-battery-source` (Battery source); task scope: shared family scope across `var-sources-loads-battery-source-battery-pack`; fidelity `F4`; concern `MODEL`.
...
- `docs/tasks/models/cmp-sources-loads-battery-source-shared-f4-model.md`
...
- One `ModelBinding` for `fam-sources-loads-battery-source` at `F4` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F4-MODEL`: shared family scope across `var-sources-loads-battery-source-battery-pack`, fidelity `F4`, concern `MODEL`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F4-MODEL-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F4-MODEL-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F4-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-BATTERY-SOURCE-SHARED-F4-MODEL` exists and is limited to shared family scope across `var-sources-loads-battery-source-battery-pack`, `F4`, and `MODEL`.
