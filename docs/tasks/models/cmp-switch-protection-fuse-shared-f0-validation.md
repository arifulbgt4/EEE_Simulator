# CMP-SWITCH-PROTECTION-FUSE-SHARED-F0-VALIDATION - Validate Fuse and resettable protector F0
...
Produce independent validation evidence for **Fuse and resettable protector** at **F0** without modifying the model under test.
...
- Stable family: `fam-switch-protection-fuse` (Fuse and resettable protector); task scope: shared family scope across `var-switch-protection-fuse-fast-blow`, `var-switch-protection-fuse-slow-blow`, `var-switch-protection-fuse-ptc-resettable`; fidelity `F0`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F0 rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The element integrates the declared I^2*t or thermal damage measure and transitions irreversibly or resetably to its open/high-resistance state at threshold.
...
- `docs/tasks/models/cmp-switch-protection-fuse-shared-f0-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SWP-FUSE-NOMINAL`, `GOLD-SWP-FUSE-BOUNDARY`, `GOLD-SWP-FUSE-FAILURE` at `F0`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-SWITCH-PROTECTION-FUSE-SHARED-F0-VALIDATION`: shared family scope across `var-switch-protection-fuse-fast-blow`, `var-switch-protection-fuse-slow-blow`, `var-switch-protection-fuse-ptc-resettable`, fidelity `F0`, concern `VALIDATION`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SWITCH-PROTECTION-FUSE-SHARED-F0-VALIDATION-NOMINAL`
- `TEST-CMP-SWITCH-PROTECTION-FUSE-SHARED-F0-VALIDATION-BOUNDARY`
- `TEST-CMP-SWITCH-PROTECTION-FUSE-SHARED-F0-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SWITCH-PROTECTION-FUSE-SHARED-F0-VALIDATION` exists and is limited to shared family scope across `var-switch-protection-fuse-fast-blow`, `var-switch-protection-fuse-slow-blow`, `var-switch-protection-fuse-ptc-resettable`, `F0`, and `VALIDATION`.
