# CMP-PASSIVES-PHOTORESISTOR-SHARED-F3-MODEL - Implement Photoresistor F3 model
...
Implement one model tier, **F3**, for **Photoresistor** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-passives-photoresistor` (Photoresistor); task scope: shared family scope across `var-passives-photoresistor-ldr`; fidelity `F3`; concern `MODEL`.
...
- Exact F3 execution rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Resistance is the declared bounded transfer R = f(illuminance,T) with response-time state; zero or out-of-range stimulus uses explicit saturation behavior.
...
- `docs/tasks/models/cmp-passives-photoresistor-shared-f3-model.md`
...
- One `ModelBinding` for `fam-passives-photoresistor` at `F3` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-PASSIVES-PHOTORESISTOR-SHARED-F3-MODEL`: shared family scope across `var-passives-photoresistor-ldr`, fidelity `F3`, concern `MODEL`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-PHOTORESISTOR-SHARED-F3-MODEL-NOMINAL`
- `TEST-CMP-PASSIVES-PHOTORESISTOR-SHARED-F3-MODEL-BOUNDARY`
- `TEST-CMP-PASSIVES-PHOTORESISTOR-SHARED-F3-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-PHOTORESISTOR-SHARED-F3-MODEL` exists and is limited to shared family scope across `var-passives-photoresistor-ldr`, `F3`, and `MODEL`.
