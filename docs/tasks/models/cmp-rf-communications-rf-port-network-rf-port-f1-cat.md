# CMP-RF-COMMUNICATIONS-RF-PORT-NETWORK-RF-PORT-F1-CAT - Specify Rf Port catalog preset
...
| Variant | `var-rf-communications-rf-port-network-rf-port` - Rf Port |
| Fidelity | F1 |
| Concern | CAT |
| Release | R7 |
...
Create the complete, immutable catalog definition for **Rf Port** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-rf-communications-rf-port-network` (RF port and network model); task scope: variant `var-rf-communications-rf-port-network-rf-port` (Rf Port); fidelity `F1`; concern `CAT`.
...
- Canonical source: [RF port and network model family-specific implementation reference](../../catalog/families/fam-rf-communications-rf-port-network.md#family-specific-implementation-reference), registry row `fam-rf-communications-rf-port-network`, and variant `var-rf-communications-rf-port-network-rf-port` (Rf Port).
...
- `docs/tasks/models/cmp-rf-communications-rf-port-network-rf-port-f1-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-rf-communications-rf-port-network-rf-port` (Rf Port), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-RF-COMMUNICATIONS-RF-PORT-NETWORK-RF-PORT-F1-CAT`: variant `var-rf-communications-rf-port-network-rf-port` (Rf Port), fidelity `F1`, concern `CAT`, and requirements REQ-011, REQ-012, REQ-024, REQ-028, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-RF-COMMUNICATIONS-RF-PORT-NETWORK-RF-PORT-F1-CAT-NOMINAL`
- `TEST-CMP-RF-COMMUNICATIONS-RF-PORT-NETWORK-RF-PORT-F1-CAT-BOUNDARY`
- `TEST-CMP-RF-COMMUNICATIONS-RF-PORT-NETWORK-RF-PORT-F1-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-RF-COMMUNICATIONS-RF-PORT-NETWORK-RF-PORT-F1-CAT` exists and is limited to variant `var-rf-communications-rf-port-network-rf-port` (Rf Port), `F1`, and `CAT`.
