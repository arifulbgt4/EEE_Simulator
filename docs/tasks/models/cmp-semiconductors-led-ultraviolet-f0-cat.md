# CMP-SEMICONDUCTORS-LED-ULTRAVIOLET-F0-CAT - Specify Ultraviolet catalog preset
...
| Variant | `var-semiconductors-led-ultraviolet` - Ultraviolet |
| Fidelity | F0 |
| Concern | CAT |
| Release | R3 |
...
Create the complete, immutable catalog definition for **Ultraviolet** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-semiconductors-led` (Light-emitting diode); task scope: variant `var-semiconductors-led-ultraviolet` (Ultraviolet); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Light-emitting diode family-specific implementation reference](../../catalog/families/fam-semiconductors-led.md#family-specific-implementation-reference), registry row `fam-semiconductors-led`, and variant `var-semiconductors-led-ultraviolet` (Ultraviolet).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: Electrical current follows the diode relation and optical output follows the declared efficiency/spectrum transfer above threshold, subject to thermal derating.
...
- `docs/tasks/models/cmp-semiconductors-led-ultraviolet-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-semiconductors-led-ultraviolet` (Ultraviolet), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-SEMICONDUCTORS-LED-ULTRAVIOLET-F0-CAT`: variant `var-semiconductors-led-ultraviolet` (Ultraviolet), fidelity `F0`, concern `CAT`, and requirements REQ-009, REQ-010, REQ-011, REQ-012, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SEMICONDUCTORS-LED-ULTRAVIOLET-F0-CAT-NOMINAL`
- `TEST-CMP-SEMICONDUCTORS-LED-ULTRAVIOLET-F0-CAT-BOUNDARY`
- `TEST-CMP-SEMICONDUCTORS-LED-ULTRAVIOLET-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-SEMICONDUCTORS-LED-ULTRAVIOLET-F0-CAT` exists and is limited to variant `var-semiconductors-led-ultraviolet` (Ultraviolet), `F0`, and `CAT`.
