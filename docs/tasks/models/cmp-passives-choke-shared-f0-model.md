# CMP-PASSIVES-CHOKE-SHARED-F0-MODEL - Implement Choke F0 model
...
Implement one model tier, **F0**, for **Choke** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-passives-choke` (Choke); task scope: shared family scope across `var-passives-choke-common-mode`, `var-passives-choke-differential-mode`; fidelity `F0`; concern `MODEL`.
...
- Exact F0 execution rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Each winding follows v = d(lambda)/dt with the declared differential/common-mode inductance matrix, winding loss, and saturation envelope.
...
- `docs/tasks/models/cmp-passives-choke-shared-f0-model.md`
...
- One `ModelBinding` for `fam-passives-choke` at `F0` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-PASSIVES-CHOKE-SHARED-F0-MODEL`: shared family scope across `var-passives-choke-common-mode`, `var-passives-choke-differential-mode`, fidelity `F0`, concern `MODEL`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-CHOKE-SHARED-F0-MODEL-NOMINAL`
- `TEST-CMP-PASSIVES-CHOKE-SHARED-F0-MODEL-BOUNDARY`
- `TEST-CMP-PASSIVES-CHOKE-SHARED-F0-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-CHOKE-SHARED-F0-MODEL` exists and is limited to shared family scope across `var-passives-choke-common-mode`, `var-passives-choke-differential-mode`, `F0`, and `MODEL`.
