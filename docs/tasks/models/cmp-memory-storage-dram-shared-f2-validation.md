# CMP-MEMORY-STORAGE-DRAM-SHARED-F2-VALIDATION - Validate Dynamic RAM F2
...
Produce independent validation evidence for **Dynamic RAM** at **F2** without modifying the model under test.
...
- Stable family: `fam-memory-storage-dram` (Dynamic RAM); task scope: shared family scope across `var-memory-storage-dram-sdram`, `var-memory-storage-dram-ddr`, `var-memory-storage-dram-mobile-dram`; fidelity `F2`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F2 rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: A bank/row/column state model enforces activate/read/write/precharge/refresh timing, retention, initialization, and conflict rules.
...
- `docs/tasks/models/cmp-memory-storage-dram-shared-f2-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-MEM-DRAM-NOMINAL`, `GOLD-MEM-DRAM-BOUNDARY`, `GOLD-MEM-DRAM-FAILURE` at `F2`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-MEMORY-STORAGE-DRAM-SHARED-F2-VALIDATION`: shared family scope across `var-memory-storage-dram-sdram`, `var-memory-storage-dram-ddr`, `var-memory-storage-dram-mobile-dram`, fidelity `F2`, concern `VALIDATION`, and requirements REQ-018, REQ-021, REQ-025, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MEMORY-STORAGE-DRAM-SHARED-F2-VALIDATION-NOMINAL`
- `TEST-CMP-MEMORY-STORAGE-DRAM-SHARED-F2-VALIDATION-BOUNDARY`
- `TEST-CMP-MEMORY-STORAGE-DRAM-SHARED-F2-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-MEMORY-STORAGE-DRAM-SHARED-F2-VALIDATION` exists and is limited to shared family scope across `var-memory-storage-dram-sdram`, `var-memory-storage-dram-ddr`, `var-memory-storage-dram-mobile-dram`, `F2`, and `VALIDATION`.
