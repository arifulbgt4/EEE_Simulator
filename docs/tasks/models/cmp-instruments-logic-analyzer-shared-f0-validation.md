# CMP-INSTRUMENTS-LOGIC-ANALYZER-SHARED-F0-VALIDATION - Validate Logic analyzer F0
...
Produce independent validation evidence for **Logic analyzer** at **F0** without modifying the model under test.
...
- Stable family: `fam-instruments-logic-analyzer` (Logic analyzer); task scope: shared family scope across `var-instruments-logic-analyzer-eight-channel`, `var-instruments-logic-analyzer-sixteen-channel`; fidelity `F0`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F0 rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Channels resolve sampled thresholds to 0/1/X/Z and capture deterministic timestamped events under declared trigger and memory rules.
...
- `docs/tasks/models/cmp-instruments-logic-analyzer-shared-f0-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-INS-LOGIC_ANALYZER-NOMINAL`, `GOLD-INS-LOGIC_ANALYZER-BOUNDARY`, `GOLD-INS-LOGIC_ANALYZER-FAILURE` at `F0`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-INSTRUMENTS-LOGIC-ANALYZER-SHARED-F0-VALIDATION`: shared family scope across `var-instruments-logic-analyzer-eight-channel`, `var-instruments-logic-analyzer-sixteen-channel`, fidelity `F0`, concern `VALIDATION`, and requirements REQ-007, REQ-021, REQ-022, REQ-023, REQ-034, REQ-037.
...
- `TEST-CMP-INSTRUMENTS-LOGIC-ANALYZER-SHARED-F0-VALIDATION-NOMINAL`
- `TEST-CMP-INSTRUMENTS-LOGIC-ANALYZER-SHARED-F0-VALIDATION-BOUNDARY`
- `TEST-CMP-INSTRUMENTS-LOGIC-ANALYZER-SHARED-F0-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-INSTRUMENTS-LOGIC-ANALYZER-SHARED-F0-VALIDATION` exists and is limited to shared family scope across `var-instruments-logic-analyzer-eight-channel`, `var-instruments-logic-analyzer-sixteen-channel`, `F0`, and `VALIDATION`.
