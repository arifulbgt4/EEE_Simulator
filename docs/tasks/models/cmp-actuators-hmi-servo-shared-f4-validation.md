# CMP-ACTUATORS-HMI-SERVO-SHARED-F4-VALIDATION - Validate Servo F4
...
Produce independent validation evidence for **Servo** at **F4** without modifying the model under test.
...
- Stable family: `fam-actuators-hmi-servo` (Servo); task scope: shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`; fidelity `F4`; concern `VALIDATION`.
...
- `docs/tasks/models/cmp-actuators-hmi-servo-shared-f4-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-ACT-SERVO-NOMINAL`, `GOLD-ACT-SERVO-BOUNDARY`, `GOLD-ACT-SERVO-FAILURE` at `F4`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-ACTUATORS-HMI-SERVO-SHARED-F4-VALIDATION`: shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`, fidelity `F4`, concern `VALIDATION`, and requirements REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F4-VALIDATION-NOMINAL`
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F4-VALIDATION-BOUNDARY`
- `TEST-CMP-ACTUATORS-HMI-SERVO-SHARED-F4-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-ACTUATORS-HMI-SERVO-SHARED-F4-VALIDATION` exists and is limited to shared family scope across `var-actuators-hmi-servo-positional`, `var-actuators-hmi-servo-continuous-rotation`, `F4`, and `VALIDATION`.
