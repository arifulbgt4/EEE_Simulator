# CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-PROFILED-LOAD-F0-CAT - Specify Profiled Load catalog preset
...
| Variant | `var-sources-loads-programmable-load-profiled-load` - Profiled Load |
| Fidelity | F0 |
| Concern | CAT |
| Release | R2 |
...
Create the complete, immutable catalog definition for **Profiled Load** without implementing a different fidelity or sibling preset.
...
- Stable family: `fam-sources-loads-programmable-load` (Programmable load); task scope: variant `var-sources-loads-programmable-load-profiled-load` (Profiled Load); fidelity `F0`; concern `CAT`.
...
- Canonical source: [Programmable load family-specific implementation reference](../../catalog/families/fam-sources-loads-programmable-load.md#family-specific-implementation-reference), registry row `fam-sources-loads-programmable-load`, and variant `var-sources-loads-programmable-load-profiled-load` (Profiled Load).
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: The selected R/I/P setpoint is a deterministic function of time or control input with declared slew, compliance, and low-voltage behavior.
...
- `docs/tasks/models/cmp-sources-loads-programmable-load-profiled-load-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-sources-loads-programmable-load-profiled-load` (Profiled Load), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-PROFILED-LOAD-F0-CAT`: variant `var-sources-loads-programmable-load-profiled-load` (Profiled Load), fidelity `F0`, concern `CAT`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-PROFILED-LOAD-F0-CAT-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-PROFILED-LOAD-F0-CAT-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-PROFILED-LOAD-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-PROFILED-LOAD-F0-CAT` exists and is limited to variant `var-sources-loads-programmable-load-profiled-load` (Profiled Load), `F0`, and `CAT`.
