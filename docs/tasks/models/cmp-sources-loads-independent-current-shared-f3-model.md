# CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F3-MODEL - Implement Independent current source F3 model
...
Implement one model tier, **F3**, for **Independent current source** using the family equations/behavior and no higher-fidelity claims.
...
- Exact F3 execution rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: The oriented branch current is i(P->N) = I(t), with I(t) supplied by the selected DC/AC/transient profile.
...
- `docs/tasks/models/cmp-sources-loads-independent-current-shared-f3-model.md`
...
- One `ModelBinding` for `fam-sources-loads-independent-current` at `F3` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F3-MODEL`: shared family scope across `var-sources-loads-independent-current-dc`, `var-sources-loads-independent-current-ac-small-signal`, `var-sources-loads-independent-current-sine`, `var-sources-loads-independent-current-pulse`, fidelity `F3`, concern `MODEL`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F3-MODEL-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F3-MODEL-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F3-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-INDEPENDENT-CURRENT-SHARED-F3-MODEL` exists and is limited to shared family scope across `var-sources-loads-independent-current-dc`, `var-sources-loads-independent-current-ac-small-signal`, `var-sources-loads-independent-current-sine`, `var-sources-loads-independent-current-pulse`, `F3`, and `MODEL`.
