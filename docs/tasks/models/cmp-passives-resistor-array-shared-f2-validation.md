# CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F2-VALIDATION - Validate Resistor array F2
...
Produce independent validation evidence for **Resistor array** at **F2** without modifying the model under test.
...
- Stable family: `fam-passives-resistor-array` (Resistor array); task scope: shared family scope across `var-passives-resistor-array-isolated`, `var-passives-resistor-array-bussed`; fidelity `F2`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F2 rule: Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: Each declared resistor branch obeys v_k = R_k*i_k; shared-terminal and matching/correlation rules come only from the selected array topology.
...
- `docs/tasks/models/cmp-passives-resistor-array-shared-f2-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-PAS-RESISTOR_ARRAY-NOMINAL`, `GOLD-PAS-RESISTOR_ARRAY-BOUNDARY`, `GOLD-PAS-RESISTOR_ARRAY-FAILURE` at `F2`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F2-VALIDATION`: shared family scope across `var-passives-resistor-array-isolated`, `var-passives-resistor-array-bussed`, fidelity `F2`, concern `VALIDATION`, and requirements REQ-009, REQ-010, REQ-011, REQ-014, REQ-015, REQ-016, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F2-VALIDATION-NOMINAL`
- `TEST-CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F2-VALIDATION-BOUNDARY`
- `TEST-CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F2-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-PASSIVES-RESISTOR-ARRAY-SHARED-F2-VALIDATION` exists and is limited to shared family scope across `var-passives-resistor-array-isolated`, `var-passives-resistor-array-bussed`, `F2`, and `VALIDATION`.
