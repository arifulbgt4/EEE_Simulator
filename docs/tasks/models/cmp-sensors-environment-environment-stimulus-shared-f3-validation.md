# CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-SHARED-F3-VALIDATION - Validate Environmental stimulus F3
...
Produce independent validation evidence for **Environmental stimulus** at **F3** without modifying the model under test.
...
- Stable family: `fam-sensors-environment-environment-stimulus` (Environmental stimulus); task scope: shared family scope across `var-sensors-environment-environment-stimulus-constant-stimulus`, `var-sensors-environment-environment-stimulus-time-profile`; fidelity `F3`; concern `VALIDATION`.
...
- Candidate and independent reference must evaluate the same F3 rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: The primitive emits a deterministic SI-valued environmental field/time profile; interpolation, spatial scope, discontinuities, and seed are explicit.
...
- `docs/tasks/models/cmp-sensors-environment-environment-stimulus-shared-f3-validation.md`
...
- One immutable candidate/reference evidence bundle for `GOLD-SEN-ENVIRONMENT_STIMULUS-NOMINAL`, `GOLD-SEN-ENVIRONMENT_STIMULUS-BOUNDARY`, `GOLD-SEN-ENVIRONMENT_STIMULUS-FAILURE` at `F3`, containing exact inputs, expected outputs, tolerance, versions, seed, and raw-result digests.
...
- Scope is limited to `CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-SHARED-F3-VALIDATION`: shared family scope across `var-sensors-environment-environment-stimulus-constant-stimulus`, `var-sensors-environment-environment-stimulus-time-profile`, fidelity `F3`, concern `VALIDATION`, and requirements REQ-014, REQ-015, REQ-018, REQ-020, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-SHARED-F3-VALIDATION-NOMINAL`
- `TEST-CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-SHARED-F3-VALIDATION-BOUNDARY`
- `TEST-CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-SHARED-F3-VALIDATION-FAILURE`

## Acceptance

1. The exact output for `CMP-SENSORS-ENVIRONMENT-ENVIRONMENT-STIMULUS-SHARED-F3-VALIDATION` exists and is limited to shared family scope across `var-sensors-environment-environment-stimulus-constant-stimulus`, `var-sensors-environment-environment-stimulus-time-profile`, `F3`, and `VALIDATION`.
