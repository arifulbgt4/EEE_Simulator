# CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F0-MODEL - Implement Programmable load F0 model
...
Implement one model tier, **F0**, for **Programmable load** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-sources-loads-programmable-load` (Programmable load); task scope: shared family scope across `var-sources-loads-programmable-load-profiled-load`; fidelity `F0`; concern `MODEL`.
...
- Exact F0 execution rule: Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The selected R/I/P setpoint is a deterministic function of time or control input with declared slew, compliance, and low-voltage behavior.
...
- `docs/tasks/models/cmp-sources-loads-programmable-load-shared-f0-model.md`
...
- One `ModelBinding` for `fam-sources-loads-programmable-load` at `F0` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F0-MODEL`: shared family scope across `var-sources-loads-programmable-load-profiled-load`, fidelity `F0`, concern `MODEL`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F0-MODEL-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F0-MODEL-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F0-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-PROGRAMMABLE-LOAD-SHARED-F0-MODEL` exists and is limited to shared family scope across `var-sources-loads-programmable-load-profiled-load`, `F0`, and `MODEL`.
