# CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F3-MODEL - Implement Digital pattern source F3 model
...
Implement one model tier, **F3**, for **Digital pattern source** using the family equations/behavior and no higher-fidelity claims.
...
- Stable family: `fam-sources-loads-digital-pattern-source` (Digital pattern source); task scope: shared family scope across `var-sources-loads-digital-pattern-source-pattern`; fidelity `F3`; concern `MODEL`.
...
- Exact F3 execution rule: Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: The source emits the declared 0/1/X/Z vector at integer ticks; simultaneous changes use the scheduler tie order and never depend on host scheduling.
...
- `docs/tasks/models/cmp-sources-loads-digital-pattern-source-shared-f3-model.md`
...
- One `ModelBinding` for `fam-sources-loads-digital-pattern-source` at `F3` implementing the exact tier rule above, its initialization/state/stamp/event behavior, power reporting, and structured diagnostics.
...
- Scope is limited to `CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F3-MODEL`: shared family scope across `var-sources-loads-digital-pattern-source-pattern`, fidelity `F3`, concern `MODEL`, and requirements REQ-008, REQ-009, REQ-010, REQ-022, REQ-023, REQ-037.
...
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F3-MODEL-NOMINAL`
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F3-MODEL-BOUNDARY`
- `TEST-CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F3-MODEL-FAILURE`

## Acceptance

1. The exact output for `CMP-SOURCES-LOADS-DIGITAL-PATTERN-SOURCE-SHARED-F3-MODEL` exists and is limited to shared family scope across `var-sources-loads-digital-pattern-source-pattern`, `F3`, and `MODEL`.
