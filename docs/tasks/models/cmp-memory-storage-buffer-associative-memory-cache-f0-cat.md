# CMP-MEMORY-STORAGE-BUFFER-ASSOCIATIVE-MEMORY-CACHE-F0-CAT - Specify Cache catalog preset
...
| Variant | `var-memory-storage-buffer-associative-memory-cache` - Cache |
| Fidelity | F0 |
| Concern | CAT |
| Release | R8 |
...
- Stable family: `fam-memory-storage-buffer-associative-memory` (Buffer and associative memory); task scope: variant `var-memory-storage-buffer-associative-memory-cache` (Cache); fidelity `F0`; concern `CAT`.
...
- This catalog concern has no numerical equation. It freezes the selected variant's inherited/overridden fields against the governing family rule: FIFO/dual-port/CAM/cache state updates use explicit enqueue/dequeue, port arbitration, tag match/replacement, capacity, and timing rules.
...
- `docs/tasks/models/cmp-memory-storage-buffer-associative-memory-cache-f0-cat.md`
...
- One normalized `ComponentVariant` record for variant `var-memory-storage-buffer-associative-memory-cache` (Cache), retaining stable ID, aliases, inherited pins/parameters, declared tier list, package candidates, release target, provenance, and limitations.
...
- Scope is limited to `CMP-MEMORY-STORAGE-BUFFER-ASSOCIATIVE-MEMORY-CACHE-F0-CAT`: variant `var-memory-storage-buffer-associative-memory-cache` (Cache), fidelity `F0`, concern `CAT`, and requirements REQ-018, REQ-021, REQ-025, REQ-022, REQ-023, REQ-037, REQ-038.
...
- `TEST-CMP-MEMORY-STORAGE-BUFFER-ASSOCIATIVE-MEMORY-CACHE-F0-CAT-NOMINAL`
- `TEST-CMP-MEMORY-STORAGE-BUFFER-ASSOCIATIVE-MEMORY-CACHE-F0-CAT-BOUNDARY`
- `TEST-CMP-MEMORY-STORAGE-BUFFER-ASSOCIATIVE-MEMORY-CACHE-F0-CAT-FAILURE`

## Acceptance

1. The exact output for `CMP-MEMORY-STORAGE-BUFFER-ASSOCIATIVE-MEMORY-CACHE-F0-CAT` exists and is limited to variant `var-memory-storage-buffer-associative-memory-cache` (Cache), `F0`, and `CAT`.
