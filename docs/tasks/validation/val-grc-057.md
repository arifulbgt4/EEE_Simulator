# VAL-GRC-057 - Validate Physical RL transient
...
|---|---|
| Status | Planned |
| Target | Physical RL transient |
| Requirements | REQ-039, REQ-040, REQ-041, REQ-042, REQ-043, REQ-044, REQ-045, REQ-046, REQ-047 |
| Golden fixture | GRC-057 |
| Physical benchmark | PBC-003 |
| Test ID | TEST-GRC-057 |
| Release | R4 |
...
Produce independent, reproducible physical-to-simulation correlation evidence for **Physical RL transient** without changing the model, solver, instrument, or feature under test.
...
- `docs/tasks/validation/val-grc-057.md`
...
- Primary assertion: Measured current rise, winding resistance, source limit, probe bandwidth and uncertainty.
- `GRC_057_NOMINAL`: candidate output is compared with retained physical measurements using the declared metric, interval, sample alignment, and category-specific envelope.
- `GRC_057_BOUNDARY`: minimum/maximum declared operating points, environmental bounds, instrument range, source limit, and applicable tolerance corners are classified explicitly.
- `GRC_057_DIMENSION_INVALID`: incompatible or missing units reject the fixture before comparison.
- `GRC_057_CALIBRATION_INVALID`: missing, expired, or out-of-scope calibration prevents a verified/correlated status.
- `GRC_057_VALIDITY_EXCEEDED`: out-of-envelope operation is reported as unsupported, never silently accepted or extrapolated as accurate.
- `GRC_057_LINEAGE_INCOMPLETE`: missing specimen, model, dependency, raw-data, processing, license, or digest lineage prevents publication.

## Acceptance

1. `TEST-GRC-057` evaluates nominal, boundary, invalid-unit, calibration, validity, and lineage vectors without using the implementation under test as its physical reference.
