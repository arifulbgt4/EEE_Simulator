# VAL-GRC-056 - Validate Physical RC charging and discharging
...
|---|---|
| Status | Planned |
| Target | Physical RC charging and discharging |
| Requirements | REQ-039, REQ-040, REQ-041, REQ-042, REQ-043, REQ-044, REQ-045, REQ-046, REQ-047 |
| Golden fixture | GRC-056 |
| Physical benchmark | PBC-002 |
| Test ID | TEST-GRC-056 |
| Release | R4 |
...
Produce independent, reproducible physical-to-simulation correlation evidence for **Physical RC charging and discharging** without changing the model, solver, instrument, or feature under test.
...
- `docs/tasks/validation/val-grc-056.md`
...
- Primary assertion: Measured time constant, source/probe loading, parasitics, temperature and uncertainty.
- `GRC_056_NOMINAL`: candidate output is compared with retained physical measurements using the declared metric, interval, sample alignment, and category-specific envelope.
- `GRC_056_BOUNDARY`: minimum/maximum declared operating points, environmental bounds, instrument range, source limit, and applicable tolerance corners are classified explicitly.
- `GRC_056_DIMENSION_INVALID`: incompatible or missing units reject the fixture before comparison.
- `GRC_056_CALIBRATION_INVALID`: missing, expired, or out-of-scope calibration prevents a verified/correlated status.
- `GRC_056_VALIDITY_EXCEEDED`: out-of-envelope operation is reported as unsupported, never silently accepted or extrapolated as accurate.
- `GRC_056_LINEAGE_INCOMPLETE`: missing specimen, model, dependency, raw-data, processing, license, or digest lineage prevents publication.

## Acceptance

1. `TEST-GRC-056` evaluates nominal, boundary, invalid-unit, calibration, validity, and lineage vectors without using the implementation under test as its physical reference.
