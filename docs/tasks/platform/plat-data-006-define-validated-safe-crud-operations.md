# PLAT-DATA-006 - Define validated safe CRUD operations
...
| Concern | `safe_crud` |
| Effort | S |
| Depends on | PLAT-DATA-005 |

## Objective

Replace destructive row CRUD with authorized validated auditable domain operations. This card owns exactly one public concern and no implementation is authorized while its status is `Planned`.
...
- The accepted clauses for REQ-054, REQ-055, REQ-056, REQ-059, REQ-060, REQ-061, REQ-062, REQ-063, ADR-0011 through ADR-0020 where applicable, and exact predecessor evidence from `PLAT-DATA-005`.
...
- `docs/tasks/platform/plat-data-006-define-validated-safe-crud-operations.md`
...
- A versioned, schema-valid deliverable for `safe_crud` with exact inputs, outputs, defaults, dimensions, limits, state ownership, compatibility, provenance, and known limitations.
- `PLAT_DATA_006_NOMINAL`: the valid fixture resolves deterministically to the specified outcome and retains complete lineage.
- `PLAT_DATA_006_BOUNDARY`: every inclusive/exclusive limit and unsupported case returns its declared state or diagnostic.
- `UNSAFE_CRUD_REJECTED`: invalid input is rejected before authoritative publication or execution; the diagnostic names the entity, field/reference, rejected value, and remediation.
- `PLAT_DATA_006_PREREQUISITE_MISMATCH`: incompatible or missing predecessor/schema revision is rejected without implicit migration.
...
1. `TEST-PLAT-DATA-006-ACCEPTANCE` validates the nominal and exact boundary fixtures against REQ-054, REQ-055, REQ-056, REQ-059, REQ-060, REQ-061, REQ-062, REQ-063, the primary contract, schema version, revision IDs, hashes, dimensions, provenance, and expected outcome.
2. `TEST-PLAT-DATA-006-FAILURE` exercises `UNSAFE_CRUD_REJECTED` and the prerequisite mismatch path; no partial authoritative state, silent fallback, unsafe execution, broken historical reference, or lost audit record is allowed.
