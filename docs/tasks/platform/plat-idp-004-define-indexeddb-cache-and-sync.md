# PLAT-IDP-004 - Define IndexedDB cache and sync
...
| Concern | `indexeddb_cache_and_sync` |
| Effort | S |
| Depends on | PLAT-IDP-003 |

## Objective

Preserve local-first projects through cache outbox retry and explicit conflict handling. This card owns exactly one public concern and no implementation is authorized while its status is `Planned`.
...
- The accepted clauses for REQ-006, REQ-029, REQ-057, REQ-058, ADR-0011 through ADR-0020 where applicable, and exact predecessor evidence from `PLAT-IDP-003`.
...
- `docs/tasks/platform/plat-idp-004-define-indexeddb-cache-and-sync.md`
...
- A versioned, schema-valid deliverable for `indexeddb_cache_and_sync` with exact inputs, outputs, defaults, dimensions, limits, state ownership, compatibility, provenance, and known limitations.
- `PLAT_IDP_004_NOMINAL`: the valid fixture resolves deterministically to the specified outcome and retains complete lineage.
- `PLAT_IDP_004_BOUNDARY`: every inclusive/exclusive limit and unsupported case returns its declared state or diagnostic.
- `OFFLINE_SYNC_CONFLICT`: invalid input is rejected before authoritative publication or execution; the diagnostic names the entity, field/reference, rejected value, and remediation.
- `PLAT_IDP_004_PREREQUISITE_MISMATCH`: incompatible or missing predecessor/schema revision is rejected without implicit migration.
...
1. `TEST-PLAT-IDP-004-ACCEPTANCE` validates the nominal and exact boundary fixtures against REQ-006, REQ-029, REQ-057, REQ-058, the primary contract, schema version, revision IDs, hashes, dimensions, provenance, and expected outcome.
2. `TEST-PLAT-IDP-004-FAILURE` exercises `OFFLINE_SYNC_CONFLICT` and the prerequisite mismatch path; no partial authoritative state, silent fallback, unsafe execution, broken historical reference, or lost audit record is allowed.
