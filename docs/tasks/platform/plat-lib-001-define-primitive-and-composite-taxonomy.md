# PLAT-LIB-001 - Define primitive and composite taxonomy
...
| Concern | `primitive_composite_taxonomy` |
| Effort | S |
| Depends on | Gate G0 documentation baseline |

## Objective

Freeze distinct model device representation composition and artifact kinds. This card owns exactly one public concern and no implementation is authorized while its status is `Planned`.
...
- The accepted clauses for REQ-040, REQ-042, REQ-048, REQ-049, REQ-050, REQ-051, REQ-052, REQ-053, REQ-059, REQ-063, ADR-0011 through ADR-0020 where applicable, and exact predecessor evidence from `Gate G0 documentation baseline`.
...
- `docs/tasks/platform/plat-lib-001-define-primitive-and-composite-taxonomy.md`
...
- A versioned, schema-valid deliverable for `primitive_composite_taxonomy` with exact inputs, outputs, defaults, dimensions, limits, state ownership, compatibility, provenance, and known limitations.
- `PLAT_LIB_001_NOMINAL`: the valid fixture resolves deterministically to the specified outcome and retains complete lineage.
- `PLAT_LIB_001_BOUNDARY`: every inclusive/exclusive limit and unsupported case returns its declared state or diagnostic.
- `LIBRARY_KIND_INVALID`: invalid input is rejected before authoritative publication or execution; the diagnostic names the entity, field/reference, rejected value, and remediation.
- `PLAT_LIB_001_PREREQUISITE_MISMATCH`: incompatible or missing predecessor/schema revision is rejected without implicit migration.
...
1. `TEST-PLAT-LIB-001-ACCEPTANCE` validates the nominal and exact boundary fixtures against REQ-040, REQ-042, REQ-048, REQ-049, REQ-050, REQ-051, REQ-052, REQ-053, REQ-059, REQ-063, the primary contract, schema version, revision IDs, hashes, dimensions, provenance, and expected outcome.
2. `TEST-PLAT-LIB-001-FAILURE` exercises `LIBRARY_KIND_INVALID` and the prerequisite mismatch path; no partial authoritative state, silent fallback, unsafe execution, broken historical reference, or lost audit record is allowed.
