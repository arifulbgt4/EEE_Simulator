# PLAT-DATA-007 - Resolve dependencies and reverse dependencies
...
| Concern | `reverse_dependency_analysis` |
| Effort | S |
| Depends on | PLAT-DATA-006 |

## Objective

Traverse exact dependencies and block destructive changes to referenced revisions. This card owns exactly one public concern and no implementation is authorized while its status is `Planned`.
...
- The accepted clauses for REQ-054, REQ-055, REQ-056, REQ-059, REQ-060, REQ-061, REQ-062, REQ-063, ADR-0011 through ADR-0020 where applicable, and exact predecessor evidence from `PLAT-DATA-006`.
...
- `docs/tasks/platform/plat-data-007-resolve-dependencies-and-reverse-dependencies.md`
...
- A versioned, schema-valid deliverable for `reverse_dependency_analysis` with exact inputs, outputs, defaults, dimensions, limits, state ownership, compatibility, provenance, and known limitations.
- `PLAT_DATA_007_NOMINAL`: the valid fixture resolves deterministically to the specified outcome and retains complete lineage.
- `PLAT_DATA_007_BOUNDARY`: every inclusive/exclusive limit and unsupported case returns its declared state or diagnostic.
- `REFERENCED_REVISION_PROTECTED`: invalid input is rejected before authoritative publication or execution; the diagnostic names the entity, field/reference, rejected value, and remediation.
- `PLAT_DATA_007_PREREQUISITE_MISMATCH`: incompatible or missing predecessor/schema revision is rejected without implicit migration.
...
1. `TEST-PLAT-DATA-007-ACCEPTANCE` validates the nominal and exact boundary fixtures against REQ-054, REQ-055, REQ-056, REQ-059, REQ-060, REQ-061, REQ-062, REQ-063, the primary contract, schema version, revision IDs, hashes, dimensions, provenance, and expected outcome.
2. `TEST-PLAT-DATA-007-FAILURE` exercises `REFERENCED_REVISION_PROTECTED` and the prerequisite mismatch path; no partial authoritative state, silent fallback, unsafe execution, broken historical reference, or lost audit record is allowed.
