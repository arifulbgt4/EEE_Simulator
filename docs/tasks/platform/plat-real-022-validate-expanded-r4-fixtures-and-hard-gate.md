# PLAT-REAL-022 - Validate expanded R4 fixtures and hard gate
...
| Concern | `expanded_r4_gate_validation` |
| Effort | S |
| Depends on | PLAT-REAL-021, VAL-GRC-055..066 |

## Objective

Validate legacy and physical fixtures and prove G5 and G6 cannot bypass G4. This card owns exactly one public concern and no implementation is authorized while its status is `Planned`.
...
- `PLAT-REAL-021`
- `VAL-GRC-055` through `VAL-GRC-066`
...
- The accepted clauses for REQ-013, REQ-014, REQ-015, REQ-016, REQ-039, REQ-043, REQ-044, REQ-045, REQ-046, REQ-047, ADR-0011 through ADR-0020 where applicable, and exact predecessor evidence from `PLAT-REAL-021`.
...
- `docs/tasks/platform/plat-real-022-validate-expanded-r4-fixtures-and-hard-gate.md`
...
- A versioned, schema-valid deliverable for `expanded_r4_gate_validation` with exact inputs, outputs, defaults, dimensions, limits, state ownership, compatibility, provenance, and known limitations.
- `PLAT_REAL_022_NOMINAL`: the valid fixture resolves deterministically to the specified outcome and retains complete lineage.
- `PLAT_REAL_022_BOUNDARY`: every inclusive/exclusive limit and unsupported case returns its declared state or diagnostic.
- `R4_HARD_GATE_FAILED`: invalid input is rejected before authoritative publication or execution; the diagnostic names the entity, field/reference, rejected value, and remediation.
- `PLAT_REAL_022_PREREQUISITE_MISMATCH`: incompatible or missing predecessor/schema revision is rejected without implicit migration.
...
1. `TEST-PLAT-REAL-022-ACCEPTANCE` validates the nominal and exact boundary fixtures against REQ-013, REQ-014, REQ-015, REQ-016, REQ-039, REQ-043, REQ-044, REQ-045, REQ-046, REQ-047, the primary contract, schema version, revision IDs, hashes, dimensions, provenance, and expected outcome.
2. `TEST-PLAT-REAL-022-FAILURE` exercises `R4_HARD_GATE_FAILED` and the prerequisite mismatch path; no partial authoritative state, silent fallback, unsafe execution, broken historical reference, or lost audit record is allowed.
