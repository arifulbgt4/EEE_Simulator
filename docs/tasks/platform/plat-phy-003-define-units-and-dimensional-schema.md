# PLAT-PHY-003 - Define units and dimensional schema
...
| Concern | `units_and_dimensions_schema` |
| Effort | S |
| Depends on | PLAT-PHY-002 |

## Objective

Make every normative quantity dimensionally explicit in canonical SI. This card owns exactly one public concern and no implementation is authorized while its status is `Planned`.
...
- The accepted clauses for REQ-039, REQ-040, REQ-041, REQ-042, REQ-043, REQ-044, REQ-045, REQ-046, REQ-047, ADR-0011 through ADR-0020 where applicable, and exact predecessor evidence from `PLAT-PHY-002`.
...
- `docs/tasks/platform/plat-phy-003-define-units-and-dimensional-schema.md`
...
- A versioned, schema-valid deliverable for `units_and_dimensions_schema` with exact inputs, outputs, defaults, dimensions, limits, state ownership, compatibility, provenance, and known limitations.
- `PLAT_PHY_003_NOMINAL`: the valid fixture resolves deterministically to the specified outcome and retains complete lineage.
- `PLAT_PHY_003_BOUNDARY`: every inclusive/exclusive limit and unsupported case returns its declared state or diagnostic.
- `DIMENSION_MISMATCH`: invalid input is rejected before authoritative publication or execution; the diagnostic names the entity, field/reference, rejected value, and remediation.
- `PLAT_PHY_003_PREREQUISITE_MISMATCH`: incompatible or missing predecessor/schema revision is rejected without implicit migration.
...
1. `TEST-PLAT-PHY-003-ACCEPTANCE` validates the nominal and exact boundary fixtures against REQ-039, REQ-040, REQ-041, REQ-042, REQ-043, REQ-044, REQ-045, REQ-046, REQ-047, the primary contract, schema version, revision IDs, hashes, dimensions, provenance, and expected outcome.
2. `TEST-PLAT-PHY-003-FAILURE` exercises `DIMENSION_MISMATCH` and the prerequisite mismatch path; no partial authoritative state, silent fallback, unsafe execution, broken historical reference, or lost audit record is allowed.
