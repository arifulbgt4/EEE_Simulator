# PLAT-IDP-001 - Define provider-neutral Google OIDC architecture
...
| Concern | `google_oidc_architecture` |
| Effort | S |
| Depends on | Gate G0 documentation baseline |

## Objective

Plan secure Google sign-in without coupling identity contracts to one provider. This card owns exactly one public concern and no implementation is authorized while its status is `Planned`.
...
- The accepted clauses for REQ-006, REQ-029, REQ-057, REQ-058, ADR-0011 through ADR-0020 where applicable, and exact predecessor evidence from `Gate G0 documentation baseline`.
...
- `docs/tasks/platform/plat-idp-001-define-provider-neutral-google-oidc-architecture.md`
...
- A versioned, schema-valid deliverable for `google_oidc_architecture` with exact inputs, outputs, defaults, dimensions, limits, state ownership, compatibility, provenance, and known limitations.
- `PLAT_IDP_001_NOMINAL`: the valid fixture resolves deterministically to the specified outcome and retains complete lineage.
- `PLAT_IDP_001_BOUNDARY`: every inclusive/exclusive limit and unsupported case returns its declared state or diagnostic.
- `OIDC_VALIDATION_FAILED`: invalid input is rejected before authoritative publication or execution; the diagnostic names the entity, field/reference, rejected value, and remediation.
- `PLAT_IDP_001_PREREQUISITE_MISMATCH`: incompatible or missing predecessor/schema revision is rejected without implicit migration.
...
1. `TEST-PLAT-IDP-001-ACCEPTANCE` validates the nominal and exact boundary fixtures against REQ-006, REQ-029, REQ-057, REQ-058, the primary contract, schema version, revision IDs, hashes, dimensions, provenance, and expected outcome.
2. `TEST-PLAT-IDP-001-FAILURE` exercises `OIDC_VALIDATION_FAILED` and the prerequisite mismatch path; no partial authoritative state, silent fallback, unsafe execution, broken historical reference, or lost audit record is allowed.
