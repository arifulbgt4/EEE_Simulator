...
- **Family ID:** `fam-actuators-hmi-stepper-motor`
...
Stepper motor is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Stepper motor**, **Stepper Motor**, and `stepper-motor`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-actuators-hmi-stepper-motor-unipolar` | Unipolar | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-electromechanical`, `pkg-display-module`, `pkg-custom-parametric` |
| `var-actuators-hmi-stepper-motor-bipolar` | Bipolar | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-electromechanical`, `pkg-display-module`, `pkg-custom-parametric` |
| `var-actuators-hmi-stepper-motor-hybrid` | Hybrid | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-electromechanical`, `pkg-display-module`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-actuators-hmi-stepper-motor`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
- `GOLD-ACT-STEPPER_MOTOR-NOMINAL`: nominal positive-path behavior.
- `GOLD-ACT-STEPPER_MOTOR-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-ACT-STEPPER_MOTOR-FAILURE`: diagnostic and fault behavior.
