# RF port and network model

- **Family ID:** `fam-rf-communications-rf-port-network`
...
RF port and network model is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **RF port and network model**, **Rf Port Network**, and `rf-port-network`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-rf-communications-rf-port-network-rf-port` | Rf Port | F1, F2, F3 | Post-MVP catalog | `pkg-rf-module`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-rf-communications-rf-port-network-termination` | Termination | F1, F2, F3 | Post-MVP catalog | `pkg-rf-module`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-rf-communications-rf-port-network-s-parameter-black-box` | S Parameter Black Box | F1, F2, F3 | Post-MVP catalog | `pkg-rf-module`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-rf-communications-rf-port-network`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `port_count` | RF port count | 1 | 2 | 1..1024 |
| `reference_impedance` | Port reference impedance | ohm | 50 | >0 |
| `start_frequency` | Validated start frequency | Hz | 1e6 | >=0 |
| `stop_frequency` | Validated stop frequency | Hz | 1e9 | >start_frequency |
| `data_digest` | Pinned network-data digest | 1 | unset | valid digest before Ready |
...
- **Governing relation or state rule:** At each declared frequency, incident/reflected waves satisfy b = S(f)*a at the stated reference impedance; passivity, causality, and port order are validated.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: At each declared frequency, incident/reflected waves satisfy b = S(f)*a at the stated reference impedance; passivity, causality, and port order are validated.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: At each declared frequency, incident/reflected waves satisfy b = S(f)*a at the stated reference impedance; passivity, causality, and port order are validated.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: At each declared frequency, incident/reflected waves satisfy b = S(f)*a at the stated reference impedance; passivity, causality, and port order are validated.
- **Exact nominal vector:** pins `IN:RF_IN`/passive, `OUT:RF_OUT`/passive, `GND:GROUND`/reference; parameters `port_count`=2 1 (1..1024); `reference_impedance`=50 ohm (>0); `start_frequency`=1e6 Hz (>=0); `stop_frequency`=1e9 Hz (>start_frequency); `data_digest`=unset 1 (valid digest before Ready).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-RFC-RF_PORT_NETWORK-NOMINAL`, `GOLD-RFC-RF_PORT_NETWORK-BOUNDARY`, `GOLD-RFC-RF_PORT_NETWORK-FAILURE`.
...
- `GOLD-RFC-RF_PORT_NETWORK-NOMINAL`: nominal positive-path behavior.
- `GOLD-RFC-RF_PORT_NETWORK-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-RFC-RF_PORT_NETWORK-FAILURE`: diagnostic and fault behavior.
