...
- **Family ID:** `fam-mcu-fpga-fpga-hdl`
...
FPGA and HDL block is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **FPGA and HDL block**, **Fpga Hdl**, and `fpga-hdl`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-mcu-fpga-fpga-hdl-lut` | Lut | F0, F2, F3 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-mcu-fpga-fpga-hdl-block-ram` | Block Ram | F0, F2, F3 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-mcu-fpga-fpga-hdl-dsp-slice` | Dsp Slice | F0, F2, F3 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-mcu-fpga-fpga-hdl-pll` | Pll | F0, F2, F3 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-mcu-fpga-fpga-hdl-hdl-wrapper` | Hdl Wrapper | F0, F2, F3 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-mcu-fpga-fpga-hdl`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
- `GOLD-MCU-FPGA_HDL-NOMINAL`: nominal positive-path behavior.
- `GOLD-MCU-FPGA_HDL-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-MCU-FPGA_HDL-FAILURE`: diagnostic and fault behavior.
