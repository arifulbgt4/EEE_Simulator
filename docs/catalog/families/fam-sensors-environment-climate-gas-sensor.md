# Climate and gas sensor

- **Family ID:** `fam-sensors-environment-climate-gas-sensor`
...
Climate and gas sensor is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Climate and gas sensor**, **Climate Gas Sensor**, and `climate-gas-sensor`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-sensors-environment-climate-gas-sensor-humidity` | Humidity | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-climate-gas-sensor-barometric` | Barometric | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-climate-gas-sensor-carbon-dioxide` | Carbon Dioxide | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-climate-gas-sensor-voc` | Voc | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-climate-gas-sensor-generic-gas` | Generic Gas | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-sensors-environment-climate-gas-sensor`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `relative_humidity_full_scale` | Relative-humidity full scale | 1 | 1 | >0 |
| `pressure_full_scale` | Pressure full scale | Pa | 120e3 | >0 |
| `concentration_full_scale` | Gas concentration full scale | mol/mol | 0.01 | >0 |
| `warmup_time` | Warm-up time | s | 0 | >=0 |
...
- **Governing relation or state rule:** Output follows the declared humidity/gas concentration transfer with warm-up, cross-sensitivity, saturation, response/recovery, and environmental terms.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Output follows the declared humidity/gas concentration transfer with warm-up, cross-sensitivity, saturation, response/recovery, and environmental terms.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: Output follows the declared humidity/gas concentration transfer with warm-up, cross-sensitivity, saturation, response/recovery, and environmental terms.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: Output follows the declared humidity/gas concentration transfer with warm-up, cross-sensitivity, saturation, response/recovery, and environmental terms.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Output follows the declared humidity/gas concentration transfer with warm-up, cross-sensitivity, saturation, response/recovery, and environmental terms.
- **F4:** Electrothermal/tolerance/failure tier: extend the lower-tier relation with declared sampling, power-to-heat state Cth*dT/dt = P-(T-Tamb)/Rth, derating, and deterministic failure transitions; base relation: Output follows the declared humidity/gas concentration transfer with warm-up, cross-sensitivity, saturation, response/recovery, and environmental terms.
- **Exact nominal vector:** pins `S:STIMULUS`/physical, `O:OUTPUT`/output, `VDD:VDD`/power, `GND:GND`/power; parameters `relative_humidity_full_scale`=1 1 (>0); `pressure_full_scale`=120e3 Pa (>0); `concentration_full_scale`=0.01 mol/mol (>0); `warmup_time`=0 s (>=0); `response_time`=1 s (>=0).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-SEN-CLIMATE_GAS_SENSOR-NOMINAL`, `GOLD-SEN-CLIMATE_GAS_SENSOR-BOUNDARY`, `GOLD-SEN-CLIMATE_GAS_SENSOR-FAILURE`.
...
- `GOLD-SEN-CLIMATE_GAS_SENSOR-NOMINAL`: nominal positive-path behavior.
- `GOLD-SEN-CLIMATE_GAS_SENSOR-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-SEN-CLIMATE_GAS_SENSOR-FAILURE`: diagnostic and fault behavior.
