# Magnetic and motion sensor

- **Family ID:** `fam-sensors-environment-magnetic-motion-sensor`
...
Magnetic and motion sensor is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Magnetic and motion sensor**, **Magnetic Motion Sensor**, and `magnetic-motion-sensor`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-sensors-environment-magnetic-motion-sensor-hall` | Hall | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-magnetic-motion-sensor-reed` | Reed | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-magnetic-motion-sensor-accelerometer` | Accelerometer | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-magnetic-motion-sensor-gyroscope` | Gyroscope | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-magnetic-motion-sensor-magnetometer` | Magnetometer | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-sensors-environment-magnetic-motion-sensor`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
- `GOLD-SEN-MAGNETIC_MOTION_SENSOR-NOMINAL`: nominal positive-path behavior.
- `GOLD-SEN-MAGNETIC_MOTION_SENSOR-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-SEN-MAGNETIC_MOTION_SENSOR-FAILURE`: diagnostic and fault behavior.
