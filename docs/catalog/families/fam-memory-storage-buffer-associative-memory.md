# Buffer and associative memory

- **Family ID:** `fam-memory-storage-buffer-associative-memory`
...
Buffer and associative memory is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Buffer and associative memory**, **Buffer Associative Memory**, and `buffer-associative-memory`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-memory-storage-buffer-associative-memory-fifo` | Fifo | F0, F2, F3 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-memory-storage-buffer-associative-memory-cam` | Cam | F0, F2, F3 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-memory-storage-buffer-associative-memory-cache` | Cache | F0, F2, F3 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-memory-storage-buffer-associative-memory`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
- `GOLD-MEM-BUFFER_ASSOCIATIVE_MEMORY-NOMINAL`: nominal positive-path behavior.
- `GOLD-MEM-BUFFER_ASSOCIATIVE_MEMORY-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-MEM-BUFFER_ASSOCIATIVE_MEMORY-FAILURE`: diagnostic and fault behavior.
