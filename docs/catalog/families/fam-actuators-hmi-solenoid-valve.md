...
- **Family ID:** `fam-actuators-hmi-solenoid-valve`
...
Solenoid and valve is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Solenoid and valve**, **Solenoid Valve**, and `solenoid-valve`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-actuators-hmi-solenoid-valve-solenoid` | Solenoid | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-electromechanical`, `pkg-display-module`, `pkg-custom-parametric` |
| `var-actuators-hmi-solenoid-valve-pneumatic-valve` | Pneumatic Valve | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-electromechanical`, `pkg-display-module`, `pkg-custom-parametric` |
| `var-actuators-hmi-solenoid-valve-liquid-valve` | Liquid Valve | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-electromechanical`, `pkg-display-module`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-actuators-hmi-solenoid-valve`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
- `GOLD-ACT-SOLENOID_VALVE-NOMINAL`: nominal positive-path behavior.
- `GOLD-ACT-SOLENOID_VALVE-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-ACT-SOLENOID_VALVE-FAILURE`: diagnostic and fault behavior.
