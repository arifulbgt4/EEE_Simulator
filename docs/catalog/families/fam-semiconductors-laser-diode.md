...
- **Family ID:** `fam-semiconductors-laser-diode`
...
Laser diode is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Laser diode**, **Laser Diode**, and `laser-diode`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-semiconductors-laser-diode-edge-emitting` | Edge Emitting | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-led-round`, `pkg-led-smd`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-semiconductors-laser-diode`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `threshold_current` | Lasing threshold current | A | 0.02 | >0 |
| `slope_efficiency` | Optical slope efficiency | W/A | 0.5 | >=0 |
| `peak_wavelength` | Peak wavelength | m | 850e-9 | >0 |
| `series_resistance` | Series resistance | ohm | 5 | >=0 |
...
- **Governing relation or state rule:** The junction relation couples to optical power above threshold with declared slope efficiency, wavelength, thermal roll-off, and safe operating limits.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The junction relation couples to optical power above threshold with declared slope efficiency, wavelength, thermal roll-off, and safe operating limits.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: The junction relation couples to optical power above threshold with declared slope efficiency, wavelength, thermal roll-off, and safe operating limits.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: The junction relation couples to optical power above threshold with declared slope efficiency, wavelength, thermal roll-off, and safe operating limits.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: The junction relation couples to optical power above threshold with declared slope efficiency, wavelength, thermal roll-off, and safe operating limits.
...
- **Exact nominal vector:** pins `1:A`/passive, `2:K`/passive; parameters `threshold_current`=0.02 A (>0); `slope_efficiency`=0.5 W/A (>=0); `peak_wavelength`=850e-9 m (>0); `series_resistance`=5 ohm (>=0); `temperature`=300.15 K (>0).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-SEM-LASER_DIODE-NOMINAL`, `GOLD-SEM-LASER_DIODE-BOUNDARY`, `GOLD-SEM-LASER_DIODE-FAILURE`.
...
- `GOLD-SEM-LASER_DIODE-NOMINAL`: nominal positive-path behavior.
- `GOLD-SEM-LASER_DIODE-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-SEM-LASER_DIODE-FAILURE`: diagnostic and fault behavior.
