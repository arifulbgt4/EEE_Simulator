...
- **Family ID:** `fam-analog-mixed-signal-schmitt-trigger`
...
Schmitt trigger is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Schmitt trigger**, **Schmitt Trigger**, and `schmitt-trigger`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-analog-mixed-signal-schmitt-trigger-inverting` | Inverting | F0, F1, F2, F3, F4 | Realistic Electronics MVP | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-analog-mixed-signal-schmitt-trigger-non-inverting` | Non Inverting | F0, F1, F2, F3, F4 | Realistic Electronics MVP | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-analog-mixed-signal-schmitt-trigger`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
- `GOLD-AMS-SCHMITT_TRIGGER-NOMINAL`: nominal positive-path behavior.
- `GOLD-AMS-SCHMITT_TRIGGER-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-AMS-SCHMITT_TRIGGER-FAILURE`: diagnostic and fault behavior.
