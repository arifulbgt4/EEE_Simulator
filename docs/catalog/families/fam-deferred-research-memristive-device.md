...
- **Family ID:** `fam-deferred-research-memristive-device`
...
Memristive device is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Memristive device**, **Memristive Device**, and `memristive-device`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-deferred-research-memristive-device-memristor` | Memristor | F5 | Research | `pkg-custom-parametric` |
...
- Symbol ID: `sym-deferred-research-memristive-device`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
- **Governing relation or state rule:** F5 behavior uses a pinned state-dependent resistance law dstate/dt=f(v,i,state) whose bounds, passivity, initialization, and provenance are explicit.
- **F5:** Research tier: execute only through the declared isolated, pinned research adapter and retain its full configuration/provenance; baseline: F5 behavior uses a pinned state-dependent resistance law dstate/dt=f(v,i,state) whose bounds, passivity, initialization, and provenance are explicit.
...
- **Golden evidence:** `GOLD-RSH-MEMRISTIVE_DEVICE-NOMINAL`, `GOLD-RSH-MEMRISTIVE_DEVICE-BOUNDARY`, `GOLD-RSH-MEMRISTIVE_DEVICE-FAILURE`.
...
- `GOLD-RSH-MEMRISTIVE_DEVICE-NOMINAL`: nominal positive-path behavior.
- `GOLD-RSH-MEMRISTIVE_DEVICE-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-RSH-MEMRISTIVE_DEVICE-FAILURE`: diagnostic and fault behavior.
