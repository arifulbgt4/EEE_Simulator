...
- **Family ID:** `fam-passives-potentiometer`
...
Potentiometer is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Potentiometer**, **Potentiometer**, and `potentiometer`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-passives-potentiometer-linear` | Linear | F0, F1, F2, F3, F4 | Realistic Electronics MVP | `pkg-axial-2`, `pkg-smd-chip`, `pkg-radial-2` |
| `var-passives-potentiometer-logarithmic` | Logarithmic | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-axial-2`, `pkg-smd-chip`, `pkg-radial-2` |
...
- Symbol ID: `sym-passives-potentiometer`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `total_resistance` | End-to-end resistance | ohm | 10000 | >0 |
| `wiper_fraction` | Normalized wiper position | 1 | 0.5 | 0..1 |
| `taper` | Wiper taper | 1 | linear | linear or logarithmic |
| `contact_resistance` | Wiper contact resistance | ohm | 1 | >=0 |
| `power_rating` | Power rating | W | 0.25 | >0 |
...
- **Governing relation or state rule:** The total resistance is partitioned by wiper fraction a into R1 = a*R and R2 = (1-a)*R, including declared contact resistance and endpoint limits.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The total resistance is partitioned by wiper fraction a into R1 = a*R and R2 = (1-a)*R, including declared contact resistance and endpoint limits.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: The total resistance is partitioned by wiper fraction a into R1 = a*R and R2 = (1-a)*R, including declared contact resistance and endpoint limits.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: The total resistance is partitioned by wiper fraction a into R1 = a*R and R2 = (1-a)*R, including declared contact resistance and endpoint limits.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: The total resistance is partitioned by wiper fraction a into R1 = a*R and R2 = (1-a)*R, including declared contact resistance and endpoint limits.
...
- **Exact nominal vector:** pins `1:P`/passive, `2:N`/passive; parameters `total_resistance`=10000 ohm (>0); `wiper_fraction`=0.5 1 (0..1); `taper`=linear 1 (linear or logarithmic); `contact_resistance`=1 ohm (>=0); `power_rating`=0.25 W (>0).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-PAS-POTENTIOMETER-NOMINAL`, `GOLD-PAS-POTENTIOMETER-BOUNDARY`, `GOLD-PAS-POTENTIOMETER-FAILURE`.
...
- `GOLD-PAS-POTENTIOMETER-NOMINAL`: nominal positive-path behavior.
- `GOLD-PAS-POTENTIOMETER-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-PAS-POTENTIOMETER-FAILURE`: diagnostic and fault behavior.
