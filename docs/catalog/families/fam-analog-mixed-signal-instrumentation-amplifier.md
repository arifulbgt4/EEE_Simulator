...
- **Family ID:** `fam-analog-mixed-signal-instrumentation-amplifier`
...
Instrumentation amplifier is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Instrumentation amplifier**, **Instrumentation Amplifier**, and `instrumentation-amplifier`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-analog-mixed-signal-instrumentation-amplifier-three-op-amp` | Three Op Amp | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-analog-mixed-signal-instrumentation-amplifier-integrated` | Integrated | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-analog-mixed-signal-instrumentation-amplifier`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `gain` | Differential gain | 1 | 10 | >0 |
| `reference_voltage` | Output reference voltage | V | 0 | finite |
| `common_mode_rejection` | Common-mode rejection ratio | 1 | 1e5 | >0 |
| `bandwidth` | Small-signal bandwidth | Hz | 100e3 | >0 |
...
- **Governing relation or state rule:** Output equals declared differential gain times input difference plus reference, bounded by common-mode, supply, bandwidth, and output limits.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Output equals declared differential gain times input difference plus reference, bounded by common-mode, supply, bandwidth, and output limits.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: Output equals declared differential gain times input difference plus reference, bounded by common-mode, supply, bandwidth, and output limits.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: Output equals declared differential gain times input difference plus reference, bounded by common-mode, supply, bandwidth, and output limits.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Output equals declared differential gain times input difference plus reference, bounded by common-mode, supply, bandwidth, and output limits.
...
- **Exact nominal vector:** pins `1:IN+`/input, `2:IN-`/input, `3:OUT`/output, `4:V+`/power, `5:V-`/power; parameters `gain`=10 1 (>0); `reference_voltage`=0 V (finite); `common_mode_rejection`=1e5 1 (>0); `bandwidth`=100e3 Hz (>0); `input_offset`=0 V (finite).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-AMS-INSTRUMENTATION_AMPLIFIER-NOMINAL`, `GOLD-AMS-INSTRUMENTATION_AMPLIFIER-BOUNDARY`, `GOLD-AMS-INSTRUMENTATION_AMPLIFIER-FAILURE`.
...
- `GOLD-AMS-INSTRUMENTATION_AMPLIFIER-NOMINAL`: nominal positive-path behavior.
- `GOLD-AMS-INSTRUMENTATION_AMPLIFIER-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-AMS-INSTRUMENTATION_AMPLIFIER-FAILURE`: diagnostic and fault behavior.
