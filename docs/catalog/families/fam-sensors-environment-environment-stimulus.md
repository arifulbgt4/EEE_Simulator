...
- **Family ID:** `fam-sensors-environment-environment-stimulus`
...
Environmental stimulus is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Environmental stimulus**, **Environment Stimulus**, and `environment-stimulus`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-sensors-environment-environment-stimulus-constant-stimulus` | Constant Stimulus | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-environment-stimulus-time-profile` | Time Profile | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-sensors-environment-environment-stimulus`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `quantity_kind` | Emitted environmental quantity | 1 | temperature | registered stimulus quantity |
| `constant_value` | Constant SI stimulus value | 1 | 0 | finite normalized value |
| `start_time` | Profile start time | s | 0 | >=0 |
| `interpolation` | Profile interpolation | 1 | linear | step or linear |
| `seed` | Stochastic-profile seed | 1 | 1 | 0..18446744073709551615 |
...
- **Governing relation or state rule:** The primitive emits a deterministic SI-valued environmental field/time profile; interpolation, spatial scope, discontinuities, and seed are explicit.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The primitive emits a deterministic SI-valued environmental field/time profile; interpolation, spatial scope, discontinuities, and seed are explicit.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: The primitive emits a deterministic SI-valued environmental field/time profile; interpolation, spatial scope, discontinuities, and seed are explicit.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: The primitive emits a deterministic SI-valued environmental field/time profile; interpolation, spatial scope, discontinuities, and seed are explicit.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: The primitive emits a deterministic SI-valued environmental field/time profile; interpolation, spatial scope, discontinuities, and seed are explicit.
...
- **Exact nominal vector:** pins `S:STIMULUS`/physical, `O:OUTPUT`/output, `VDD:VDD`/power, `GND:GND`/power; parameters `quantity_kind`=temperature 1 (registered stimulus quantity); `constant_value`=0 1 (finite normalized value); `start_time`=0 s (>=0); `interpolation`=linear 1 (step or linear); `seed`=1 1 (0..18446744073709551615).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-SEN-ENVIRONMENT_STIMULUS-NOMINAL`, `GOLD-SEN-ENVIRONMENT_STIMULUS-BOUNDARY`, `GOLD-SEN-ENVIRONMENT_STIMULUS-FAILURE`.
...
- `GOLD-SEN-ENVIRONMENT_STIMULUS-NOMINAL`: nominal positive-path behavior.
- `GOLD-SEN-ENVIRONMENT_STIMULUS-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-SEN-ENVIRONMENT_STIMULUS-FAILURE`: diagnostic and fault behavior.
