...
- **Family ID:** `fam-sources-loads-digital-pattern-source`
...
Digital pattern source is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Digital pattern source**, **Digital Pattern Source**, and `digital-pattern-source`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-sources-loads-digital-pattern-source-pattern` | Pattern | F0, F1, F2, F3, F4 | Realistic Electronics MVP | `pkg-virtual` |
...
- Symbol ID: `sym-sources-loads-digital-pattern-source`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
- `GOLD-SRC-DIGITAL_PATTERN_SOURCE-NOMINAL`: nominal positive-path behavior.
- `GOLD-SRC-DIGITAL_PATTERN_SOURCE-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-SRC-DIGITAL_PATTERN_SOURCE-FAILURE`: diagnostic and fault behavior.
