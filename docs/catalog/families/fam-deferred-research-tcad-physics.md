# TCAD physical device model

- **Family ID:** `fam-deferred-research-tcad-physics`
- **Category:** Explicitly deferred research families
- **Lifecycle:** Deferred/Research
- **Basic component tag:** Not tagged basic
- **Release target:** Research
- **Source:** Source PDF, pp. 34-41

## Purpose and scope

TCAD physical device model is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **TCAD physical device model**, **Tcad Physics**, and `tcad-physics`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-deferred-research-tcad-physics-semiconductor-device` | Semiconductor Device | F5 | Research | `pkg-custom-parametric` |
| `var-deferred-research-tcad-physics-photonic-device` | Photonic Device | F5 | Research | `pkg-custom-parametric` |
...
- Symbol ID: `sym-deferred-research-tcad-physics`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
Supported fidelity tiers: **F5**. Supported analysis capabilities: **research only**.
...
- **Governing relation or state rule:** F5 behavior exists only as an isolated adapter to a pinned TCAD solver, mesh, material, boundary-condition, and dataset configuration.
- **F5:** Research tier: execute only through the declared isolated, pinned research adapter and retain its full configuration/provenance; baseline: F5 behavior exists only as an isolated adapter to a pinned TCAD solver, mesh, material, boundary-condition, and dataset configuration.
...
- **Golden evidence:** `GOLD-RSH-TCAD_PHYSICS-NOMINAL`, `GOLD-RSH-TCAD_PHYSICS-BOUNDARY`, `GOLD-RSH-TCAD_PHYSICS-FAILURE`.
...
The taxonomy entry is project-authored from the platform scope (Source PDF, pp. 34-41). Symbol artwork is original and intended for Apache-2.0 distribution. Imported model licenses remain model-specific and require an SPDX identifier plus redistribution review. IEC references guide terminology only; proprietary symbol artwork is not copied.

## Golden validation

- `GOLD-RSH-TCAD_PHYSICS-NOMINAL`: nominal positive-path behavior.
- `GOLD-RSH-TCAD_PHYSICS-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-RSH-TCAD_PHYSICS-FAILURE`: diagnostic and fault behavior.
