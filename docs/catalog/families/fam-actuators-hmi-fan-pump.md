...
- **Family ID:** `fam-actuators-hmi-fan-pump`
...
Fan and pump is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Fan and pump**, **Fan Pump**, and `fan-pump`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-actuators-hmi-fan-pump-fan` | Fan | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-electromechanical`, `pkg-display-module`, `pkg-custom-parametric` |
| `var-actuators-hmi-fan-pump-blower` | Blower | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-electromechanical`, `pkg-display-module`, `pkg-custom-parametric` |
| `var-actuators-hmi-fan-pump-pump` | Pump | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-electromechanical`, `pkg-display-module`, `pkg-custom-parametric` |
| `var-actuators-hmi-fan-pump-compressor` | Compressor | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-electromechanical`, `pkg-display-module`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-actuators-hmi-fan-pump`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
- `GOLD-ACT-FAN_PUMP-NOMINAL`: nominal positive-path behavior.
- `GOLD-ACT-FAN_PUMP-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-ACT-FAN_PUMP-FAILURE`: diagnostic and fault behavior.
