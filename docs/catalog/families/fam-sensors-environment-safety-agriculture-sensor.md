# Safety and agriculture sensor

- **Family ID:** `fam-sensors-environment-safety-agriculture-sensor`
...
Safety and agriculture sensor is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Safety and agriculture sensor**, **Safety Agriculture Sensor**, and `safety-agriculture-sensor`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-sensors-environment-safety-agriculture-sensor-smoke` | Smoke | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-safety-agriculture-sensor-flame` | Flame | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-safety-agriculture-sensor-soil-moisture` | Soil Moisture | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-safety-agriculture-sensor-rain` | Rain | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-safety-agriculture-sensor-water-level` | Water Level | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-sensors-environment-safety-agriculture-sensor`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `normalized_stimulus` | Normalized stimulus level | 1 | 0 | 0..1 |
| `trip_threshold` | Detection threshold | 1 | 0.5 | 0..1 |
| `hysteresis` | Detection hysteresis | 1 | 0.05 | 0..1 |
| `response_time` | Response time | s | 1 | >=0 |
| `fault_level` | Fault output level | 1 | 1 | 0..1 |
...
- **Governing relation or state rule:** Output follows the selected smoke/moisture/agriculture stimulus transfer with calibrated thresholds, hysteresis, contamination, and fault behavior.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Output follows the selected smoke/moisture/agriculture stimulus transfer with calibrated thresholds, hysteresis, contamination, and fault behavior.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: Output follows the selected smoke/moisture/agriculture stimulus transfer with calibrated thresholds, hysteresis, contamination, and fault behavior.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: Output follows the selected smoke/moisture/agriculture stimulus transfer with calibrated thresholds, hysteresis, contamination, and fault behavior.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Output follows the selected smoke/moisture/agriculture stimulus transfer with calibrated thresholds, hysteresis, contamination, and fault behavior.
...
- **Exact nominal vector:** pins `S:STIMULUS`/physical, `O:OUTPUT`/output, `VDD:VDD`/power, `GND:GND`/power; parameters `normalized_stimulus`=0 1 (0..1); `trip_threshold`=0.5 1 (0..1); `hysteresis`=0.05 1 (0..1); `response_time`=1 s (>=0); `fault_level`=1 1 (0..1).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-SEN-SAFETY_AGRICULTURE_SENSOR-NOMINAL`, `GOLD-SEN-SAFETY_AGRICULTURE_SENSOR-BOUNDARY`, `GOLD-SEN-SAFETY_AGRICULTURE_SENSOR-FAILURE`.
...
- `GOLD-SEN-SAFETY_AGRICULTURE_SENSOR-NOMINAL`: nominal positive-path behavior.
- `GOLD-SEN-SAFETY_AGRICULTURE_SENSOR-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-SEN-SAFETY_AGRICULTURE_SENSOR-FAILURE`: diagnostic and fault behavior.
