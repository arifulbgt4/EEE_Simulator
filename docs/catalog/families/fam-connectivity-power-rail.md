...
- **Family ID:** `fam-connectivity-power-rail`
...
Power rail is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Power rail**, **Power Rail**, and `power-rail`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-connectivity-power-rail-parameterized-rail` | Parameterized Rail | F0, F1 | Realistic Electronics MVP | `pkg-virtual` |
...
- Symbol ID: `sym-connectivity-power-rail`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
- `GOLD-CON-POWER_RAIL-NOMINAL`: nominal positive-path behavior.
- `GOLD-CON-POWER_RAIL-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-CON-POWER_RAIL-FAILURE`: diagnostic and fault behavior.
