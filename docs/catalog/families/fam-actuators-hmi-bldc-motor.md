...
- **Family ID:** `fam-actuators-hmi-bldc-motor`
...
Brushless DC motor is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Brushless DC motor**, **Bldc Motor**, and `bldc-motor`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-actuators-hmi-bldc-motor-sensored` | Sensored | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-electromechanical`, `pkg-display-module`, `pkg-custom-parametric` |
| `var-actuators-hmi-bldc-motor-sensorless` | Sensorless | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-electromechanical`, `pkg-display-module`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-actuators-hmi-bldc-motor`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `phase_resistance` | Per-phase resistance | ohm | 1 | >0 |
| `phase_inductance` | Per-phase inductance | H | 1e-3 | >=0 |
| `torque_constant` | Torque constant | N*m/A | 0.1 | >0 |
| `back_emf_constant` | Back-EMF constant | V*s/rad | 0.1 | >0 |
| `pole_pairs` | Rotor pole pairs | 1 | 4 | 1..1024 |
...
- **Governing relation or state rule:** Phase voltage/current, back-EMF, commutation, torque, rotor position, inertia, and load follow the declared multi-phase electromechanical state model.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Phase voltage/current, back-EMF, commutation, torque, rotor position, inertia, and load follow the declared multi-phase electromechanical state model.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: Phase voltage/current, back-EMF, commutation, torque, rotor position, inertia, and load follow the declared multi-phase electromechanical state model.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: Phase voltage/current, back-EMF, commutation, torque, rotor position, inertia, and load follow the declared multi-phase electromechanical state model.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Phase voltage/current, back-EMF, commutation, torque, rotor position, inertia, and load follow the declared multi-phase electromechanical state model.
...
- **Exact nominal vector:** pins `1:DRIVE+`/input, `2:DRIVE-`/input, `M:MECHANICAL_OR_DISPLAY`/physical; parameters `phase_resistance`=1 ohm (>0); `phase_inductance`=1e-3 H (>=0); `torque_constant`=0.1 N*m/A (>0); `back_emf_constant`=0.1 V*s/rad (>0); `pole_pairs`=4 1 (1..1024); `rotor_inertia`=1e-4 kg*m^2 (>0).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-ACT-BLDC_MOTOR-NOMINAL`, `GOLD-ACT-BLDC_MOTOR-BOUNDARY`, `GOLD-ACT-BLDC_MOTOR-FAILURE`.
...
- `GOLD-ACT-BLDC_MOTOR-NOMINAL`: nominal positive-path behavior.
- `GOLD-ACT-BLDC_MOTOR-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-ACT-BLDC_MOTOR-FAILURE`: diagnostic and fault behavior.
