...
- **Family ID:** `fam-connectivity-bus`
...
Bus conductor is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Bus conductor**, **Bus**, and `bus`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-connectivity-bus-parameterized-bus` | Parameterized Bus | F0, F1 | Realistic Electronics MVP | `pkg-virtual` |
...
- Symbol ID: `sym-connectivity-bus`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---|---|
| `A` | BUS_A | bidirectional | electrical, digital, schematic |
| `B` | BUS_B | bidirectional | electrical, digital, schematic |
...
|---|---|---:|---:|---|
| `width` | Bus width | bit | 1 | 1..4096 |
| `bit_order` | Bit ordering | 1 | ascending | ascending or descending |
| `mapping` | Explicit bit or slice mapping | 1 | identity | bijective declared mapping |
...
- **Governing relation or state rule:** Connectivity is evaluated bit-for-bit over the declared bus width; every source bit maps to exactly one destination bit and width or slice mismatch is an error.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Connectivity is evaluated bit-for-bit over the declared bus width; every source bit maps to exactly one destination bit and width or slice mismatch is an error.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: Connectivity is evaluated bit-for-bit over the declared bus width; every source bit maps to exactly one destination bit and width or slice mismatch is an error.
- **Exact nominal vector:** pins `A:BUS_A`/bidirectional, `B:BUS_B`/bidirectional; parameters `width`=1 bit (1..4096); `bit_order`=ascending 1 (ascending or descending); `mapping`=identity 1 (bijective declared mapping).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-CON-BUS-NOMINAL`, `GOLD-CON-BUS-BOUNDARY`, `GOLD-CON-BUS-FAILURE`.
...
- `GOLD-CON-BUS-NOMINAL`: nominal positive-path behavior.
- `GOLD-CON-BUS-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-CON-BUS-FAILURE`: diagnostic and fault behavior.
