...
- **Family ID:** `fam-sources-loads-electronic-load`
...
Electronic load is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Electronic load**, **Electronic Load**, and `electronic-load`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-sources-loads-electronic-load-cc-cv-cr-cp` | Cc Cv Cr Cp | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-virtual` |
...
- Symbol ID: `sym-sources-loads-electronic-load`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `mode` | Regulation mode | 1 | CC | CC, CV, CR, or CP |
| `current_setpoint` | Current setpoint | A | 1 | >=0 |
| `voltage_setpoint` | Voltage setpoint | V | 5 | >=0 |
| `resistance_setpoint` | Resistance setpoint | ohm | 10 | >0 |
| `power_setpoint` | Power setpoint | W | 5 | >=0 |
| `control_bandwidth` | Control bandwidth | Hz | 1000 | >0 |
...
- **Governing relation or state rule:** A bounded feedback law drives the selected current/power/resistance target subject to bandwidth, compliance voltage, current limit, and thermal state.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: A bounded feedback law drives the selected current/power/resistance target subject to bandwidth, compliance voltage, current limit, and thermal state.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: A bounded feedback law drives the selected current/power/resistance target subject to bandwidth, compliance voltage, current limit, and thermal state.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: A bounded feedback law drives the selected current/power/resistance target subject to bandwidth, compliance voltage, current limit, and thermal state.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: A bounded feedback law drives the selected current/power/resistance target subject to bandwidth, compliance voltage, current limit, and thermal state.
...
- **Exact nominal vector:** pins `1:P`/passive, `2:N`/passive; parameters `mode`=CC 1 (CC, CV, CR, or CP); `current_setpoint`=1 A (>=0); `voltage_setpoint`=5 V (>=0); `resistance_setpoint`=10 ohm (>0); `power_setpoint`=5 W (>=0); `control_bandwidth`=1000 Hz (>0).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-SRC-ELECTRONIC_LOAD-NOMINAL`, `GOLD-SRC-ELECTRONIC_LOAD-BOUNDARY`, `GOLD-SRC-ELECTRONIC_LOAD-FAILURE`.
...
- `GOLD-SRC-ELECTRONIC_LOAD-NOMINAL`: nominal positive-path behavior.
- `GOLD-SRC-ELECTRONIC_LOAD-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-SRC-ELECTRONIC_LOAD-FAILURE`: diagnostic and fault behavior.
