...
- **Family ID:** `fam-passives-crystal-resonator`
...
Crystal and resonator is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Crystal and resonator**, **Crystal Resonator**, and `crystal-resonator`. Variant names are also aliases. Aliases never replace stable IDs.
...
- Symbol ID: `sym-passives-crystal-resonator`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
- `GOLD-PAS-CRYSTAL_RESONATOR-NOMINAL`: nominal positive-path behavior.
- `GOLD-PAS-CRYSTAL_RESONATOR-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-PAS-CRYSTAL_RESONATOR-FAILURE`: diagnostic and fault behavior.
