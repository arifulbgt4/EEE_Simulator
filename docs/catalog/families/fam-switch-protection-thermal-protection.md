...
- **Family ID:** `fam-switch-protection-thermal-protection`
...
Thermal protection is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Thermal protection**, **Thermal Protection**, and `thermal-protection`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-switch-protection-thermal-protection-thermal-fuse` | Thermal Fuse | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-custom-parametric` |
| `var-switch-protection-thermal-protection-thermostat` | Thermostat | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-custom-parametric` |
...
- Symbol ID: `sym-switch-protection-thermal-protection`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `trip_temperature` | Trip temperature | K | 373.15 | >0 |
| `reset_temperature` | Reset temperature | K | 353.15 | 0..trip_temperature |
| `thermal_time_constant` | Thermal response time | s | 1 | >=0 |
| `contact_resistance` | Closed-contact resistance | ohm | 0.01 | >=0 |
| `resettable` | Resettable behavior | 1 | true | true or false |
...
- **Governing relation or state rule:** Temperature drives a hysteretic open/closed or limiting state at declared trip/reset thresholds with thermal delay and fail-safe behavior.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Temperature drives a hysteretic open/closed or limiting state at declared trip/reset thresholds with thermal delay and fail-safe behavior.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: Temperature drives a hysteretic open/closed or limiting state at declared trip/reset thresholds with thermal delay and fail-safe behavior.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: Temperature drives a hysteretic open/closed or limiting state at declared trip/reset thresholds with thermal delay and fail-safe behavior.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Temperature drives a hysteretic open/closed or limiting state at declared trip/reset thresholds with thermal delay and fail-safe behavior.
- **F4:** Electrothermal/tolerance/failure tier: extend the lower-tier relation with declared sampling, power-to-heat state Cth*dT/dt = P-(T-Tamb)/Rth, derating, and deterministic failure transitions; base relation: Temperature drives a hysteretic open/closed or limiting state at declared trip/reset thresholds with thermal delay and fail-safe behavior.
- **Exact nominal vector:** pins `1:P`/passive, `2:N`/passive; parameters `trip_temperature`=373.15 K (>0); `reset_temperature`=353.15 K (0..trip_temperature); `thermal_time_constant`=1 s (>=0); `contact_resistance`=0.01 ohm (>=0); `resettable`=true 1 (true or false).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-SWP-THERMAL_PROTECTION-NOMINAL`, `GOLD-SWP-THERMAL_PROTECTION-BOUNDARY`, `GOLD-SWP-THERMAL_PROTECTION-FAILURE`.
...
- `GOLD-SWP-THERMAL_PROTECTION-NOMINAL`: nominal positive-path behavior.
- `GOLD-SWP-THERMAL_PROTECTION-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-SWP-THERMAL_PROTECTION-FAILURE`: diagnostic and fault behavior.
