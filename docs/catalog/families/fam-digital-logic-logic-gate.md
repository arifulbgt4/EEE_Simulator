...
- **Family ID:** `fam-digital-logic-logic-gate`
...
Logic gate is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Logic gate**, **Logic Gate**, and `logic-gate`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-digital-logic-logic-gate-buffer` | Buffer | F0, F2, F3 | Realistic Electronics MVP | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-digital-logic-logic-gate-inverter` | Inverter | F0, F2, F3 | Realistic Electronics MVP | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-digital-logic-logic-gate-and` | And | F0, F2, F3 | Realistic Electronics MVP | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-digital-logic-logic-gate-nand` | Nand | F0, F2, F3 | Realistic Electronics MVP | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-digital-logic-logic-gate-or` | Or | F0, F2, F3 | Realistic Electronics MVP | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-digital-logic-logic-gate-nor` | Nor | F0, F2, F3 | Realistic Electronics MVP | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-digital-logic-logic-gate-xor` | Xor | F0, F2, F3 | Realistic Electronics MVP | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-digital-logic-logic-gate-xnor` | Xnor | F0, F2, F3 | Realistic Electronics MVP | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-digital-logic-logic-gate`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `input_count` | Gate input count | 1 | 2 | 1..1024 |
| `width` | Vector width | bit | 1 | 1..4096 |
| `rise_delay` | Low-to-high delay | s | 0 | >=0 |
| `fall_delay` | High-to-low delay | s | 0 | >=0 |
| `logic_family` | Threshold and drive profile | 1 | cmos | registered profile |
...
- **Governing relation or state rule:** The selected truth function maps 0/1/X/Z inputs to output, then schedules the result after the declared rise/fall delay and drive-strength rules.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The selected truth function maps 0/1/X/Z inputs to output, then schedules the result after the declared rise/fall delay and drive-strength rules.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: The selected truth function maps 0/1/X/Z inputs to output, then schedules the result after the declared rise/fall delay and drive-strength rules.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: The selected truth function maps 0/1/X/Z inputs to output, then schedules the result after the declared rise/fall delay and drive-strength rules.
- **Exact nominal vector:** pins `1..N:INPUTS`/input, `N+1..M:OUTPUTS`/output, `VDD:VDD`/power, `VSS:VSS`/power; parameters `input_count`=2 1 (1..1024); `width`=1 bit (1..4096); `rise_delay`=0 s (>=0); `fall_delay`=0 s (>=0); `logic_family`=cmos 1 (registered profile).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-DIG-LOGIC_GATE-NOMINAL`, `GOLD-DIG-LOGIC_GATE-BOUNDARY`, `GOLD-DIG-LOGIC_GATE-FAILURE`.
...
- `GOLD-DIG-LOGIC_GATE-NOMINAL`: nominal positive-path behavior.
- `GOLD-DIG-LOGIC_GATE-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-DIG-LOGIC_GATE-FAILURE`: diagnostic and fault behavior.
