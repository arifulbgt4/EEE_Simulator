...
- **Family ID:** `fam-deferred-research-vacuum-tube`
...
Vacuum tube is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Vacuum tube**, **Vacuum Tube**, and `vacuum-tube`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-deferred-research-vacuum-tube-triode` | Triode | F5 | Research | `pkg-custom-parametric` |
| `var-deferred-research-vacuum-tube-pentode` | Pentode | F5 | Research | `pkg-custom-parametric` |
...
- Symbol ID: `sym-deferred-research-vacuum-tube`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
- **Governing relation or state rule:** F5 behavior is supplied only by an approved external vacuum-tube compact/physical model with pinned equations, parameters, provenance, and envelope.
- **F5:** Research tier: execute only through the declared isolated, pinned research adapter and retain its full configuration/provenance; baseline: F5 behavior is supplied only by an approved external vacuum-tube compact/physical model with pinned equations, parameters, provenance, and envelope.
...
- **Golden evidence:** `GOLD-RSH-VACUUM_TUBE-NOMINAL`, `GOLD-RSH-VACUUM_TUBE-BOUNDARY`, `GOLD-RSH-VACUUM_TUBE-FAILURE`.
...
- `GOLD-RSH-VACUUM_TUBE-NOMINAL`: nominal positive-path behavior.
- `GOLD-RSH-VACUUM_TUBE-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-RSH-VACUUM_TUBE-FAILURE`: diagnostic and fault behavior.
