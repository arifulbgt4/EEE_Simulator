# Timing, filter, and converter abstraction

- **Family ID:** `fam-analog-mixed-signal-timing-filter-conversion`
...
Timing, filter, and converter abstraction is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Timing, filter, and converter abstraction**, **Timing Filter Conversion**, and `timing-filter-conversion`. Variant names are also aliases. Aliases never replace stable IDs.
...
- Symbol ID: `sym-analog-mixed-signal-timing-filter-conversion`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
- `GOLD-AMS-TIMING_FILTER_CONVERSION-NOMINAL`: nominal positive-path behavior.
- `GOLD-AMS-TIMING_FILTER_CONVERSION-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-AMS-TIMING_FILTER_CONVERSION-FAILURE`: diagnostic and fault behavior.
