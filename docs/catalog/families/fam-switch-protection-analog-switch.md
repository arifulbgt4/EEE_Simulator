...
- **Family ID:** `fam-switch-protection-analog-switch`
...
Analog switch is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Analog switch**, **Analog Switch**, and `analog-switch`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-switch-protection-analog-switch-spst` | Spst | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-custom-parametric` |
| `var-switch-protection-analog-switch-spdt` | Spdt | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-custom-parametric` |
| `var-switch-protection-analog-switch-multiplexer` | Multiplexer | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-custom-parametric` |
...
- Symbol ID: `sym-switch-protection-analog-switch`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `channel_count` | Signal-channel count | 1 | 1 | 1..1024 |
| `on_resistance` | On resistance | ohm | 10 | >=0 |
| `off_resistance` | Off resistance | ohm | 1e12 | >0 |
| `off_leakage` | Off leakage current | A | 1e-9 | >=0 |
| `break_before_make` | Break-before-make time | s | 0 | >=0 |
...
- **Governing relation or state rule:** The signal path uses control-dependent Ron/Roff plus declared leakage, charge injection, bandwidth, and rail/compliance limits.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The signal path uses control-dependent Ron/Roff plus declared leakage, charge injection, bandwidth, and rail/compliance limits.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: The signal path uses control-dependent Ron/Roff plus declared leakage, charge injection, bandwidth, and rail/compliance limits.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: The signal path uses control-dependent Ron/Roff plus declared leakage, charge injection, bandwidth, and rail/compliance limits.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: The signal path uses control-dependent Ron/Roff plus declared leakage, charge injection, bandwidth, and rail/compliance limits.
...
- **Exact nominal vector:** pins `1:P`/passive, `2:N`/passive; parameters `channel_count`=1 1 (1..1024); `on_resistance`=10 ohm (>=0); `off_resistance`=1e12 ohm (>0); `off_leakage`=1e-9 A (>=0); `break_before_make`=0 s (>=0).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-SWP-ANALOG_SWITCH-NOMINAL`, `GOLD-SWP-ANALOG_SWITCH-BOUNDARY`, `GOLD-SWP-ANALOG_SWITCH-FAILURE`.
...
- `GOLD-SWP-ANALOG_SWITCH-NOMINAL`: nominal positive-path behavior.
- `GOLD-SWP-ANALOG_SWITCH-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-SWP-ANALOG_SWITCH-FAILURE`: diagnostic and fault behavior.
