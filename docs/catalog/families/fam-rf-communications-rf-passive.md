...
- **Family ID:** `fam-rf-communications-rf-passive`
...
RF passive is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **RF passive**, **Rf Passive**, and `rf-passive`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-rf-communications-rf-passive-attenuator` | Attenuator | F1, F2, F3 | Post-MVP catalog | `pkg-rf-module`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-rf-communications-rf-passive-balun` | Balun | F1, F2, F3 | Post-MVP catalog | `pkg-rf-module`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-rf-communications-rf-passive-coupler` | Coupler | F1, F2, F3 | Post-MVP catalog | `pkg-rf-module`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-rf-communications-rf-passive-splitter-combiner` | Splitter Combiner | F1, F2, F3 | Post-MVP catalog | `pkg-rf-module`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-rf-communications-rf-passive`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
- `GOLD-RFC-RF_PASSIVE-NOMINAL`: nominal positive-path behavior.
- `GOLD-RFC-RF_PASSIVE-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-RFC-RF_PASSIVE-FAILURE`: diagnostic and fault behavior.
