...
- **Family ID:** `fam-analog-mixed-signal-analog-mux`
...
Analog multiplexer is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Analog multiplexer**, **Analog Mux**, and `analog-mux`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-analog-mixed-signal-analog-mux-mux` | Mux | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-analog-mixed-signal-analog-mux-demux` | Demux | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-analog-mixed-signal-analog-mux-crosspoint` | Crosspoint | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-analog-mixed-signal-analog-mux`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `channel_count` | Analog channel count | 1 | 2 | 2..1024 |
| `on_resistance` | Selected-channel resistance | ohm | 10 | >=0 |
| `off_leakage` | Unselected-channel leakage | A | 1e-9 | >=0 |
| `break_before_make` | Break-before-make interval | s | 0 | >=0 |
| `charge_injection` | Equivalent switching charge | C | 0 | >=0 |
...
- **Governing relation or state rule:** The selected channel alone connects through finite Ron; invalid/unknown select states, break-before-make timing, leakage, and injection are explicit.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The selected channel alone connects through finite Ron; invalid/unknown select states, break-before-make timing, leakage, and injection are explicit.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: The selected channel alone connects through finite Ron; invalid/unknown select states, break-before-make timing, leakage, and injection are explicit.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: The selected channel alone connects through finite Ron; invalid/unknown select states, break-before-make timing, leakage, and injection are explicit.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: The selected channel alone connects through finite Ron; invalid/unknown select states, break-before-make timing, leakage, and injection are explicit.
...
- **Exact nominal vector:** pins `1:IN+`/input, `2:IN-`/input, `3:OUT`/output, `4:V+`/power, `5:V-`/power; parameters `channel_count`=2 1 (2..1024); `on_resistance`=10 ohm (>=0); `off_leakage`=1e-9 A (>=0); `break_before_make`=0 s (>=0); `charge_injection`=0 C (>=0).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-AMS-ANALOG_MUX-NOMINAL`, `GOLD-AMS-ANALOG_MUX-BOUNDARY`, `GOLD-AMS-ANALOG_MUX-FAILURE`.
...
- `GOLD-AMS-ANALOG_MUX-NOMINAL`: nominal positive-path behavior.
- `GOLD-AMS-ANALOG_MUX-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-AMS-ANALOG_MUX-FAILURE`: diagnostic and fault behavior.
