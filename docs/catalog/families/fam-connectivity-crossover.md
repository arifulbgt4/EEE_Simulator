# Non-connected crossover

- **Family ID:** `fam-connectivity-crossover`
...
Non-connected crossover is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Non-connected crossover**, **Crossover**, and `crossover`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-connectivity-crossover-bridge-crossover` | Bridge Crossover | F0, F1 | Realistic Electronics MVP | `pkg-virtual` |
...
- Symbol ID: `sym-connectivity-crossover`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---|---|
| `A1` | A_IN | passive | electrical, schematic |
| `A2` | A_OUT | passive | electrical, schematic |
| `B1` | B_IN | passive | electrical, schematic |
| `B2` | B_OUT | passive | electrical, schematic |
...
|---|---|---:|---:|---|
| `bridge_path` | Visually bridged path | 1 | A | A or B |
| `bridge_style` | Non-connection visual style | 1 | arc | arc or gap |
| `crossing_clearance` | Rendered crossing clearance | m | 0.001 | >0 |
...
- **Governing relation or state rule:** The two crossing conductor paths remain separate equivalence classes: A1 is connected only to A2 and B1 only to B2; no crossing-node merge is permitted.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The two crossing conductor paths remain separate equivalence classes: A1 is connected only to A2 and B1 only to B2; no crossing-node merge is permitted.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: The two crossing conductor paths remain separate equivalence classes: A1 is connected only to A2 and B1 only to B2; no crossing-node merge is permitted.
- **Exact nominal vector:** pins `A1:A_IN`/passive, `A2:A_OUT`/passive, `B1:B_IN`/passive, `B2:B_OUT`/passive; parameters `bridge_path`=A 1 (A or B); `bridge_style`=arc 1 (arc or gap); `crossing_clearance`=0.001 m (>0).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-CON-CROSSOVER-NOMINAL`, `GOLD-CON-CROSSOVER-BOUNDARY`, `GOLD-CON-CROSSOVER-FAILURE`.
...
- `GOLD-CON-CROSSOVER-NOMINAL`: nominal positive-path behavior.
- `GOLD-CON-CROSSOVER-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-CON-CROSSOVER-FAILURE`: diagnostic and fault behavior.
