...
- **Family ID:** `fam-sources-loads-programmable-load`
...
Programmable load is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Programmable load**, **Programmable Load**, and `programmable-load`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-sources-loads-programmable-load-profiled-load` | Profiled Load | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-virtual` |
...
- Symbol ID: `sym-sources-loads-programmable-load`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `mode` | Initial load mode | 1 | CC | CC, CV, CR, or CP |
| `current_setpoint` | CC setpoint | A | 1 | >=0 |
| `voltage_setpoint` | CV setpoint | V | 5 | >=0 |
| `resistance_setpoint` | CR setpoint | ohm | 10 | >0 |
| `power_setpoint` | CP setpoint | W | 5 | >=0 |
| `slew_time` | Setpoint transition time | s | 0 | >=0 |
...
- **Governing relation or state rule:** The selected R/I/P setpoint is a deterministic function of time or control input with declared slew, compliance, and low-voltage behavior.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The selected R/I/P setpoint is a deterministic function of time or control input with declared slew, compliance, and low-voltage behavior.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: The selected R/I/P setpoint is a deterministic function of time or control input with declared slew, compliance, and low-voltage behavior.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: The selected R/I/P setpoint is a deterministic function of time or control input with declared slew, compliance, and low-voltage behavior.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: The selected R/I/P setpoint is a deterministic function of time or control input with declared slew, compliance, and low-voltage behavior.
- **F4:** Electrothermal/tolerance/failure tier: extend the lower-tier relation with declared sampling, power-to-heat state Cth*dT/dt = P-(T-Tamb)/Rth, derating, and deterministic failure transitions; base relation: The selected R/I/P setpoint is a deterministic function of time or control input with declared slew, compliance, and low-voltage behavior.
- **Exact nominal vector:** pins `1:P`/passive, `2:N`/passive; parameters `mode`=CC 1 (CC, CV, CR, or CP); `current_setpoint`=1 A (>=0); `voltage_setpoint`=5 V (>=0); `resistance_setpoint`=10 ohm (>0); `power_setpoint`=5 W (>=0); `slew_time`=0 s (>=0).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-SRC-PROGRAMMABLE_LOAD-NOMINAL`, `GOLD-SRC-PROGRAMMABLE_LOAD-BOUNDARY`, `GOLD-SRC-PROGRAMMABLE_LOAD-FAILURE`.
...
- `GOLD-SRC-PROGRAMMABLE_LOAD-NOMINAL`: nominal positive-path behavior.
- `GOLD-SRC-PROGRAMMABLE_LOAD-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-SRC-PROGRAMMABLE_LOAD-FAILURE`: diagnostic and fault behavior.
