...
- **Family ID:** `fam-sensors-environment-water-quality-sensor`
...
Water quality sensor is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Water quality sensor**, **Water Quality Sensor**, and `water-quality-sensor`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-sensors-environment-water-quality-sensor-ph` | Ph | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-water-quality-sensor-conductivity` | Conductivity | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-water-quality-sensor-turbidity` | Turbidity | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-water-quality-sensor-dissolved-oxygen` | Dissolved Oxygen | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-water-quality-sensor-salinity` | Salinity | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-water-quality-sensor-orp` | Orp | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-sensors-environment-water-quality-sensor`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `ph` | Hydrogen-ion activity index | 1 | 7 | 0..14 |
| `conductivity` | Electrical conductivity | S/m | 0.1 | >=0 |
| `turbidity` | Turbidity | FNU | 0 | >=0 |
| `dissolved_oxygen` | Dissolved oxygen concentration | kg/m^3 | 0.008 | >=0 |
| `temperature` | Water temperature | K | 298.15 | >0 |
| `response_time` | Probe response time | s | 1 | >=0 |
...
- **Governing relation or state rule:** Output follows the selected pH/conductivity/turbidity/dissolved-oxygen transfer with temperature compensation, calibration, range, fouling, and drift.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Output follows the selected pH/conductivity/turbidity/dissolved-oxygen transfer with temperature compensation, calibration, range, fouling, and drift.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: Output follows the selected pH/conductivity/turbidity/dissolved-oxygen transfer with temperature compensation, calibration, range, fouling, and drift.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: Output follows the selected pH/conductivity/turbidity/dissolved-oxygen transfer with temperature compensation, calibration, range, fouling, and drift.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Output follows the selected pH/conductivity/turbidity/dissolved-oxygen transfer with temperature compensation, calibration, range, fouling, and drift.
...
- **Exact nominal vector:** pins `S:STIMULUS`/physical, `O:OUTPUT`/output, `VDD:VDD`/power, `GND:GND`/power; parameters `ph`=7 1 (0..14); `conductivity`=0.1 S/m (>=0); `turbidity`=0 FNU (>=0); `dissolved_oxygen`=0.008 kg/m^3 (>=0); `temperature`=298.15 K (>0); `response_time`=1 s (>=0).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-SEN-WATER_QUALITY_SENSOR-NOMINAL`, `GOLD-SEN-WATER_QUALITY_SENSOR-BOUNDARY`, `GOLD-SEN-WATER_QUALITY_SENSOR-FAILURE`.
...
- `GOLD-SEN-WATER_QUALITY_SENSOR-NOMINAL`: nominal positive-path behavior.
- `GOLD-SEN-WATER_QUALITY_SENSOR-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-SEN-WATER_QUALITY_SENSOR-FAILURE`: diagnostic and fault behavior.
