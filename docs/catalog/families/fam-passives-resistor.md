...
- **Family ID:** `fam-passives-resistor`
...
Resistor is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Resistor**, **Resistor**, and `resistor`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-passives-resistor-carbon-film` | Carbon Film | F0, F1, F2, F3, F4 | Realistic Electronics MVP | `pkg-axial-2`, `pkg-smd-chip`, `pkg-radial-2` |
| `var-passives-resistor-metal-film` | Metal Film | F0, F1, F2, F3, F4 | Realistic Electronics MVP | `pkg-axial-2`, `pkg-smd-chip`, `pkg-radial-2` |
| `var-passives-resistor-wirewound` | Wirewound | F0, F1, F2, F3, F4 | Realistic Electronics MVP | `pkg-axial-2`, `pkg-smd-chip`, `pkg-radial-2` |
| `var-passives-resistor-thick-film` | Thick Film | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-axial-2`, `pkg-smd-chip`, `pkg-radial-2` |
| `var-passives-resistor-current-sense` | Current Sense | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-axial-2`, `pkg-smd-chip`, `pkg-radial-2` |
...
- Symbol ID: `sym-passives-resistor`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `resistance` | Nominal resistance | ohm | 1000 | >0 |
| `tolerance` | Manufacturing tolerance | 1 | 0.05 | 0..1 |
| `temperature_coefficient` | Resistance temperature coefficient | 1/K | 0 | finite |
| `power_rating` | Continuous power rating | W | 0.25 | >0 |
...
- **Governing relation or state rule:** The ideal relation is v = R*i and p = v*i = i^2*R; temperature/tolerance profiles compute R(T,sample) from the declared coefficient and seed.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The ideal relation is v = R*i and p = v*i = i^2*R; temperature/tolerance profiles compute R(T,sample) from the declared coefficient and seed.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: The ideal relation is v = R*i and p = v*i = i^2*R; temperature/tolerance profiles compute R(T,sample) from the declared coefficient and seed.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: The ideal relation is v = R*i and p = v*i = i^2*R; temperature/tolerance profiles compute R(T,sample) from the declared coefficient and seed.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: The ideal relation is v = R*i and p = v*i = i^2*R; temperature/tolerance profiles compute R(T,sample) from the declared coefficient and seed.
- **F4:** Electrothermal/tolerance/failure tier: extend the lower-tier relation with declared sampling, power-to-heat state Cth*dT/dt = P-(T-Tamb)/Rth, derating, and deterministic failure transitions; base relation: The ideal relation is v = R*i and p = v*i = i^2*R; temperature/tolerance profiles compute R(T,sample) from the declared coefficient and seed.
- **Exact nominal vector:** pins `1:P`/passive, `2:N`/passive; parameters `resistance`=1000 ohm (>0); `tolerance`=0.05 1 (0..1); `temperature_coefficient`=0 1/K (finite); `power_rating`=0.25 W (>0).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-PAS-RESISTOR-NOMINAL`, `GOLD-PAS-RESISTOR-BOUNDARY`, `GOLD-PAS-RESISTOR-FAILURE`.
...
- `GOLD-PAS-RESISTOR-NOMINAL`: nominal positive-path behavior.
- `GOLD-PAS-RESISTOR-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-PAS-RESISTOR-FAILURE`: diagnostic and fault behavior.
