...
- **Family ID:** `fam-digital-logic-register`
...
Register is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Register**, **Register**, and `register`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-digital-logic-register-shift` | Shift | F0, F2, F3 | Realistic Electronics MVP | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-digital-logic-register-parallel` | Parallel | F0, F2, F3 | Realistic Electronics MVP | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-digital-logic-register-serial-converter` | Serial Converter | F0, F2, F3 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-digital-logic-register`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `width` | Register width | bit | 8 | 1..4096 |
| `operation` | Register operation | 1 | parallel | parallel, shift, or serial-convert |
| `initial_state` | Initial register vector | 1 | 0 | width-matched 0/1/X vector |
...
- **Governing relation or state rule:** A fixed-width state vector updates on the declared edge/enable/load operation and exposes deterministic reset, shift, timing, and X behavior.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: A fixed-width state vector updates on the declared edge/enable/load operation and exposes deterministic reset, shift, timing, and X behavior.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: A fixed-width state vector updates on the declared edge/enable/load operation and exposes deterministic reset, shift, timing, and X behavior.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: A fixed-width state vector updates on the declared edge/enable/load operation and exposes deterministic reset, shift, timing, and X behavior.
- **Exact nominal vector:** pins `1..N:INPUTS`/input, `N+1..M:OUTPUTS`/output, `VDD:VDD`/power, `VSS:VSS`/power; parameters `width`=8 bit (1..4096); `operation`=parallel 1 (parallel, shift, or serial-convert); `initial_state`=0 1 (width-matched 0/1/X vector); `clock_to_output`=0 s (>=0).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-DIG-REGISTER-NOMINAL`, `GOLD-DIG-REGISTER-BOUNDARY`, `GOLD-DIG-REGISTER-FAILURE`.
...
- `GOLD-DIG-REGISTER-NOMINAL`: nominal positive-path behavior.
- `GOLD-DIG-REGISTER-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-DIG-REGISTER-FAILURE`: diagnostic and fault behavior.
