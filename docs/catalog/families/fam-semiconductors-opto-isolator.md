# Optically isolated semiconductor

- **Family ID:** `fam-semiconductors-opto-isolator`
...
Optically isolated semiconductor is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Optically isolated semiconductor**, **Opto Isolator**, and `opto-isolator`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-semiconductors-opto-isolator-phototransistor-output` | Phototransistor Output | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sot-23`, `pkg-to-92`, `pkg-to-220`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-semiconductors-opto-isolator-triac-output` | Triac Output | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sot-23`, `pkg-to-92`, `pkg-to-220`, `pkg-qfn`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-semiconductors-opto-isolator`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `input_forward_voltage` | Input LED forward voltage | V | 1.2 | >0 |
| `current_transfer_ratio` | Current transfer ratio | 1 | 1 | >=0 |
| `propagation_delay` | Input-to-output delay | s | 10e-6 | >=0 |
| `isolation_capacitance` | Barrier capacitance | F | 1e-12 | >=0 |
| `breakdown_voltage` | Isolation breakdown voltage | V | 1000 | >0 |
...
- **Governing relation or state rule:** Input diode current produces output photocurrent or logic transfer through declared CTR, delay, saturation, isolation leakage, and temperature behavior.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Input diode current produces output photocurrent or logic transfer through declared CTR, delay, saturation, isolation leakage, and temperature behavior.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: Input diode current produces output photocurrent or logic transfer through declared CTR, delay, saturation, isolation leakage, and temperature behavior.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: Input diode current produces output photocurrent or logic transfer through declared CTR, delay, saturation, isolation leakage, and temperature behavior.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Input diode current produces output photocurrent or logic transfer through declared CTR, delay, saturation, isolation leakage, and temperature behavior.
- **F4:** Electrothermal/tolerance/failure tier: extend the lower-tier relation with declared sampling, power-to-heat state Cth*dT/dt = P-(T-Tamb)/Rth, derating, and deterministic failure transitions; base relation: Input diode current produces output photocurrent or logic transfer through declared CTR, delay, saturation, isolation leakage, and temperature behavior.
- **Exact nominal vector:** pins `1:P`/passive, `2:N`/passive; parameters `input_forward_voltage`=1.2 V (>0); `current_transfer_ratio`=1 1 (>=0); `propagation_delay`=10e-6 s (>=0); `isolation_capacitance`=1e-12 F (>=0); `breakdown_voltage`=1000 V (>0).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-SEM-OPTO_ISOLATOR-NOMINAL`, `GOLD-SEM-OPTO_ISOLATOR-BOUNDARY`, `GOLD-SEM-OPTO_ISOLATOR-FAILURE`.
...
- `GOLD-SEM-OPTO_ISOLATOR-NOMINAL`: nominal positive-path behavior.
- `GOLD-SEM-OPTO_ISOLATOR-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-SEM-OPTO_ISOLATOR-FAILURE`: diagnostic and fault behavior.
