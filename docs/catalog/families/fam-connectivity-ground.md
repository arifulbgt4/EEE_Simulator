...
- **Family ID:** `fam-connectivity-ground`
- **Category:** Connectivity and schematic primitives
- **Lifecycle:** Planned
- **Basic component tag:** `basic-component`
- **Release target:** Realistic Electronics MVP
- **Source:** Source PDF, pp. 6-8

## Purpose and scope

Ground reference is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Ground reference**, **Ground**, and `ground`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-connectivity-ground-earth` | Earth | F0, F1 | Realistic Electronics MVP | `pkg-virtual` |
| `var-connectivity-ground-signal` | Signal | F0, F1 | Realistic Electronics MVP | `pkg-virtual` |
| `var-connectivity-ground-chassis` | Chassis | F0, F1 | Realistic Electronics MVP | `pkg-virtual` |
| `var-connectivity-ground-analog` | Analog | F0, F1 | Realistic Electronics MVP | `pkg-virtual` |
| `var-connectivity-ground-digital` | Digital | F0, F1 | Realistic Electronics MVP | `pkg-virtual` |
| `var-connectivity-ground-power` | Power | F0, F1 | Realistic Electronics MVP | `pkg-virtual` |
...
- Symbol ID: `sym-connectivity-ground`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `reference_kind` | Reference domain | 1 | signal | signal, analog, digital, power, chassis, earth, or protective-earth |
| `reference_potential` | Reference potential | V | 0 | finite |
| `bond_impedance` | Optional explicit bond impedance | ohm | 0 | >=0 |

All numerical values use SI base units internally. Display prefixes and localized formatting are presentation concerns. Variant-specific parameters may refine this table but may not weaken its validation rules.

## Fidelity and supported analyses

Supported fidelity tiers: **F0, F1**. Supported analysis capabilities: **connectivity, ERC**.
...
- **Governing relation or state rule:** The selected electrical reference domain fixes its reference-node potential to 0 V; signal, chassis, earth, and protective-earth domains remain distinct unless an explicit adapter connects them.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The selected electrical reference domain fixes its reference-node potential to 0 V; signal, chassis, earth, and protective-earth domains remain distinct unless an explicit adapter connects them.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: The selected electrical reference domain fixes its reference-node potential to 0 V; signal, chassis, earth, and protective-earth domains remain distinct unless an explicit adapter connects them.
- **Exact nominal vector:** pins `1:NET`/power; parameters `reference_kind`=signal 1 (signal, analog, digital, power, chassis, earth, or protective-earth); `reference_potential`=0 V (finite); `bond_impedance`=0 ohm (>=0).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-CON-GROUND-NOMINAL`, `GOLD-CON-GROUND-BOUNDARY`, `GOLD-CON-GROUND-FAILURE`.
...
The taxonomy entry is project-authored from the platform scope (Source PDF, pp. 6-8). Symbol artwork is original and intended for Apache-2.0 distribution. Imported model licenses remain model-specific and require an SPDX identifier plus redistribution review. IEC references guide terminology only; proprietary symbol artwork is not copied.

## Golden validation

- `GOLD-CON-GROUND-NOMINAL`: nominal positive-path behavior.
- `GOLD-CON-GROUND-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-CON-GROUND-FAILURE`: diagnostic and fault behavior.
