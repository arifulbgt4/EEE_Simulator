...
- **Family ID:** `fam-computer-gpu-cpu-building-block`
...
CPU building block is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **CPU building block**, **Cpu Building Block**, and `cpu-building-block`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-computer-gpu-cpu-building-block-alu` | Alu | F0, F2 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-computer-gpu-cpu-building-block-register-file` | Register File | F0, F2 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-computer-gpu-cpu-building-block-control-unit` | Control Unit | F0, F2 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-computer-gpu-cpu-building-block-datapath` | Datapath | F0, F2 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-computer-gpu-cpu-building-block-pipeline` | Pipeline | F0, F2 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-computer-gpu-cpu-building-block-cache` | Cache | F0, F2 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-computer-gpu-cpu-building-block-mmu` | Mmu | F0, F2 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-computer-gpu-cpu-building-block`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
- `GOLD-CPU-CPU_BUILDING_BLOCK-NOMINAL`: nominal positive-path behavior.
- `GOLD-CPU-CPU_BUILDING_BLOCK-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-CPU-CPU_BUILDING_BLOCK-FAILURE`: diagnostic and fault behavior.
