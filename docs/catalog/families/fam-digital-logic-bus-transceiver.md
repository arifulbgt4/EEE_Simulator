...
- **Family ID:** `fam-digital-logic-bus-transceiver`
...
Bus transceiver is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Bus transceiver**, **Bus Transceiver**, and `bus-transceiver`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-digital-logic-bus-transceiver-bidirectional` | Bidirectional | F0, F2, F3 | Realistic Electronics MVP | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-digital-logic-bus-transceiver-open-drain` | Open Drain | F0, F2, F3 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-digital-logic-bus-transceiver`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `width` | Bus width | bit | 8 | 1..4096 |
| `direction` | Initial transfer direction | 1 | A-to-B | A-to-B or B-to-A |
| `enable_polarity` | Output-enable polarity | 1 | active-low | active-high or active-low |
| `propagation_delay` | Bus propagation delay | s | 0 | >=0 |
| `logic_family` | Drive profile | 1 | cmos | registered profile |
...
- **Governing relation or state rule:** Direction and enable map fixed-width data between buses through declared drive strengths; conflicts, Z, and X resolve deterministically.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Direction and enable map fixed-width data between buses through declared drive strengths; conflicts, Z, and X resolve deterministically.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: Direction and enable map fixed-width data between buses through declared drive strengths; conflicts, Z, and X resolve deterministically.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Direction and enable map fixed-width data between buses through declared drive strengths; conflicts, Z, and X resolve deterministically.
- **Exact nominal vector:** pins `1..N:INPUTS`/input, `N+1..M:OUTPUTS`/output, `VDD:VDD`/power, `VSS:VSS`/power; parameters `width`=8 bit (1..4096); `direction`=A-to-B 1 (A-to-B or B-to-A); `enable_polarity`=active-low 1 (active-high or active-low); `propagation_delay`=0 s (>=0); `logic_family`=cmos 1 (registered profile).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-DIG-BUS_TRANSCEIVER-NOMINAL`, `GOLD-DIG-BUS_TRANSCEIVER-BOUNDARY`, `GOLD-DIG-BUS_TRANSCEIVER-FAILURE`.
...
- `GOLD-DIG-BUS_TRANSCEIVER-NOMINAL`: nominal positive-path behavior.
- `GOLD-DIG-BUS_TRANSCEIVER-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-DIG-BUS_TRANSCEIVER-FAILURE`: diagnostic and fault behavior.
