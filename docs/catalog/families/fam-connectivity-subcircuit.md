...
- **Family ID:** `fam-connectivity-subcircuit`
...
Hierarchical subcircuit is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Hierarchical subcircuit**, **Subcircuit**, and `subcircuit`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-connectivity-subcircuit-subcircuit-instance` | Subcircuit Instance | F0, F1 | Realistic Electronics MVP | `pkg-virtual` |
...
- Symbol ID: `sym-connectivity-subcircuit`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---|---|
| `1..N` | PORTS | configurable | electrical, schematic |
...
|---|---|---:|---:|---|
| `definition_id` | Referenced subcircuit ID | 1 | builtin-empty | registered stable ID |
| `parameter_bindings` | Instance parameter map | 1 | empty | schema-valid map |
| `recursion_limit` | Hierarchy expansion limit | 1 | 64 | 1..1024 |
...
- **Governing relation or state rule:** Instantiation expands the referenced graph with scoped node IDs and an explicit parameter map; recursion and unresolved ports are rejected.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Instantiation expands the referenced graph with scoped node IDs and an explicit parameter map; recursion and unresolved ports are rejected.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: Instantiation expands the referenced graph with scoped node IDs and an explicit parameter map; recursion and unresolved ports are rejected.
- **Exact nominal vector:** pins `1..N:PORTS`/configurable; parameters `definition_id`=builtin-empty 1 (registered stable ID); `parameter_bindings`=empty 1 (schema-valid map); `recursion_limit`=64 1 (1..1024).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-CON-SUBCIRCUIT-NOMINAL`, `GOLD-CON-SUBCIRCUIT-BOUNDARY`, `GOLD-CON-SUBCIRCUIT-FAILURE`.
...
- `GOLD-CON-SUBCIRCUIT-NOMINAL`: nominal positive-path behavior.
- `GOLD-CON-SUBCIRCUIT-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-CON-SUBCIRCUIT-FAILURE`: diagnostic and fault behavior.
