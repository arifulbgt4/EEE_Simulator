...
- **Family ID:** `fam-sources-loads-dependent-source`
...
Controlled source is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Controlled source**, **Dependent Source**, and `dependent-source`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-sources-loads-dependent-source-vcvs` | Vcvs | F0, F1, F2, F3, F4 | Realistic Electronics MVP | `pkg-virtual` |
| `var-sources-loads-dependent-source-vccs` | Vccs | F0, F1, F2, F3, F4 | Realistic Electronics MVP | `pkg-virtual` |
| `var-sources-loads-dependent-source-ccvs` | Ccvs | F0, F1, F2, F3, F4 | Realistic Electronics MVP | `pkg-virtual` |
| `var-sources-loads-dependent-source-cccs` | Cccs | F0, F1, F2, F3, F4 | Realistic Electronics MVP | `pkg-virtual` |
...
- Symbol ID: `sym-sources-loads-dependent-source`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
- `GOLD-SRC-DEPENDENT_SOURCE-NOMINAL`: nominal positive-path behavior.
- `GOLD-SRC-DEPENDENT_SOURCE-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-SRC-DEPENDENT_SOURCE-FAILURE`: diagnostic and fault behavior.
