...
- **Family ID:** `fam-sources-loads-mains-source`
...
Mains source is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Mains source**, **Mains Source**, and `mains-source`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-sources-loads-mains-source-single-phase` | Single Phase | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-virtual` |
...
- Symbol ID: `sym-sources-loads-mains-source`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `rms_voltage` | RMS line voltage | V | 230 | >0 |
| `frequency` | Line frequency | Hz | 50 | >0 |
| `phase` | Initial phase | rad | 0 | finite |
| `source_impedance` | Line source impedance | ohm | 0.1 | >=0 |
...
- **Governing relation or state rule:** The ideal baseline is v(t) = sqrt(2)*V_rms*sin(2*pi*f*t + phase); distortion, source impedance, and protective limits are explicit higher-fidelity parameters.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The ideal baseline is v(t) = sqrt(2)*V_rms*sin(2*pi*f*t + phase); distortion, source impedance, and protective limits are explicit higher-fidelity parameters.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: The ideal baseline is v(t) = sqrt(2)*V_rms*sin(2*pi*f*t + phase); distortion, source impedance, and protective limits are explicit higher-fidelity parameters.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: The ideal baseline is v(t) = sqrt(2)*V_rms*sin(2*pi*f*t + phase); distortion, source impedance, and protective limits are explicit higher-fidelity parameters.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: The ideal baseline is v(t) = sqrt(2)*V_rms*sin(2*pi*f*t + phase); distortion, source impedance, and protective limits are explicit higher-fidelity parameters.
- **F4:** Electrothermal/tolerance/failure tier: extend the lower-tier relation with declared sampling, power-to-heat state Cth*dT/dt = P-(T-Tamb)/Rth, derating, and deterministic failure transitions; base relation: The ideal baseline is v(t) = sqrt(2)*V_rms*sin(2*pi*f*t + phase); distortion, source impedance, and protective limits are explicit higher-fidelity parameters.
- **Exact nominal vector:** pins `1:P`/passive, `2:N`/passive; parameters `rms_voltage`=230 V (>0); `frequency`=50 Hz (>0); `phase`=0 rad (finite); `source_impedance`=0.1 ohm (>=0).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-SRC-MAINS_SOURCE-NOMINAL`, `GOLD-SRC-MAINS_SOURCE-BOUNDARY`, `GOLD-SRC-MAINS_SOURCE-FAILURE`.
...
- `GOLD-SRC-MAINS_SOURCE-NOMINAL`: nominal positive-path behavior.
- `GOLD-SRC-MAINS_SOURCE-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-SRC-MAINS_SOURCE-FAILURE`: diagnostic and fault behavior.
