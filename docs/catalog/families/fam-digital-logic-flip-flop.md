...
- **Family ID:** `fam-digital-logic-flip-flop`
...
Flip-flop is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Flip-flop**, **Flip Flop**, and `flip-flop`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-digital-logic-flip-flop-d` | D | F0, F2, F3 | Realistic Electronics MVP | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-digital-logic-flip-flop-jk` | Jk | F0, F2, F3 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-digital-logic-flip-flop-t` | T | F0, F2, F3 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
| `var-digital-logic-flip-flop-sr` | Sr | F0, F2, F3 | Post-MVP catalog | `pkg-dip`, `pkg-soic`, `pkg-tssop`, `pkg-qfp`, `pkg-qfn`, `pkg-bga`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-digital-logic-flip-flop`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `width` | Stored width | bit | 1 | 1..4096 |
| `edge` | Active clock edge | 1 | rising | rising or falling |
| `initial_state` | Initial stored vector | 1 | X | width-matched 0/1/X vector |
| `clock_to_q` | Clock-to-output delay | s | 0 | >=0 |
| `setup_time` | Setup time | s | 0 | >=0 |
| `hold_time` | Hold time | s | 0 | >=0 |
...
- **Governing relation or state rule:** Stored state updates on the declared clock edge from D/JK/T/SR inputs, subject to async controls, setup/hold, clock, and X rules.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Stored state updates on the declared clock edge from D/JK/T/SR inputs, subject to async controls, setup/hold, clock, and X rules.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: Stored state updates on the declared clock edge from D/JK/T/SR inputs, subject to async controls, setup/hold, clock, and X rules.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Stored state updates on the declared clock edge from D/JK/T/SR inputs, subject to async controls, setup/hold, clock, and X rules.
- **Exact nominal vector:** pins `1..N:INPUTS`/input, `N+1..M:OUTPUTS`/output, `VDD:VDD`/power, `VSS:VSS`/power; parameters `width`=1 bit (1..4096); `edge`=rising 1 (rising or falling); `initial_state`=X 1 (width-matched 0/1/X vector); `clock_to_q`=0 s (>=0); `setup_time`=0 s (>=0); `hold_time`=0 s (>=0).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-DIG-FLIP_FLOP-NOMINAL`, `GOLD-DIG-FLIP_FLOP-BOUNDARY`, `GOLD-DIG-FLIP_FLOP-FAILURE`.
...
- `GOLD-DIG-FLIP_FLOP-NOMINAL`: nominal positive-path behavior.
- `GOLD-DIG-FLIP_FLOP-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-DIG-FLIP_FLOP-FAILURE`: diagnostic and fault behavior.
