...
- **Family ID:** `fam-semiconductors-hemt`
...
High-electron-mobility transistor is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **High-electron-mobility transistor**, **Hemt**, and `hemt`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-semiconductors-hemt-gaas` | Gaas | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sot-23`, `pkg-to-92`, `pkg-to-220`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-semiconductors-hemt-gan` | Gan | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sot-23`, `pkg-to-92`, `pkg-to-220`, `pkg-qfn`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-semiconductors-hemt`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `threshold_voltage` | Gate threshold voltage | V | -2 | finite |
| `transconductance` | Channel transconductance | S | 1 | >0 |
| `on_resistance` | Channel on resistance | ohm | 0.05 | >=0 |
| `input_capacitance` | Input capacitance | F | 1e-9 | >=0 |
| `trap_time_constant` | Trapping time constant | s | 0 | >=0 |
...
- **Governing relation or state rule:** The GaN/SiC HEMT profile binds a declared channel/charge model with trapping, capacitance, switching, and thermal envelope; no vendor claim is inferred.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The GaN/SiC HEMT profile binds a declared channel/charge model with trapping, capacitance, switching, and thermal envelope; no vendor claim is inferred.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: The GaN/SiC HEMT profile binds a declared channel/charge model with trapping, capacitance, switching, and thermal envelope; no vendor claim is inferred.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: The GaN/SiC HEMT profile binds a declared channel/charge model with trapping, capacitance, switching, and thermal envelope; no vendor claim is inferred.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: The GaN/SiC HEMT profile binds a declared channel/charge model with trapping, capacitance, switching, and thermal envelope; no vendor claim is inferred.
...
- **Exact nominal vector:** pins `1:D`/passive, `2:G`/input, `3:S`/passive, `4:B`/optional; parameters `threshold_voltage`=-2 V (finite); `transconductance`=1 S (>0); `on_resistance`=0.05 ohm (>=0); `input_capacitance`=1e-9 F (>=0); `trap_time_constant`=0 s (>=0); `temperature`=300.15 K (>0).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-SEM-HEMT-NOMINAL`, `GOLD-SEM-HEMT-BOUNDARY`, `GOLD-SEM-HEMT-FAILURE`.
...
- `GOLD-SEM-HEMT-NOMINAL`: nominal positive-path behavior.
- `GOLD-SEM-HEMT-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-SEM-HEMT-FAILURE`: diagnostic and fault behavior.
