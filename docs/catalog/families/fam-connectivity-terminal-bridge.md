# Terminal, net tie, jumper, and domain adapter

- **Family ID:** `fam-connectivity-terminal-bridge`
...
Terminal, net tie, jumper, and domain adapter is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Terminal, net tie, jumper, and domain adapter**, **Terminal Bridge**, and `terminal-bridge`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-connectivity-terminal-bridge-terminal` | Terminal | F0, F1 | Realistic Electronics MVP | `pkg-virtual` |
| `var-connectivity-terminal-bridge-net-tie` | Net Tie | F0, F1 | Realistic Electronics MVP | `pkg-virtual` |
| `var-connectivity-terminal-bridge-jumper` | Jumper | F0, F1 | Realistic Electronics MVP | `pkg-virtual` |
| `var-connectivity-terminal-bridge-domain-adapter` | Domain Adapter | F0, F1 | Realistic Electronics MVP | `pkg-virtual` |
...
- Symbol ID: `sym-connectivity-terminal-bridge`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
- `GOLD-CON-TERMINAL_BRIDGE-NOMINAL`: nominal positive-path behavior.
- `GOLD-CON-TERMINAL_BRIDGE-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-CON-TERMINAL_BRIDGE-FAILURE`: diagnostic and fault behavior.
