...
- **Family ID:** `fam-analog-mixed-signal-regulator-reference`
...
Linear regulator and reference is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Linear regulator and reference**, **Regulator Reference**, and `regulator-reference`. Variant names are also aliases. Aliases never replace stable IDs.
...
- Symbol ID: `sym-analog-mixed-signal-regulator-reference`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
- `GOLD-AMS-REGULATOR_REFERENCE-NOMINAL`: nominal positive-path behavior.
- `GOLD-AMS-REGULATOR_REFERENCE-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-AMS-REGULATOR_REFERENCE-FAILURE`: diagnostic and fault behavior.
