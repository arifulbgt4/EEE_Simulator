# Sound and distance sensor

- **Family ID:** `fam-sensors-environment-sound-distance-sensor`
...
Sound and distance sensor is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Sound and distance sensor**, **Sound Distance Sensor**, and `sound-distance-sensor`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-sensors-environment-sound-distance-sensor-microphone` | Microphone | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-sound-distance-sensor-ultrasonic` | Ultrasonic | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-sound-distance-sensor-infrared-distance` | Infrared Distance | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
| `var-sensors-environment-sound-distance-sensor-lidar` | Lidar | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-sensor-module`, `pkg-sot-23`, `pkg-qfn`, `pkg-custom-parametric` |
...
- Symbol ID: `sym-sensors-environment-sound-distance-sensor`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `sound_pressure_full_scale` | Acoustic-pressure full scale | Pa | 2 | >0 |
| `distance_full_scale` | Distance full scale | m | 4 | >0 |
| `propagation_speed` | Propagation speed | m/s | 343 | >0 |
| `timeout` | Measurement timeout | s | 0.1 | >0 |
...
- **Governing relation or state rule:** Output follows the acoustic pressure or time-of-flight relation with propagation speed, range, blanking, echo/noise, and timeout behavior.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: Output follows the acoustic pressure or time-of-flight relation with propagation speed, range, blanking, echo/noise, and timeout behavior.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: Output follows the acoustic pressure or time-of-flight relation with propagation speed, range, blanking, echo/noise, and timeout behavior.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: Output follows the acoustic pressure or time-of-flight relation with propagation speed, range, blanking, echo/noise, and timeout behavior.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: Output follows the acoustic pressure or time-of-flight relation with propagation speed, range, blanking, echo/noise, and timeout behavior.
- **F4:** Electrothermal/tolerance/failure tier: extend the lower-tier relation with declared sampling, power-to-heat state Cth*dT/dt = P-(T-Tamb)/Rth, derating, and deterministic failure transitions; base relation: Output follows the acoustic pressure or time-of-flight relation with propagation speed, range, blanking, echo/noise, and timeout behavior.
- **Exact nominal vector:** pins `S:STIMULUS`/physical, `O:OUTPUT`/output, `VDD:VDD`/power, `GND:GND`/power; parameters `sound_pressure_full_scale`=2 Pa (>0); `distance_full_scale`=4 m (>0); `propagation_speed`=343 m/s (>0); `timeout`=0.1 s (>0); `response_time`=0.01 s (>=0).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-SEN-SOUND_DISTANCE_SENSOR-NOMINAL`, `GOLD-SEN-SOUND_DISTANCE_SENSOR-BOUNDARY`, `GOLD-SEN-SOUND_DISTANCE_SENSOR-FAILURE`.
...
- `GOLD-SEN-SOUND_DISTANCE_SENSOR-NOMINAL`: nominal positive-path behavior.
- `GOLD-SEN-SOUND_DISTANCE_SENSOR-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-SEN-SOUND_DISTANCE_SENSOR-FAILURE`: diagnostic and fault behavior.
