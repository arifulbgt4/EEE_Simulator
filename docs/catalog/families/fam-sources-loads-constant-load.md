...
- **Family ID:** `fam-sources-loads-constant-load`
...
Constant load is the canonical family for the variants listed below. The family boundary is behavioral: package, color, marking, tolerance, and vendor model choices do not create a new electrical family. Vendor-specific parts bind to a variant and an importable model.

## Aliases

Search aliases are **Constant load**, **Constant Load**, and `constant-load`. Variant names are also aliases. Aliases never replace stable IDs.

## Variants

| Stable variant ID | Display name | Model tiers | Release target | Compatible package profiles |
|---|---|---|---|---|
| `var-sources-loads-constant-load-constant-power` | Constant Power | F0, F1, F2, F3, F4 | Post-MVP catalog | `pkg-virtual` |
...
- Symbol ID: `sym-sources-loads-constant-load`. The artwork is original project work aligned with familiar IEC/ANSI conventions; IEC 60617 artwork must not be copied.
...
|---|---|---:|---:|---|
| `power_setpoint` | Constant-power setpoint | W | 1 | >=0 |
| `minimum_voltage` | Low-voltage regularization threshold | V | 0.1 | >0 |
| `maximum_current` | Compliance current | A | 10 | >0 |
| `slew_rate` | Power slew rate | W/s | 1e6 | >0 |
...
- **Governing relation or state rule:** The selected mode enforces R load (v = iR), constant current (i = Iset), or constant power (v*i = Pset) with explicit low-voltage regularization.
- **F0:** Connectivity-only: validate declared pins, domains, width/direction, hierarchy, and package mapping; do not claim numerical behavior. Family baseline: The selected mode enforces R load (v = iR), constant current (i = Iset), or constant power (v*i = Pset) with explicit low-voltage regularization.
- **F1:** Ideal/equation tier: implement exactly this family baseline and its declared parameter limits: The selected mode enforces R load (v = iR), constant current (i = Iset), or constant power (v*i = Pset) with explicit low-voltage regularization.
- **F2:** Behavioral/timing tier: preserve the family baseline using deterministic integer-tick state/event rules and explicit initialization: The selected mode enforces R load (v = iR), constant current (i = Iset), or constant power (v*i = Pset) with explicit low-voltage regularization.
- **F3:** Compact/macro/external tier: bind a pinned model or executable relation that preserves ordered pins and the validated envelope; the governing family relation is: The selected mode enforces R load (v = iR), constant current (i = Iset), or constant power (v*i = Pset) with explicit low-voltage regularization.
...
- **Exact nominal vector:** pins `1:P`/passive, `2:N`/passive; parameters `power_setpoint`=1 W (>=0); `minimum_voltage`=0.1 V (>0); `maximum_current`=10 A (>0); `slew_rate`=1e6 W/s (>0).
- **Boundary vector:** every declared inclusive/exclusive parameter limit, supported pin/domain/width edge, and supported-analysis boundary is exercised independently; combinations outside the declared envelope are invalid, not extrapolated.
- **Failure vector:** `open-circuit`, `short-circuit`, `parameter-drift`, `overstress-or-saturation`, plus non-finite parameters, invalid pin maps, unsupported analysis, and unavailable fidelity.
- **Golden evidence:** `GOLD-SRC-CONSTANT_LOAD-NOMINAL`, `GOLD-SRC-CONSTANT_LOAD-BOUNDARY`, `GOLD-SRC-CONSTANT_LOAD-FAILURE`.
...
- `GOLD-SRC-CONSTANT_LOAD-NOMINAL`: nominal positive-path behavior.
- `GOLD-SRC-CONSTANT_LOAD-BOUNDARY`: declared parameter and operating boundaries.
- `GOLD-SRC-CONSTANT_LOAD-FAILURE`: diagnostic and fault behavior.
